...
The `!in` operator applies a case-sensitive string or regex literal filter, and excludes events that match the specified criteria.
...
Filter the `logs` Dataset for `host` fields that don't have a case-sensitive match in the `foo` query.
