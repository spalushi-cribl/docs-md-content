...
The `in~` operator applies a case-insensitive string or regex literal filter.
...
Filter the `logs` Dataset for `host` fields that have a case-insensitive match in the `foo` query.
