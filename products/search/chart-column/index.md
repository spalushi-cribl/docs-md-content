...
Visualize your search results, using a Column Chart.

---

A **Column** Chart is best used to visualize categories of data. You could use a **Column** Chart to compare a
distribution of values over time, to monitor the number of CPUs used by machine type, or to visualze the number of
requests handled by a Pod.


![Column Chart](search-column-chart.png)
{scale="100%"}

**Column** is Cribl Search's default Chart type. (This type was labeled **Bar** in earlier Cribl Search versions.) To
switch the vertical orientation, select the [Horizontal Bar](chart-horizontal-bar) Chart type.
