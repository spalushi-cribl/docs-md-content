...
![Options to add or update an Edge Node](ed-add-new-node.20a4f130ed.png)
{border="true"}

> For Linux, the **Update** option in the UI adjusts the Leader connection details for the currently installed software. To upgrade the Edge Node – that is, install a newer version of the software – see [Upgrading](upgrading).
> 
> For Windows, the **Update** option in the UI generates an upgrade script. 
>
{.box .info}
...
1. In the resulting **Add Edge Node** modal, the **Install package location** defaults to `Cribl CDN`. If desired, change this to `Download URL`. 

1. As needed, correct the target **Fleet**, as well as the **Leader hostname/IP** (URI).
...
1. Optionally, add **Tags** that you can use for filtering and grouping in Cribl Edge. Use a tab or hard return between (arbitrary) tag names.
...
![Generating a script to bootstrap an Edge Node on Windows](ed-bootstrap-win.35734fc538.png)
{border="true"}
