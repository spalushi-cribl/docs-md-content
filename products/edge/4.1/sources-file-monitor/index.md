...
The File Monitor Source collects log (text) files, and generates events from lines or records in those files. This Source is available on Stream and Edge, but not on Cloud. 
...
- **Manual**: Tells the Source to discover the files within the **Search path** (a directory) that you specify, down to the **Max depth**. You must enter an absolute path (no wildcards) for the **Search path**, as File Monitor searches files in a single directory in Manual mode.
