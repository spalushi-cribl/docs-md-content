...
- On the list of all Edge Nodes. Navigate to **Edge Nodes** from the sidebar.

![The Filters option](filters-ui-4.15.png)
...
|Outpost|Which Outpost the Edge Node is connected to.|`info.outpost.host.toLowerCase().includes(<outpost-host>)`|
|Connection status|Whether the Edge Node is currently connected to the Leader.|`(disconnected === <true\|false>)`|
...
1. Type directly in the **Expression** field to fine-tune the filter.
   ...
   ![Filter Edge Nodes in JSON view](filter-nodes-advanced-data-4.13.png)
   {border="true" scale="75%"}

#### Supported Edge Node Properties

You can filter on these Edge Node properties using a JavaScript expression. Remember
...
**Disconnected Edge Nodes whose last heartbeat was before Jul 02 2025 15:30:00**

The `lastMsgTime` variable represents last heartbeat, and its value is a UNIX timestamp.
Convert the human-readable date into a timestamp, then use it as the value in the filter expression.

```js
disconnected === true && lastMsgTime >= 1751463000
```

**Edge Nodes running the System State Source**

```JS
lastMetrics.hasOwnProperty('health.inputs:input:system_state:in_system_state')
```

**Edge Nodes Running the System State Source and experiencing issues with Destinations**

```JS
lastMetrics.hasOwnProperty('health.inputs:input:system_state:in_system_state') && lastMetrics['health.outputs'] === 2
```

**Edge Nodes with errors in either Sources or Destinations**
