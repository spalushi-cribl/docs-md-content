...
- [JSON schemas](#schema-library) for validating JSON events, described on this page.
- [Parquet schemas](parquet-schemas) for writing data from a Cribl Edge Destination to Parquet files.
...
JSON schemas are based on the popular [JSON Schema standard](https://json-schema.org/), and Cribl Edge supports schemas matching that standard's [Drafts 0 through 7](https://json-schema.org/specification-links.html).

To access the schema library, select **Fleets** from the sidebar, and choose a Fleet.
Then, on the **Fleets** submenu, select **More**, then **Knowledge**, then **Schemas**.

Its purpose is to provide an interface for creating, editing, and maintaining schemas. 
...
- Making a decision before accepting an event (for example, drop an event if invalid).
- Making a decision to route an event based on the result of validation.

### JSON Schema Example {#json-example}

To add this example JSON Schema, on the **Schemas** page, select **Add Schema**.
Enter the following:
...
![Schema validation results for the above events](cribl-json-schema-validator-4101.png)
{border="true"}
