...
Configure the proxy server to handle HTTP/S requests.
...
> To set up Leader/Worker communication, use the [CRIBL_DIST_WORKER_PROXY](environment-variables#cribl_dist_worker_proxy) environment variable. 
>
{.box .info}
...
Several Cribl Edge endpoints rely on the HTTPS protocol – the Cribl [telemetry endpoint](licensing#telemetry-data), which must be accessed with some license types, as well as the CDN used to propagate application updates and certain documentation features (API Reference and docs PDFs).
...
If you are proxying outbound traffic and starting Cribl Edge using systemd, add your proxy environment variables to the systemd override file (see [Persisting Overrides](deploy-linux#persisting-overrides)). Add statements of this form:
...
If a proxy presents its own certificate, Cribl Edge must trust the entire certificate chain.

To ensure the entire chain is available:

1. Gather all the required CA certificates in PEM format, including the root and intermediate certificates.
1. Create a single PEM file containing all the certificates, in the correct order, starting with intermediate certificates, ending with the root. You can do it with the follow command:

   ```shell
   $ cat intermediate1.pem intermediate2.pem root_ca.pem > my_proxy_ca_chain.pem
   ```

1. Place the PEM file in a directory outside `$CRIBL_HOME`.
...
## Client Configuration and Leader Reference {#config}

The way a client references the Leader Node depends on its configuration, which originates from the Distributed Settings for that particular Node. These settings reside in the [`$CRIBL_HOME/local/_system/instance.yml`](instanceyml) file. 

When deploying a configuration update, the Leader Node informs all connected clients (Workers and Edge Nodes) within its Group or Fleet that a new configuration bundle is available. The Leader will reference itself using the same method (IP address or FQDN) that clients already use to connect to it in the distributed environment. This ensures a seamless download process for the clients, regardless of their initial connection method.

 If a Worker is configured to use the Leader's IP for communication, the URL will also contain that specific IP. Consequently, the `NO_PROXY` variable on the client must match the IP address used in the client configuration and Leader URL for optimal operation.

> Cribl v.4.6.0 introduces direct S3 downloads for configuration bundles, bypassing the Leader Node and scaling deployments with many Edge Nodes. However, if your Edge Nodes are firewalled or requires access controls, a proxy might be needed. In such scenarios, you need to exclude the S3 bucket address from the "NO_PROXY" list for successful downloads. 
>
{.box .info}
...
## Proxying Multiple Edge Instances in One Browser {#multiple}

Cribl Edge stores authentication tokens based on each http header's URI scheme, host, and port information. Within a given browser, Cribl Edge enforces a [same-origin policy](https://en.wikipedia.org/wiki/Same-origin_policy) to isolate instances.
