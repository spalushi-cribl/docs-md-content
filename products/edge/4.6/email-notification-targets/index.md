...
If you are a Cribl.Cloud user with an Enterprise license, you can use the [default email Notification target](#default-cloud-target). This target requires no configuration.
...
In Cribl.Cloud, Enterprise orgs can use `system_email` – the [default email notification target](#default-cloud-target) – and avoid configuring an SMTP server. If you **do** wish to configure an SMTP server in Cribl.Cloud, use any port other than port `25`.
...
### Sending Domain for Cribl.Cloud Email Notifications {#sending-domain}
