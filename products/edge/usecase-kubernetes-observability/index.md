...
Deploy, configure, and set data flow for a Kubernetes Cribl Edge environment.
...
## Visualize Your Kubernetes Cluster {#visualize}

Cribl Edge includes a powerful [Kubernetes Explorer UI](explore-edge-linux#kubernetes) that allows you to visualize and analyze your Kubernetes cluster data in real-time. This UI provides a comprehensive view of your deployments, pods, containers, metrics, and events, all in one place.

The Kubernetes Explorer UI can help you to:

- Identify trends and anomalies in your cluster performance.
- Troubleshoot issues quickly and efficiently.
- Optimize your cluster resource utilization.
- Gain deeper insights into the health and behavior of your applications.

We recommend using the Kubernetes Explorer UI in conjunction with the Kubernetes Sources described below to get the most out of your Cribl Edge deployment. 
...
### Configure Fleets & Fleet Settings on Cribl Edge {#fleets}
...
We recommend [configuring](fleets#configuring) a separate Fleet to manage your Kubernetes environment. It is easier to deploy or upgrade Kubernetes Edge Nodes using our [Helm Charts](#helm) when they are all in one Fleet. For best practices on configuring Fleets, see [Design Fleet Hierarchy](fleets-design).
...
![Generating a script to bootstrap an Edge Node on Windows](usecase-k8-2-4101.png)
{border="true"}
...
![Set limits](ed-settings-limits.png)
{border="true"}
...
## Configure the Kubernetes Sources  {#sources}
...
Below the **Capture** button, click the ellipsis (three dots or `…`) and then toggle **Show Internal Fields** on to explore the `__metadata` fields forwarded from the Cribl Edge Nodes.These metadata fields show information about the Pod, Node, and Cribl environment where they were collected.
...
Next, in the Fleet's submenu, go to **Collect** to attach Kubernetes Logs to Loki using the configured Pipeline
...
Next, in the Fleet's submenu, go to **Collect** to attach the Kubernetes Metrics Source (or Prometheus Edge Scraper) to the Prometheus Destination, using the  `prometheus_metrics1` Pipeline as the connection.
