...
```js
C.Time.adjustTZ(epochTime: number, tzTo: string, tzFrom: string = 'UTC'): number | undefined
```
...
```js
C.Time.clamp(date: T, earliest: T, latest: T, defaultDate?: T ): T  | undefined
```
...
```js
C.Time.strftime(date: number | Date | string, format: string, utc: boolean = true): string | undefined
```
...
```js
C.Time.strptime(str: string, format: string, utc: boolean = true, strict: boolean = false): Date
```
...
```js
C.Time.timestampFinder(utc?: boolean).find(str: string): IAutoTimeParser
```

Extracts time from the specified field, using the same algorithm as the [Auto Timestamp](auto-timestamp-function) Function and [Event Breakers](event-breakers#timestamp-settings).
...
```js
C.Time.timePartition(date: number | Date | string, level: 'd' | 'h'): string | undefined
```
...
|Parameter|Type|Description|
|---|---|---|
|`date` | number \| Date \| string | Datetime to extract the file path prefix for. |
|`level` | string | Granularity of the partitioning, `d` for daily, and `h` for hourly. |
...
```js
C.Time.s3TimePartition(date: number | Date | string, level: 'd' | 'h'): string | undefined
```
