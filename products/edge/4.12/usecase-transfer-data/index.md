...
Cribl Edge provides you with the ability to transfer data between Cribl
Edge Nodes that report to the **same** Leader or different Leaders, via the [Cribl
TCP](sources-cribl-tcp) and [Cribl HTTP](sources-cribl-http) Source and
Destination pairs. 

You can enable data sharing across Edge Nodes that do not share a Leader as long as both
...
- Data sharing between Cribl.Cloud and on-prem environments.
- Workers that are not upgraded (on 4.11.1 and older) communicating with
  upgraded Workers (4.12.0 and newer).  
...
- On Cribl-managed Edge Nodes in Cribl.Cloud, make sure that TLS is
...
This workflow assumes you already have data ingested in Cribl Edge, and you want
to move the data to a Edge Node in another Organization:
