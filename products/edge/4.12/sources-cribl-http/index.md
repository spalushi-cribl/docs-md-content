...
The Cribl HTTP Source receives data from a [Cribl HTTP Destination](destinations-cribl-http). Relaying data between a Cribl HTTP Destination and Cribl HTTP Source pair prevents double-billing: You won't incur any additional charges or credit usage after initial ingest to the first Fleet (depending
...
>
>For guidance on when to choose this Source versus the [Cribl TCP](sources-cribl-tcp) Source, see [Interoperation Protocols](/reference-architectures/reference-arch-full-suite/#protocols).
{.box .info}
...
You can use the Cribl HTTP Source to transfer data between Edge Nodes and Workers. If the Cribl HTTP Source receives data from its [Cribl HTTP Destination](destinations-cribl-http) counterpart on another Worker, you're billed for ingress only once – when Cribl first receives the data. 
