...
Cribl Edge's persistent queues (PQ) supplement in-memory queues to minimize data loss during a backpressure event. When a backpressure event occurs, Cribl Edge can engage a backpressure queue to temporarily store data by writing it to disk during the outage. When the outage resolves, Cribl Edge then forwards data from the queue.

![You can enable persistent queues at both ends of the data flow](cribl-diagram-in-outs.png)
{scale="100%"}

Cribl Edge's persistent queue (PQ) feature helps minimize data loss in these situations:

- A downstream Destination receiver is unreachable.
- A downstream Destination receiver is slow to respond. (This feature is only available in Cribl Edge 4.9.0 and newer.)
- When a configured Source sends a higher volume of data than Cribl Edge can process.
...
>
> The persistent queue feature is only available for certain Cribl licenses. See the [Pricing](https://cribl.io/pricing/) page for details.
>
{.box .info}
...
- For Kafka-based Destinations (such as [Kafka](destinations-kafka), [Confluent Cloud](destinations-confluent), [Amazon MSK](destinations-msk), and [Azure Event Hubs](destinations-azure-event-hubs)), persistent queues engage when Cribl Edge can't reach the brokers.
...
Cribl Edge supports persistent queueing for [Push Sources](/stream/sources#push). These sources ingest streaming data, which makes it possible to enable persistent queue.
...
To determine whether Cribl Edge supports persistent queue for a specific Source, consult the documentation for that Source.

## Supported Destinations {#supported-destinations}

Cribl Edge supports persistent queue for:
...
To determine whether Cribl Edge supports persistent queue for a specific Destination, consult the documentation for that Destination.
