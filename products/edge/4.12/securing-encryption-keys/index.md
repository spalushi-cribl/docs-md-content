...
- In a Distributed deployment with multiple Fleets, keys are managed per Fleet. Select **Manage** > **Groups** > `<groupName>` **Group Settings > Security > Encryption Keys**.
...
> Cribl Edge performs decryption only when the data is being sent to Splunk. For details, see [Decryption of Data in Splunk](/stream/securing-data-decryption)
{.box .success}
