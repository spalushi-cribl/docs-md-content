...
In Cribl Edge 4.2 and later, the Roles/Policies model described on this page exists in parallel with a new, more flexible [Members/Permissions](members) model, which will eventually replace it. To provide cross-compatibility, we have added several new [Default Roles](#default-roles) and [Default Policies](#permissions) that are counterparts to new Permissions. 
...
> Users are independent Cribl Edge objects that you can configure even without RBAC enabled. For details, see [Local Users](local-users).
>
{.box .success}
...
> In Cribl Edge v.2.4.x through 4.1.x, Roles are customizable only down to the Worker Group/Fleet level. E.g., you can grant Edit permission on Worker Group/Fleet `WG1` to User A and User B, but cannot grant them finer-grained permissions on child objects such as Pipelines, Routes, etc.
>
{.box .info}
...
--- | --- | --- |
**admin** | Superuser – authorized to do anything in the system. | N/A
...
**user** | Default Role that gets only a home/landing page to authenticate. This is a fallback for users who have not yet been assigned a higher Role by an admin. | N/A
**project_user** | **Deprecated as of v.4.2.** Read/write access to the simplified Stream [Projects](/stream/projects) UI and related data Subscriptions. <br/>  [Assign](/stream/sharing-projects) the new Project-level **Editor** Permission instead. | N/A
...
Let's highlight an example from the [above screen capture](#add-edit-role) of Cribl Edges built-in Roles: The `editor_all` Role has the `GroupEdit` Policy, with permission to exercise it on any and all Worker Groups/Fleets (as indicated by the `*` wildcard).
...
Name | Description | Permission Equivalent
--- | --- | ---
`GroupRead` | The most basic Fleet-level permission. Enables users to view a Fleet and its configuration, but not modify or delete the config. | Fleet-level [**Read Only**](members#group).
`GroupEdit` | Building on `GroupRead`, grants the ability to also change and commit a Fleet's configuration. | Fleet-level [**Editor**](members#group).
`GroupFull` | Building on `GroupEdit`, grants the ability to also deploy a Fleet. | Fleet-level [**Admin**](members#group).
`GroupCollect` | Grants the ability to create, configure, and run Collectors on a Worker Group. | N/A
`GroupUser` | Access Fleet. | Fleet-level [**User**](members#group).
...
Name | Description | Permission Equivalent
--- | --- | ---
`DatasetMaintain` | Full access to Dataset configuration. | Search Datasets [**Maintainer**](/search/sharing#resources).
`DatasetRead` | Can view and search, but not modify Dataset configurations. | Search Datasets [**Read Only**](/search/sharing#resources).
`DatasetProviderMaintain` | Full access to Dataset Provider configuration.  | Search Dataset Providers [**Maintainer**](/search/sharing#resources).
`DatasetProviderRead` | Can view and search, but not modify Dataset Provider configurations. | Search Dataset Providers [**Read Only**](/search/sharing#resources).
`SearchUser` | Search data and view results shared with the user | Search Product-level [**User**](/search/cloud-managing#search-member-roles).
`SearchMaintainer` | Manage Datasets, Dataset Providers, Dashboards and settings. | Search Product-level [**Editor**](/search/cloud-managing#search-member-roles).
...
Once you've defined a Role, you can associate it with Cribl Edge users. On the Leader Node, select **Settings** > **Global Settings** > **Access Management** > **Local Users**. For details, see [Local Users](local-users). 
...
## External Groups and Cribl Edge Roles {#groups}
...
1. Under **Role Mapping**, first select a Cribl Edge **Default role** to apply to external user groups that have no explicit Cribl Edge mapping defined below.

1. Next, map external groups as you've configured them in your external identity provider (left field below) to Cribl Edge Roles (right drop-down list below).
...
![Mapping external user groups to Cribl Edge Roles](role-ldap-mapping-composite.642d315413.png)

And here, we've set a conservative **Default Role** and one explicit **Mapping**:

![External user groups mapped to Cribl Edge Roles](role-ldap-mapped.04460283ba.png)
