...
Lookups are data tables that can be used in Cribl Edge to enrich events as they are processed by the [Lookup Function](lookup-function). You can access the Lookups library, which provides a management interface for all lookups, from Cribl Edge's top nav under **Processing** > **Knowledge** > **Lookups**. 
...
![Lookups Library](cribl-lookups.f7a8d99f7d.png)
{border="true"}
...
For large lookup files, you'll need to provide extra memory beyond [basic requirements](deploy-single-instance#requirements) for Cribl Edge and the OS. To determine how much extra memory to add to a Worker Node/Edge Node for a lookup, use this formula:
