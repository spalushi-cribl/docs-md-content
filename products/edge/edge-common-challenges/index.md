...
When changes you make to Cribl Edge configuration are not applied,
the first thing to check is whether you have committed and deployed them.
It is an easy to forget step that is necessary for most configuration changes to take effect.
...
To disable SNI routing for a Fleet: Navigate to **Fleet Settings** > **General Settings** > **SNI** > **Disable SNI Routing** option. **Commit** and **Deploy** and then proceed with the upgrade. You must enable this setting for every Fleet that is behind a web proxy or firewall.
...
- In the Leader Node's sidebar, select **Edge Nodes** and confirm there are Nodes currently
...
Leaders on v.4.2.x are compatible with Edge Nodes on v.4.1.2 and newer. Due to a security update, Edge Nodes running on v.4.0.4 cannot receive configurations from v.4.2.x Leaders. For details, see [Upgrade to 4.2.x](upgrading-troubleshooting#upgrade-42).
...
When you deploy Cribl Edge as part of a host image or VM, the presence of the `.dat` file can cause duplicate GUIDs.
To avoid it, stop Cribl Edge, then remove the `.dat` file.
The file will regenerate on the next run.
...
MSIs older than v.4.3.0 install the app so it belongs to the user running the installer. If you later try to upgrade it as another user, things go awry — resulting in a rather confusing mess with a seemingly random mix of the old and new package contents on disk.

Moving forward, we recommend adding `ALLUSERS=1` to the `msiexec` command when installing or upgrading versions older than 4.3.0 for testing. Example command: 
...
### Installation Issues with Leader Auth Token

If you encounter problems installing newer versions of Cribl Edge, this may be due to Leader `Auth token` issues. To resolve:

1. Verify the `Auth token` is correct and current in your Leader Node. (You can find the Auth token setting by navigating to **Settings** > **Global** > **System** > **Distributed Settings** > **Leader Settings** > **Auth token**.)
1. Check network connectivity between Edge Nodes and the Leader.
1. Validate successful connections with both Cribl Edge and Cribl Stream products after installation.
...
1. Configure Windows events via `XML` format in Cribl Edge (recommended over `JSON`).
1. Build a basic Pipeline/transformation to ensure all expected fields are present.
1. Verify fields are in the desired format after processing.
1. Consider implementing data reduction in your pipeline to optimize usage.

### PowerShell Performance Issues with Windows Sources

PowerShell-based collection for System State, Windows Metrics, and Windows Event Log Sources
is a legacy option that will be removed in a future release.

We highly recommend keeping this option disabled and using the newer native capabilities.

To disable PowerShell-based collection:

1. Upgrade to Cribl Edge `4.13.x` or newer.
1. Disable **Advanced Settings** > **Use Windows Tools**. 
...
- [Create a Fleet](fleets) dedicated to Windows Nodes, and constrain Windows Sources
  to that Fleet.

- If you want to collect logs only, disable the Windows Metrics Source.

- The [Users and Groups collector](sources-system-state#users-groups) 
  in the System State Source can trigger high CPU utilization in the Windows
  `lsass.exe` process where there are many user accounts on the host. This is
  not uncommon on Domain Controllers. Consider disabling the User and Groups
  collector if you see that process consuming excessive CPU.

Windows Event Logs Source optimization:

- During the Windows Event Log capture, run the collection for 10 seconds and
  tweak the settings if necessary so that collection doesn't always have to
  catch up. Consider changing the Windows Events Log Batch Size and Frequency.
...
- For Domain Controllers with heavy log volumes, consider using the Windows Event
  Forwarder and sending the logs to Cribl Stream, instead of using the Windows
  Event Log Source in Cribl Edge.

- If you're collecting Windows Events Logs in JSON format, you may experience
  disk space issues. You can either set XML as the event format on the Windows
  Events Logs Source, or upgrade Cribl Edge to v.4.8.x or newer and toggle off
  **Use Windows Tools**. See the [Windows Event Logs Source](sources-windows-event-logs#advanced-tab)
...
scan, you can add these commands to the appropriate Windows Edge Nodes:

```bash
Add-MpPreference -ExclusionProcess "C:\Program Files\Cribl\bin\nssm.exe"
Add-MpPreference -ExclusionProcess "C:\Program Files\Cribl\bin\cribl.exe"
Add-MpPreference -ExclusionPath "C:\ProgramData\Cribl"
```
...
### Empty Diag Files on Windows

For Cribl Edge Nodes running on Windows, set the `CRIBL_VOLUME_DIR` environment variable
before running the `diag` command. Otherwise, you might get empty diag files.
...
On Fleets running Edge Nodes with version 4.8.2 and older, the API server is configured be default to listen on the loopback interface (`127.0.0.1`).
If you have such nodes, change the listening interface address to `0.0.0.0` in **Fleet Settings** > **General Settings** > **API Server Settings** > **General**.
...
### Duplicated Nodes in Node List

Edge Nodes in Kubernetes restart with new GUIDs,
causing the **Edge Nodes** page to track and display each restart as a separate Node.
To prevent this behavior in Fleets containing Kubernetes nodes,
set **General Settings > Fleet Configuration > Time to keep disconnected Nodes** to 0.
This ensures that Nodes in these Fleets will not be tracked between restarts.
...
### Log Loss Due to Polling Interval and Rotation

The [Kubernetes Logs Source](sources-kubernetes-logs) polls the local kubelet for Pod discovery. While this is a lightweight operation, the **Polling interval** setting is important for complete log capture, especially with high log volumes and frequent rotations. For details, see [Polling Interval and Log Rotation](sources-kubernetes-logs#polling).


## Which Destination to Use for Cribl Stream

Always use Cribl TCP or Cribl HTTP as the Destinations to send data to Cribl Stream.  Otherwise, you might see duplication of certain data (such as host). Also, data sent to Cribl Stream via TCP/HTTP is not counted against your ingest data quota.
