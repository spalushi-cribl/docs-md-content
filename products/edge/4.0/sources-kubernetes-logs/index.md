...
**Enable timestamps**: When toggled to `Yes`, Cribl Edge prefixes a timestamp to each line of the container's raw console output. Use this toggle together with the example **Kubernetes Logs** Event Breaker Ruleset, which sets the `_time` property based on the prefixed timestamps. This combination is designed for working with containers whose console output lacks proper timestamps (either because the timestamps are missing altogether or because they specify time only but not date). More complicated logs might require tweaking the Event Breaker.
