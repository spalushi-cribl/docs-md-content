...
- Pending expiration of a Cribl Edge license. 

Notifications are not designed to take the place of alerts on your overall infrastructure's health – but they warn you about conditions that could impede expected data flow into and out of Cribl Edge.

> Notifications require an Enterprise or Standard [license](licensing), without which the configuration options described below will be hidden or disabled in Cribl Edge's UI.
>
{.box .success}
...
All Notifications will also be sent as events to Cribl Edge's [internal logs](monitoring#internal-logs-and-metrics) – both application-wide, and with a filtered view available on affected Sources and Destinations. The application-wide logs are recorded as `notifications.log` on the Leader Node. The Leader Node is also responsible for sending all Notifications.



You can also send any Notification to additional targets, using Cribl Edge's native PagerDuty integration and/or by specifying custom webhooks. For details, see [Configuring Targets](notifications-targets).

## Notifications and RBAC

Notifications work with Cribl Edge's role-based access control. For users with non-administrative permissions, their assigned [Roles and Policies](roles) determine the Worker Groups on which they can view Notification messages, and can create and manage Notifications and targets
...
The threshold for the Notification to trigger is: Cribl Edge detected a blocked or dropped state during ≥ 5% of the trailing **Time window** that you configure in the [Notification Settings](#notification-settings-d) below.



![Backpressure notification](backpressure-notification-4.0.dc009256dc.png)
...
![Persistent Queue notification](notification-pq-fly-out.0643974d96.png)
{scale="70%" border="true"}
...
1. Click **New Notification** to access the **New Notification** modal shown below.



![Configuring a Destination Notification (composite screenshot)](notification-dest-tab-new-composite.c1e16fdee9.png)
{border="true"}
...
![Configuring a High Data Volume Notification (composite screenshot)](se-notifications-source-config.35f230e5ad.png)
{border="true"}
...
![Configuring an expiration Notification (composite screenshot)](license-notifications-composite.9e029d0ba3.png)
...
This tab is where you centrally configure and manage targets that are available across Cribl Edge – to all Source-, Destination-, and license-based Notifications. See [Configuring Targets](notifications-targets) for details.
...
In future Cribl Edge releases, Cribl plans to expand the Notifications feature with options to configure additional triggering conditions and time resolutions.
