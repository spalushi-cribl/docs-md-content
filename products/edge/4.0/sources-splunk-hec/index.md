...
> This single endpoint supports both JSON events via `/event` and raw events via `/raw`. See the [examples](#examples) below.
>
{.box .info}
...
- Configure Cribl Edge to listen on port `10080` with an auth token of `myToken42`.
...
curl -k "http://<myCriblHost>:10080/services/collector/event?token=mToken42" -d '{"event":"this is a sample event ", "host":"myHost", "source":"mySource", "fieldA":"valueA", "fieldB":"valueB"}'
```
...
```shell
curl -k http://<myCriblHost>:10080/services/collector/raw -H 'Authorization: myToken42' -d '<146>Jul 2 08:40:30 tpapigw02 c2b_server_1: APIGW-C2B: timestamp="07.02.2021 08:40:30,968" ip="10.88.157.144" filterType="ChangeMessageFilter"'
```
