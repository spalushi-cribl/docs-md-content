...
In a single-instance deployment, Packs are global. From Cribl Edge's top-level navigation, just select **Processing** > **Packs**.

![Packs, single-instance navigation](packs-single-instance.ed1374dec3.png)
{border="true"}

#### Distributed/Default Fleet

In a distributed deployment with the default single Fleet (Leader mode), select **Manage**, click into the `default` group, and then select **Processing** > **Packs**.
...
In a [distributed deployment](/stream/deploy-distributed) with multiple Worker Groups/Fleets (Leader mode), Packs are associated with (and installed within) Worker Groups. Navigate to the parent Fleet, then select **Processing** > **Packs** from that Group's top nav.

![Fleet > Manage Packs page](packs-worker-group.74a421500b.png)
{border="true"}
...
The right pane defaults to displaying all sample data files available on your Cribl Edge instance. If you prefer to filter only sample files internal to the Pack, toggle the **In Pack only** slider to the right.
