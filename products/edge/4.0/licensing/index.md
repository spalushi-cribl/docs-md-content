...
You can add and manage licenses at **Settings** > **Global Settings** > **Licensing**. Click **+ Add License** to paste in a license key provided to you by Cribl.
...
When an Enterprise or Standard license expires, Cribl Edge will fall back to the Sales Trial or Free type. However, an expired Sales Trial license cannot fall back to a Free license.
...
With licenses that limit the number of Worker/Edge Node Processes, Cribl Edge will attempt to balance (or rebalance) Worker/Edge Node (threads) as evenly as possible across all licensed Worker/Edge Nodes.

On Cribl Stream (LogStream) 2.3 or later, and Cribl Edge, you need to configure licensing only on the Leader Node. (See [Managing Licenses – Adding and Renewing](#managing).) The Leader will push license information down to Worker Groups/Fleets as part of the heartbeat ([Edge](/edge/setting-up-leader-and-edge-nodes), [Stream](/stream/deploy-distributed)).
...
Forward [Cribl Internal](sources-cribl-internal) metrics to your Metrics Destination of choice, and run a report on `cribl.total.in_bytes`.
