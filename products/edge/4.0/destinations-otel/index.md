...
Cribl Edge supports sending events to [OTLP](https://opentelemetry.io/docs/specs/otlp/)-compliant targets. (Cribl Edge can receive OTel events through the [OTel Source](sources-otel).) Besides native OTel Trace and Metric events, you can send Cribl Edge’s Gauge metric events through this OTel Destination.
...
By default, includes `cribl_host` (Cribl Edge Node that processed the event). 
