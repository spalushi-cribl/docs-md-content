...
Cribl Edge's StatsD Extended Destination supports sending out data in expanded [StatsD](https://github.com/statsd/statsd) format.
...
Or, to configure via the [Routing](routes) UI, click **Data** > **Destinations** (Stream) or **More** > **Destinations** (Edge). From the resulting page's tiles or the **Destinations** left nav, select **Metrics** > **StatsD Extended**. Next, click **New Destination** to open a **New Destination** modal that provides the options below.
