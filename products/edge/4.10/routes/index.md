...
> There can be multiple Routes in a Cribl Edge deployment, but each Route can be associated with only **one processing** Pipeline or [Pack](packs). (Sources and Destinations can also have their own attached pre-/post-processing [Pipeline](pipelines#types-of-pipelines) or [Pack](packs).)
>
...
{caption="Data flow for a Route with Final toggled on"}
...
{caption="Data flow for a Route with Final toggled off"}
...
The `endRoute` bumper appears at the bottom of the Routing table. This is a reminder that, if **no** Route in the table has **Final** toggled on, events will continue to Cribl Edge's configured [Default Destination](destinations-default).
