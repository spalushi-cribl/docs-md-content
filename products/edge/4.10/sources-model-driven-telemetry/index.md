...
Cribl Edge supports receiving network device metrics and events via Model Driven Telemetry (MDT).
...
When you configure MDT on a network device you choose among a variety of options and techniques. For this Source (in Cribl Edge version 4.5), you must configure your upstream sending device to use the following:
...
## Configure Cribl Edge to Receive Model Driven Telemetry Data
...
- **Validate client certs**: Reject certificates that are not authorized by a CA in the **CA certificate path**, or by another trusted CA (such as the system's CA). Default is toggled on.
