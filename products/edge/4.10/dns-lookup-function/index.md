...
- Reverse DNS Lookup. (This duplicates the behavior of Cribl Edge's prior [Reverse DNS](reverse-dns-function) Function, which was deprecated as of v.2.4, and has been removed as of v.4.0.)
...
### Reverse DNS Lookup Fields Section {#reverse}
...
**Use /etc/resolv.conf**: Toggle on if you want Cribl Edge to resolve DNS short names using the search or domain directive from `/etc/resolv.conf`. Default is toggled off. Not applicable for Windows hosts.

**Use search or domain fallback(s)**: Add fallback value(s) for the DNS resolver to use when it cannot resolve a DNS short name.

**Fall back to DNS.lookup()**: Toggle on if you want Cribl Edge to make a `DNS.lookup()` call to resolve a DNS short name it can't resolve by other methods.
...
**DNS server overrides**: IP address(es), in [RFC 5952](https://tools.ietf.org/html/rfc5952) format, of the DNS server(s) to use for resolution. IPv4 examples: `1.1.1.1`, `4.2.2.2:53`. IPv6 examples: `[2001:4860:4860::8888]`, `[2001:4860:4860::8888]:1053`. If this field is not specified, Cribl Edge will use the system's DNS server.
...
**Cache size limit**: Maximum number of DNS resolutions to cache locally. Before changing the default `5000`, consider the implications for your system. Higher values will increase memory usage. Highest allowed value is `100000`.

**Log level for failed lookups**: The log level at which to capture failed lookup events. Defaults to error. 
...
See [Monitoring](monitoring) for a general explanation of logging in Cribl Edge.
...
![DNS Lookup: Eval Function](dns-lookup-eval-4101.png)
{border="true"}
...
![DNS Lookup: multiple record types](dns-lookup-direct-4101.png)
{border="true"}
...
![DNS lookup with multi-value result](dns-multi-first-4101.png)
{border="true"}
...
![DNS lookup multi-value result converted into a single value](dns-multi-second-4101.png)
{border="true"}
