...
A Fleet's General Settings allows you to configure teleporting, throughput
 ...
 **Tags**: Use tags to organize Fleets into logical categories. Then, you can search by tags on the **View all Groups**
  (Stream) or **View all Fleets** (Edge) interface. This search filters the list of Fleets, showing only those with the tag you entered.
 
 **Enable teleporting to Workers**: Toggle on to enable authenticated access to Workers' UI from the Leader ([Stream](/stream/setting-up-leader-and-worker-nodes#teleporting), [Edge](/edge/explore-edge#teleport)). Toggle off to disable.

In Cribl Edge 4.1.2 and later, you can configure the following option in [Shutdown Settings](#shutdown).
...
[Snippet not found: content/shared/4.10/snippets/_api-server-settings-authentication.md]
...
**Worker Process PQ size limit**: Highest accepted value for the **Max queue size** option used in individual Sources' and Destinations' persistent queues. Default: `1 TB`. Consult Cribl Support before increasing beyond this value.
...
**Disable jobs/tasks**: Toggled on by default in Edge Fleets and toggled off by
default in Stream Groups. In Edge, Nodes no longer poll the Leader for
upgrade jobs. Toggle on in Edge to reduce application load from the
Leader.
...
**Disable jobs/tasks**: When enabled, the Edge Nodes won't poll the Leader for
jobs/tasks. The job limits settings below will not affect Edge Nodes on version
4.5.0 and newer. Edge Nodes running 4.4.4 and older still use these jobs
...
**Artifact reaper period**: Interval on which Cribl Edge attempts to reap jobs'
...
mode, it sends all invalidations, requiring more processing by Cribl Edge.

##### Connections {#redis-connections-limit}

**Reuse Redis connections**: Toggle on if you want Cribl Edge to try to reuse Redis connections when multiple [Redis Functions](redis-function) (or references to them) are present. 
...
**Use Windows tools to collect process info**: Enable PowerShell to collect process information instead of using the native API.

> When using PowerShell to collect process information, the environment variables displayed in the [**Explore** > **Processes**](/edge/explore-edge-win#processes) tab might be inaccurate.
