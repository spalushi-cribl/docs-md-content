...
> 
> In [distributed deployments](/stream/deploy-distributed), ensure that the MaxMind database file is in the same location on the Leader Node and all Edge Nodes. However, if you've uploaded `.mmdb` files via Cribl Edge's [Lookups Library](lookups-library) UI, just select this box to display and select them on a drop-down list. Your selection here will handle the path management automatically.
>
{.box .info}
