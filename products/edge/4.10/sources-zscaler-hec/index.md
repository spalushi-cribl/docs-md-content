...
* **API URL**: The URL varies depending on your Cribl environment. Use the same port and HEC endpoint you define in your Zscaler Cloud NSS Source.
  * **Cribl.Cloud**: Enter the [Workspace **Public Ingress Address**](workspaces#stream-worker-group-details) in the following format: `https://<group-name>.<workspaceID>.<your_unique_cribl_cloud_name>.cribl.cloud:<port><hec-endpoint>`
  * **On-prem**: Enter the IP address or FQDN for either your Cribl Edge instance, or the load balancer you’re using with your Cribl Edge instances.
* **HTTP Headers**:
