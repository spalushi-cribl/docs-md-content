...
or install the Cribl Edge agent on a physical or virtual machine.
...
Before you continue, make sure your machine meets the minimum deployment requirements for Edge.
Edge Nodes should have sufficient CPU, RAM, network, and storage capacity to handle your **specific** workload.
It's very important to test this before deploying to production.
...
You’ll get a ready-to-use Cribl Leader that includes Cribl Edge functionality.
...
1. Select an AWS Region to host your Cribl.Cloud Leader and Cribl-managed Workers. Cribl currently supports the following Regions: `US West (Oregon)`,  `US East (Virginia)`, `Europe (Frankfurt)`, and `Europe (London)`.
...
> - [Managing Cribl.Cloud](cloud-managing).
{.box .success}
...
> To add an Edge Node on a Windows or using Docker or Kubernetes, refer to [Add/Update Edge Nodes](managing-edge-nodes#add-node).
{.box .success}
