...
Many Linux distros maintain a system cleaner service (e.g., [systemd-tmpfiles](https://man.archlinux.org/man/systemd-tmpfiles-clean.service.8)) that removes files from this directory periodically, such as every 10 days. If Cribl's sockets are removed, this breaks certain UI pages, such as those for [Fleets](managing-edge-nodes#fleets-tab) and [Monitoring](monitoring). You can protect the sockets in either of two ways.
...
The `-H` and `-p` parameters are required. For other options, see the [CLI Reference](cli-reference#mode-Edge).For other options, see the [CLI Reference for mode-managed-edge](cli-reference#mode-managed-edge). Here is an example command:
