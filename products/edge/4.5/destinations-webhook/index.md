...
**Load balancing**: When enabled (default), lets you specify multiple [Webhook URLs](#webhook-urls) and load weights. With the default `No` setting, if you notice that Cribl Edge is not sending data to all possible IP addresses, enable **Advanced Settings** > **Round-robin DNS**.
