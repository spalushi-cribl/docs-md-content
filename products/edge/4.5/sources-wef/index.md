...
Keeping the **Keep-alive timeout** low (such as the default 90 seconds) will minimize Cribl Edge memory usage (due to long-lived open connections), but it will result in an additional network round-trip on each batch of events delivered.
...
We recommend you set either **Subscriptions** > **Batch timeout** OR **Subscriptions** > **Heartbeat** to be less than or equal to **Keep-alive timeout**.
