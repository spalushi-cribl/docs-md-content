...
**Enforce required fields**: When toggled to `Yes` (default), v2c and v3 traps must meet the following requirements:
...
**Drop failed events**: When toggled to `Yes` (default), events that fail to serialize into valid traps are dropped entirely, preventing them from passing through the Pipeline. This is useful when sending to an SNMP Trap Destination, as invalid traps will not be sent.

When toggled to `No`, the event will pass through without `__snmpRaw` and will include a `snmpSerializeErrors` field for troubleshooting.

Regardless of whether the toggle is set to `Yes` or `No`, if an event fails to serialize, the `__snmpRaw` field is deleted, preventing the event from leaving the SNMP Trap Destination. 

Recommended setup flow:

1. Get a sample of the traps to modify and configure the SNMP Trap Serialize Function.
1. Enable **Drop failed events** and run a [Preview](data-preview).
