...
In your IDP (identity provider), configure user groups that match Permissions for both Cribl.Cloud [Organizations](members#invite-members#member-roles) and individual [products](permissions#product).
...
If you are not using [Teams](teams), or want to grant [Organization-level](members#invite-members#member-roles) permissions to IDP users,
