...
This is because Cribl Edge creates a separate TCP connection to Redis for every Redis Function and every _reference to_ a Redis Function.
...
1. In the sidebar, select **Fleets** and select the Fleet you want to configure.
1. On the **Fleet** submenu, select **Fleet Settings**.
1. Select **General Settings** > **Limits** > **Redis** > **Connections**. 
1. Toggle **Reuse Redis connections** to **Yes**.
1. Configure [**Max number of connections**](#number-of-connections) and confirm with **Save**.

> In a Single-instance deployment, the Redis connections settings are located in the main **Settings** menu.
{.box .info}

When you enable **Reuse Redis connections**, Cribl Edge attempts to reuse an existing Redis connection instead of creating a new one if:

- A new Redis connection is necessary, and
- The new connection's properties match those of an existing, open connection.

The [example](#managing-example) below illustrates how powerfully this can affect the demands Cribl Edge places on a Redis server.

### Determining Number of Connections {#number-of-connections}

[**Max number of connections**](settings-group-fleet#redis-connections-limit) is the upper limit of connections to Redis that Cribl Edge can establish before starting to reuse them.
...
> be careful not to reduce **Max number of connections** _below_ the current connection number.
{.box .info}
...
Enable **Reuse Redis connections** and set **Max number of connections** to `4`. Now revise the calculation:
...
Server-assisted caching is a remedy for this problem. With server-assisted caching enabled, your Redis instance will notify your client-side cache when Redis determines that a key's value has changed. Then the Cribl Edge client-side cache will immediately evict the stale key. Now the client-side cache is more accurate. Redis also has [documentation](https://redis.io/docs/manual/client-side-caching/) about server-assisted client-side caching.
...
1. In **Broadcast** mode, the Redis server need not remember which keys came from where, because it simply notifies Cribl Edge when the value of **any** key has changed. This is most efficient for Redis, but now the Cribl Edge client needs to inspect every key that Redis says has changed. This consumes some extra processing time on the client, and generates extra network activity to accommodate notifications about keys that Cribl Edge never sees.
