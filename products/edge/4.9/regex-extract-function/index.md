...
The Regex Extract Function extracts fields using named capture groups. Fields that start with `__` (double underscore) are special in Cribl Edge. They are ephemeral: any Function downstream can use them, but they will be serialized only to Cribl [internal Destinations](destinations#internal-destinations).
