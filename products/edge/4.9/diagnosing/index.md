...
To help diagnose Cribl Edge problems, you can share a diagnostic bundle with Cribl Support. The bundle contains a snapshot of configuration files and logs at the time the bundle was created, and gives troubleshooters insights into how  Cribl Edge was configured and operating at that time. 
...
If you're managing your own  Cribl Edge deployment (single-instance [Stream](/stream/deploy-single-instance/)/[Edge](/edge/deploy-single-instance#run-edge/) or distributed [Stream](/stream/deploy-distributed/)/[Edge](/edge/fleet-management/)), you can create and securely share bundles with Cribl Support, either from the UI or via the CLI. 
...
Using either medium, you'll need outbound internet access to `https://diag‑upload.cribl.io` and a valid support case number. The above URL works only when using the `cribl diag` command or uploading using the  Cribl Edge UI. (So connecting directly to it with your web browser will fail.)
...
Depending on the issue you're debugging, you can create a bundle either from the Leader or from an individual Edge Node.

#### UI Options {#ui-options}

The **Create & Export Diag Bundle** modal provides these additional options:
- **Rename .js files**: When enabled, appends `.txt` to JavaScript files in the bundle. Defaults to `Yes`.
- **Include metrics**: Includes metrics in the bundle. Defaults to `Yes`.
- **Include git**: Includes the last 14 days of git log in the bundle, identifying each commit's message, author, timestamp, and files changed. Defaults to `Yes`.
- **Latest number of jobs to include**: The number of jobs (and all tasks) the bundle will include. Defaults to empty, which means unlimited jobs.
- **Include install logs**: Includes Windows MSI logs, only available on
  Windows deployments of Cribl Edge. Defaults to `No`.
...
- To share the bundle with Cribl Support, toggle **Send to Cribl Support** to `Yes`. Enter your case number, and then select **Export**. 
- To download the bundle locally to your machine (on-prem only), select **Export**.

#### From Edge Nodes {#worker}



To create a bundle from individual Edge Nodes, you must be able to access the Edge Node's UI. You can easily use the Leader to get authenticated access to each Edge Node, if you first enable the parent Fleet's UI Access (teleporting) setting:

1. In the Cribl Edge sidebar, select **Fleets**.
2. Make sure the relevant Fleet's **UI Access** toggle is on. 
3. Select the displayed number of Edge Nodes to access the **Edge Nodes** page.
4. In the **GUID** column, select the link for the Edge Node you're debugging. 
5. Look for the purple border to confirm that you've teleported to that Edge Node.
6. Select **Edge Node Settings** (top right) > **Diagnostics** (lower left) > **Diagnostic Bundle**, and select **Create Diagnostic Bundle**.
...
To create a bundle using the CLI, use the `diag` command. (This option is on-prem only.) Depending on the issue you're debugging, you can create a bundle either from the Leader host or from an individual Edge Node host.
...
1. Use `top` or `htop` on the Edge Node to identify Edge Node PIDs consuming a lot of CPU.
...
To automatically capture memory snapshots when Worker Processes approach or exceed memory limits, turn on the **Enable heap snapshots setting** in the Fleet's **General Settings**. Navigate to **Fleet Settings** > **System** > **Worker Processes** and toggle the setting to `Yes`. Only the two most recent heap snapshots are retained. Older snapshots are automatically deleted. This behavior cannot be modified. 
...
For details, see [Worker Processes](settings-group-fleet#processes).

### Manually Capture a Memory Snapshot

#### Prerequisites

To manually generate a reliable memory snapshot for Cribl Edge diagnostics, ensure two key conditions are met:

1. **Correct `CRIBL_HOME` Environment Variable**: This variable points to the Cribl Edge installation directory. Open a new terminal session and verify its value using `echo $CRIBL_HOME`. If empty, set it appropriately for your operating system (for example,`export CRIBL_HOME=/path/to/cribl` on Linux, ` setex cribl C:\Program Files\Cribl /m` on Windows).

2. **Run as the `cribl` User**: The `diag heapsnapshot` command requires permissions to create the snapshot file within the run Cribl Edge directory. Use `sudo su - cribl` to switch to the cribl user before running the command. Remember to exercise caution with `sudo`. On Windows, if Cribl is set to run under a specific user account, log in as that user before running Cribl. Alternatively, you can run  Cribl Edge as an `Administrator`.
...
2. In a terminal, `cd` to the directory where  Cribl Edge is installed.
    ```shell
       cd $CRIBL_HOME/bin
    ``` 
...
1. Navigate to the Cribl Edge installation directory: For example, `C:\Program Files\Cribl\bin` or `$CRIBL_HOME\bin`. 
