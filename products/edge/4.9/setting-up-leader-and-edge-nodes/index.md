...
1. Confirm or enter the required Leader settings (**Address** and **Port**).
   Customize the optional settings if desired. 
1. Select **Save** to restart the Cribl Edge server.
...
1. For each desired Fleet, toggle on **UI Access**.

  ![Manage Fleets](edge-node-ui-access.png)
...
The command requires the `-H` and `-p` parameters. For other options, see the [CLI Reference for mode-managed-edge](cli-reference#mode-managed-edge). Here is an example command:
