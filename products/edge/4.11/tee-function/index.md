...
> In Cribl.Cloud, the Tee Function is only available on customer-managed [hybrid](cloud-enterprise#hybrid) Edge Nodes.
{.box .cloud}
...
**Args**: Select **Add Arg** to supply arguments to the command.

**Restart on exit**: Restart the process if it exits and/or we fail to write to it. Defaults to toggled on.

**Environment variables**: Environment variables to set or overwrite. Select **Add Variable** to add key-value pairs.
...
We’ve also selected **Add Arg**, to specify a local output file in the resulting **Args** field. (A file path would normally be the first argument to a `tee` command executed from the command line. The Cribl Edge user must have write permission on the specified file path.)
