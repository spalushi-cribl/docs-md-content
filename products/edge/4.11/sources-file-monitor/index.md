...
2. In the **New Source** modal, configure the following under **General Settings**:
   ...
   - **Discovery Mode**: Use the buttons to select one of these options: **Auto** or **Manual**. For details, see [Discovery Mode](#mode) below.
...
### Discovery Mode {#mode}
The File Monitor Source offers two options: 
...
Selecting **Manual** mode exposes additional configuration options:

-  **Suppress errors when search path does not exist**: Enable this option to hide
    `Path does not exist` errors when a directory doesn't exist. When enabled, 
    missing directories will not affect the File Monitor Source health status.
    This is useful when you know the search path
    contains nonexistent directories for some hosts, and you don't 
    want to see the error messages in the File Monitor status and logs.
-  **Delete files** configuration option within the [**Advanced Settings** ](#advanced) tab.
