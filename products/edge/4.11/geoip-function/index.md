...
You need to host the `.mmdb` database file. The following steps cover this process at a high level. They link to our [Managing Large Lookups](/stream/large-lookups) topic, where you can find additional details.
...
1. Optionally, set up automatic updates to the [MaxMind](/stream/large-lookups#automatic-updates-to-the-maxmind-database) and [IPinfo](/stream/large-lookups#automatic-updates-to-the-ipinfo-database) database files.
...
> 
> In [distributed deployments](/stream/deploy-distributed), ensure that the database file is in the same location on the Leader Node and all Edge Nodes. However, if you've uploaded `.mmdb` files via Cribl Edge's [Lookups Library](lookups-library) UI, just select this box to display and select them on a drop-down list. Your selection here will handle the path management automatically.
>
{.box .info}
