# Knowledge Libraries


Select **Processing** > **Knowledge** to access Cribl Edge's libraries of default Lookup tables, Event Breaker rulesets, Parsers, Global Variables, Regex expressions, Grok patterns, Schemas, Database Connections, and AppScope config templates. You can customize these libraries by adding your own Knowledge objects.

From the **Processing** > **Knowledge** top nav, you can select:

  * [Lookups](lookups-library)
  * [Event Breakers](event-breakers)
  * [Parsers](parsers-library)
  * [Global Variables](global-variables-library)
  * [Regexes](regex-library)
  * [Grok Patterns](grok-patterns-library)
  * [Schemas](schema-library) (JSON)
  * [Parquet Schemas](parquet-schemas)
  * [Database Connections](/stream/database-connections) (Cribl Stream only)
  * [HMAC Functions](/stream/hmac-functions) (Cribl Stream only)
  * [AppScope Configs](appscope-configs)
