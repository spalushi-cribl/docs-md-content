...
* **File Monitoring**: Enhance the efficiency of the file monitoring phase, by filtering out older, irrelevant files. Adjust the **Maximum age duration** and **Check file modification** **times** settings. 
...
The  **Maximum age duration** and **Check file modification** **times** settings apply to both **Manual** and **Auto** discovery modes in the File Monitor.
...
* **Setting age duration alone reads all files:**  Without enabling **Check file modification times**, the File Monitor will read all files that match the specified **Filename allowlist**. No age criteria will be applied to the entire file. However, the **Maximum age duration** will still be used to filter events within the files that have a timestamp older than the specified duration.
* **Setting both ensures efficient filtering:** To achieve optimal filtering, set both **Maximum age duration** and **Check file modification times**. This configuration allows the File Monitor to skip files that are older than the specified duration, based on their modification time. And within the remaining files, it will skip events that are older than the specified duration.
