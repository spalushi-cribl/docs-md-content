...
Once enabled on a specific Source, PQ will engage when any of the following occur:
...
* Source PQ is set to `Always On` mode.

When PQ engages on a Source:
* Events queue **before** any pre-processing Pipeline takes effect.
* If at least one Destination among a pool of Destinations sends an unhealthy signal, this Source will queue new events to disk even if some peer Destinations remain up. Note that this will delay data delivery to the peer healthy Destinations.
...
These Notifications always appear in Cribl Edge's UI and internal logs. You can also send them to external systems. For setup details, see [Destination-State Notifications](/stream/notifications#destination).
