...
Cribl Edge supports sending data to the [Exabeam](https://docs.exabeam.com/) security operations platform or (as some people think of it) SIEM.

The Exabeam Destination supersedes and improves upon the "old way" to get data from Cribl Edge to Exabeam [using the Cribl Webhook Destination](https://cribl.io/blog/a-winning-trio-cribl-stream-the-cribl-packs-dispensary-and-exabeam-for-effective-data-management/).

> Type: Non-Streaming | TLS Support: Yes | PQ Support: No
>
{.box .info}

## Understanding How Exabeam and Cribl Edge Work Together {#understanding}
...
Click the **New Collector** button, and in the resulting **Collector for Cribl** drawer:  

1. Enter a name for your new Cribl Cloud Collector.  
1. Optionally, in **Advanced Settings**, configure the **Metadata** values **SITENAME** and **TIMEZONE**. Keep the values you entered handy – you'll need them when configuring the Cribl Edge Exabeam Destination.
    > Cribl Edge version 4.4.4 copies the value of **SITENAME** into another **Metadata** field named **SITE ID**. Therefore, when **SITENAME** is set, both metadata fields will have values. Cribl recommends that you do not leave **SITENAME** blank.
    >
    {.box .info}
2. Click **Install**.
...
1. Click **Go to Overview** to exit the modal.
...
* **Google Cloud Storage bucket**: Name of the destination bucket – in the Exabeam Cribl Cloud Collector, this is called **GCS Bucket Name**. This value can be a constant, or a JavaScript expression that can be evaluated only at init time. For example, you can reference a Global Variable like this: `myBucket-${C.vars.myVar}`.

* **Google Cloud Storage bucket region**: In the Exabeam Cribl Cloud Collector, this is called **GCS Bucket Region**. It is the [GCS region](https://cloud.google.com/storage/docs/locations) where the bucket is located.

* **Collector instance ID**: In the Exabeam Cribl Cloud Collector, this is called **Instance ID**.

* **Access key**: In the Exabeam Cribl Cloud Collector, this is called **Access Key**.

* **Secret**: In the Exabeam Cribl Cloud Collector, this is called **Secret Key**.

If you configured the optional settings when creating your Cribl Cloud Collector, then enter the appropriate values.

* **Site name**: In the Exabeam Cribl Cloud Collector, this is called **SITENAME**.

* **Timezone offset**: In the Exabeam Cribl Cloud Collector, this is called **TIMEZONE**.
