...
3. Next, you can configure the following **Optional Settings**: 
     ...
     - **Compression**: Codec to compress the data before sending to Kafka. Select `None`, `Gzip` `Snappy`, `LZ4`, or `ZSTD`.
...
This section governs Kafka Schema Registry Authentication for data encoded with Avro or JSON schemas, with schemas stored in the Confluent Schema Registry. Cribl Edge determines the schema type by inspecting the schema registered in the Confluent Schema Registry for the provided key and value schema IDs.
