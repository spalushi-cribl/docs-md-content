...
1. Next, you can configure the following **Optional Settings**:
    ...
    - **Tags**: Add tags that you can use to filter and group Sources in Cribl Edge's UI. These tags aren't added to processed events. Use a tab or hard return between (arbitrary) tag names.
...
**CORS allowed headers**: List HTTP headers that Cribl Edge will send in a CORS preflight response to your configured **CORS allowed origins** as `Access-Control-Allow-Headers`. Enter `*` to allow all headers.
