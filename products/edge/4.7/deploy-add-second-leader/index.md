...
To handle unexpected outages in customer-managed (on-prem) distributed deployments, Cribl Edge supports configuring a standby Leader for failover. In this High Availability (HA) scenario, if the primary Leader goes down, Collectors and Collector-based Sources can continue ingesting data without interruption. 
...
There is only ever one active Leader Node at one time. The standby Leader will become active only in the event of failover. In the configuration for all Leaders, you must specify the same **failover volume** – a shared Network File System (NFS) volume.
...
- Cribl Edge will recover by switching to the standby Leader.
...
> Prior to Cribl Edge 4.7, it was possible for the primary and standby Leaders to be configured differently, potentially causing issues, especially around authentication and authorization.
> 
{.box .info}
...
For detailed information on the `/health` endpoint's query and response formats, refer to the [Query the Health Endpoint](/api/query-health-endpoint) documentation.
...
### How Cribl Edge Manages Leader Settings
...
2. In the **Failover volume** field, enter the NFS directory to support Leader failover. This directory must be outside of `$CRIBL_HOME`. One valid solution is to use `CRIBL_DIST_MASTER_FAILOVER_VOLUME=<shared_dir>`. See [Using Environment Variables](#env-vars) for more information.

3. Optionally, adjust the **Lease refresh period** from its default `5s`. This setting determines how often the primary Leader tries to refresh its hold on the Lease file.

4. Optionally, adjust the  **Missed refresh limit** from its default `3`. This setting determines how many Lease refresh periods elapse before standby Nodes attempt to promote themselves to primary.

5. Click **Save** to restart.


> In Cribl Edge 4.0.3 and later, when you save the **Resiliency**: `Failover` setting, further **Distributed Settings** changes via the UI will be locked on both the primary and backup Leader. (This prevents errors in [bootstrapping Workers](/stream/deploy-workers) due to incomplete token synchronization between the two leaders.) However, you can still update each Leader's distributed settings by modifying its configuration files, as covered in the very next section.
>
{.box .warning}
...
You can configure additional Leader Nodes via the following environment variables: 
 ...
 - `CRIBL_DIST_MASTER_FAILOVER_VOLUME=<shared_dir>`: Sets the location of the NFS directory to support Leader failover – for example, `/mnt/cribl`.
...
1. Stop all Leaders.
1. Upgrade the primary Leader for your [Stream](/stream/upgrading/) or [Edge](/edge/upgrading/) deployment.
1. Upgrade standby Leaders.
1. Start each Leader again, one by one.
1. Upgrade each Edge Node, respectively.

## Disabling the Standby Leader 
