...
Depending on the issue you're debugging, you can create a bundle either from the Leader or from an individual Edge Node's host. 
...
To create a bundle using the CLI, use the `diag` command. (This option is on-prem only.) Depending on the issue you're debugging, you can create a bundle either from the Leader's host or from an individual Edge Node's host.
...
To generate a reliable memory snapshot for Cribl Edge diagnostics, ensure two key conditions are met:
