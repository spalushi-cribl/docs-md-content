...
* Fields that start with a double-underscore are known as **internal fields**, and each [Source](sources) can add one or many to each event. For example, [Syslog](sources-syslog) adds both a `__inputId` and a `__srcIpPort` field. Internal fields are used only within Cribl Edge, so they are serialized only to Cribl [internal Destinations](destinations#available-internal-destinations).
