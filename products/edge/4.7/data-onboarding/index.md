...
Use a sample of your real data in [Data Preview](data-preview). Sample data can come from a sample Source file that you upload or paste into Cribl Edge. 
...
While events can be processed almost arbitrarily by Functions in Cribl Edge Pipelines, make sure you understand the [event processing order](event-processing-order). This is very important, as it tells you exactly where certain processing steps occur. For instance, as we'll see just below, quite a few steps can be accomplished at the Source level, before data even hits Cribl Edge Routes.

![Source-level processing options](source-level-processing-options.3d1eef6a65.png)
{border="true"}
