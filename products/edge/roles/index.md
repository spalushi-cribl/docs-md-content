...
For on-prem deployments, the Roles/Policies model exists in parallel with a more flexible [Members/Permissions](members) model.
...
> [Which Access Model Should I Use?](access-management#restrictions) describes
> the differences between the Roles/Policies model and the Members/Permissions model
> for on-prem deployments.
> 
> Roles/Policies are not supported on Cribl.Cloud. Use the [Members](members)/[Permissions](permissions) model on Cribl.Cloud.
{.box .success}
...
The Roles/Policies model is available only for on-prem deployments. On Cribl.Cloud, use the [Members](members)/[Permissions](permissions) model.
...
**org_iam_admin** | Manage the Organization SSO configuration and invite, update, and delete Members without having access to data engineering functions. | Organization [IAM Admin](permissions#organization)
...
#### Fleet–Level Roles {#group-level-roles}
...
>
> When you first install Cribl Edge with the prerequisites to enable RBAC (Distributed deployment with an [Enterprise license](https://cribl.io/pricing/)), you will be granted the **admin** Role. Using this Role, you can then define and apply additional Roles for other users.
> 
...
Let's highlight an example from the [above screen capture](#add-edit-role) of Cribl Edges built-in Roles: The `editor_all` Role has the `GroupEdit` Policy, with permission to exercise it on any and all Fleets (as indicated by the `*` wildcard).
...
Name | Description | Permission Equivalent
--- | --- | ---
`GroupRead` | The most basic Fleet-level permission. Enables users to view a Fleet and its configuration, but not modify or delete the config. | Fleet-level [**Read Only**](permissions#group).
`GroupEdit` | Building on `GroupRead`, grants the ability to also change and commit a Fleet's configuration. | Fleet-level [**Editor**](permissions#group).
`GroupFull` | Building on `GroupEdit`, grants the ability to also deploy a Fleet. | Fleet-level [**Admin**](permissions#group).
`GroupCollect` | Grants the ability to create, configure, and run Collectors on a Worker Group. | N/A
`GroupUser` | Access Fleet. | Fleet-level [**User**](permissions#group).
...
Once you've defined a Role, you can associate it with Cribl Edge users. For details, see [Local Users](local-users). 
