...
AppScope is included in your Cribl Edge installation. The `in_appscope` [AppScope Source](sources-appscope) is enabled right out-of-the-box; it is configured to send data to Cribl Edge or Cribl Stream on the same host.  
...
>
> When the `sample_config` file has been assigned to an [AppScope Source's Rules](sources-appscope#rules), you can't restore its default settings. (An error message will state that the AppScope Config file is currently in use.) To resolve this:
> 
