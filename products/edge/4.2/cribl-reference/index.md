...
Native Cribl Edge function methods can be found under `C.*`, and can be invoked from any Function that allows for expression evaluations. For example, to create a field that is the SHA1 of a another field's value, you can use the Eval Function with this **Evaluate Fields** pair:  
...
Generates an [HMAC](https://en.wikipedia.org/wiki/HMAC) that can be added to events, or can be used to validate events that contain an HMAC. (Available in Cribl Edge v.3.1.2+.)  
...
Example that returns an array – the specified `fieldToReturn1` and `fieldToReturn2` columns from the matched lookup row:  
`C.Lookup('file','primaryKeyCol').match('someValue',['fieldToReturn1', 'fieldToReturn2'])`

Example that returns an array – all columns from the matched lookup row:  
...
Determine if the value supplied is an IPv4 address. (Available in Cribl Edge v.3.1.2+.)  
@param – `value` – the IP address to test.

`C.Net.isIpV6()`  
(method) `Net.isIpV6(value: string): boolean`  
Determine if the value supplied is an IPv6 address. (Available in Cribl Edge v.3.1.2+.)  
@param – `value` – the IP address to test.


`C.Net.isIpV4AllInterfaces()`  
(method) `Net.isIpV4AllInterfaces(value: string): boolean`  
Determine if the value supplied is the IPv4 all-interfaces address, typically used to bind to all IPv4 interfaces – i.e., '0.0.0.0'. (Available in Cribl Edge v.3.1.2+.)
@param – `value` – the IP address to test.

`C.Net.isIpV6AllInterfaces()`  
(method) `Net.isIpV6AllInterfaces(value: string): boolean`  
Determine if the value supplied is an IPv6 all-interfaces address, typically used to bind to all IPv6 interfaces – one of: '::', '0:0:0:0:0:0:0:0', or '0000:0000:0000:0000:0000:0000:0000:0000'. (Available in Cribl Edge v.3.1.2+.)  
@param – `value` – the IP address to test.
