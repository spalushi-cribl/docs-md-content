...
[Metrics](sources-metrics), [Raw UDP](sources-raw-udp), [SNMP Trap](sources-snmp-traps), and [Syslog](sources-syslog).
...
### 2023-08-28 - v.4.2.2 through Current - PQ doesn't drain for the Event Hubs Destination when Acknowledgements is set to All or Leader [CRIBL-19756]
...
**Fix**: Planned for Cribl Edge 4.3.1.
...
### 2023-08-10 – All Versions through v4.2.2 – Kafka-based Destinations fail to report backpressure properly [CRIBL-16944]
...
### 2023-07-27 - All Versions through Current - Recent Actions endpoint returns results for certain unauthorized users [CRIBL-18818]
...
### 2023-07-18 – v.4.2.0–4.2.1 – Migrated Local Users appear in the Members UI with "No Access" [CRIBL-18973]
...
### 2023-07-18 – v.4.2.0–4.2.1 – Product-level Editor Permissions do not allow a commit/deploy action on the Worker Groups/Fleets page [CRIBL-18970]
...
### 2023-07-16 – v.4.2.0–4.2.1 – After upgrading, Monitoring dashboards are not visible for users with `GroupRead` and `GroupCollect` Policies [CRIBL-18925]
...
### 2023-07-03 – v.4.2.0–4.2.1 – Users with Group-level Read Only Permissions can interact with logging level menus  [CRIBL-18556]
...
### 2023-03-21 – v.4.1.0-4.2.1 – Unable to bind `CRIBL_DIST_LEADER_URL` to an IPv6 address [CRIBL-16284]
...
### 2023-01-25 – v.3.5.x through Current – Cascading problems when Windows Event Logs Source collects from `ForwardedEvents` channel [CRIBL‑14869]
...
**Fix**: This issue no longer applies to Cloud.Cloud as of 4.0.1. On-prem customers should use the workaround, which will be added to the deployment documentation as a standard practice. 
...
### 2022-08-27 – All versions through 4.1.3 – Azure Event Hubs Destination with PQ drops events when inbound data is interrupted [CRIBL-17661, replaces CRIBL‑12649]
...
**Workaround**: On the Source whose events are being interrupted, configure an [Always On](persistent-queues#source) persistent queue. This buffering will cause the Destination to drop fewer events. Note that persistent queuing will not engage until Cribl Edge has exhausted all attempts to send the data to the Kafka receiver. 

**Fix**: In Cribl Edge 4.2.

{{< id `CRIBL-12066` >}}

### 2022-08-25 – Versions 3.4.1 through Current – Splunk Load Balanced Destination degradation with acks enabled [CRIBL‑12066]
...
### 2022-08-16 – All versions through Current – Splunk HEC shows higher outbound data volume than other Splunk Destinations
...
### 2022-04-18 – All versions through Current – Upgrade Version List is Limited to Latest Version [CRIBL‑9568]
...
### 2022-02-18 – v.3.3.x – Can't upgrade Workers to 3.3.x via UI [CRIBL‑8346]
...
### 2021-10-02 – v.3.2.x through Current – Kafka with Kerberos causes Workers' continuous restart [CRIBL‑6674]
