...
> Currently, you can't upgrade **Windows** Edge Nodes via Cribl Edge's UI.
> You must re‑run a Windows installer on these Nodes to upgrade them. This Windows
> limitation also applies to the Rollback section below.
>  
> You can use the **Add/Update Edge Nodes** option in the UI, to generate an
> upgrade script for Windows. For details, see [Add/Update Edge
> Nodes](managing-edge-nodes##add-node). 
>
{.box .info}
...
according to your **Quantity %** value. When enabled, this slider also enables
