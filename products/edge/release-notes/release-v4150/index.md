...
Cribl Edge 4.15.0 includes significant performance improvements, new capabilities, and important bug fixes.
...
### Windows SID Lookup Function

The new Windows SID Lookup Function lets you translate Windows SID (security identifiers) into canonical account names on Windows-based Edge Nodes.

### Fabric Real-Time Intelligence Destination

A new [**Fabric Real-Time Intelligence Destination**](destinations-fabric-real-time-intelligence) is now available in Cribl Edge. This Destination allows you to send telemetry and observability data from Cribl Edge to Microsoft Fabric Eventstreams via the new Kakfa-based Cribl Data source in the Microsoft Fabric Portal. Plus, there are no load balancing requirements—Fabric handles load balancing when it receives the data. You’ll find the Destination in the new **Fabric** folder in the Destinations catalog.
...
- You can now filter Edge Nodes by the Outpost they are connected to. If no Outposts are connected to the Leader, the filter will be hidden.
- The Edge Node list now offer an optional column to display which Outpost the Node is connected through.
- Outpost Node information, diagnostic bundles, and logs are now presented as a more comfortable full page instead of a drawer.
- Configuring Cribl Outpost as a service with the `cribl boot-start enable` command now gives the service the name `cribl-outpost`.
...
The Hello Packs sample Pack is no longer included in Cribl Edge starting with this release. Existing deployments will retain the Hello Packs content.
...
- Windows Event Logs Source now exposes the `eventID` property of an event.
...
|ID|Description|
|--|-----------|
| <div style="width: 100px">CRIBL-36384</div> | Fixed an issue where adding a macOS `.pkg` hash location to **Package source** would throw an error. |
| CRIBL-36517 | Fixed an issue where inputting a backslash or quote character into the Edge Node filter textbox could crash the application. |
| CRIBL-34952, CRIBL-35075 | Resolved a race condition that could cause persistent queues (PQ) to unexpectedly re-initialize. This fix stabilizes the PQ processes under high load or backpressure, eliminating the source of `uncaughtException` errors. |
...
|ID|Description|
|--|-----------|
| <div style="width: 100px">CRIBL-36319</div> | Worker initialization no longer hangs if a Redis connection is misconfigured and Redis Functions have client-side caching enabled. Cribl Edge now handles failed Redis connections gracefully during startup, allowing Workers to initialize and data flow to continue even if Redis is unavailable. |
