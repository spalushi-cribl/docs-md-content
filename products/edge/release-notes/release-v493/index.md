...
|ID|Description|
|--|-----------|
|<div style="width: 100px">CRIBL-28017</div>| The MSI installer, UI, and `cribl mode-*` CLI commands now normalize tags to ensure that duplicate and empty tags are removed.  |
| CRIBL-29035| Resolved an error that caused the Git commit list to incorrectly display multiple commits when users made only a single change. |
...
|CRIBL-29208 | A previous issue where the Windows Event Logs Source incorrectly interpreted the seek position argument has been resolved. This issue ledto duplicate events and longer-than-expected collection times. This fix ensures accurate event collection and improves overall performance.  |
