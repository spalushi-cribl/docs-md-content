...
**Map View**: Here, a query builder allows you to display metrics from the Edge Nodes in the Fleet.
...
To [teleport](managing-edge-nodes#teleport-into-an-edge-node) from the Leader into the Edge Node, you must
turn on **Enable teleporting to Nodes** in [Fleet Settings](fleet-settings). 
...
|Target Version|If you have a target version selected for the Fleet, it will appear on this button. If not, the button will show **None** (the default). To learn more about this feature, see [Upgrade Edge Nodes in a Fleet](upgrade-nodes-fleet).|
