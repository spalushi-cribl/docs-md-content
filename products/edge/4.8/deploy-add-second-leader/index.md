...
To handle unexpected outages in customer-managed (on-prem) distributed deployments, Cribl Edge supports configuring standby Leaders for failover. In this High Availability (HA) scenario, if the primary Leader goes down, Collectors and Collector-based Sources can continue ingesting data without interruption. 
...
You can configure additional Leader Nodes via the following environment variables: 
 ...
 - `CRIBL_DIST_MASTER_FAILOVER_MISSED_HB_LIMIT`: Determines how many Lease refresh periods elapse before standby Nodes attempt to promote themselves to primary. Cribl recommends setting this to `3`.
...
For further variables, see [Environment Variables](environment-variables). 
