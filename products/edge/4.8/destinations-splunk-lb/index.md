...
**Indexer discovery**: When enabled, Cribl Edge will automatically discover the indexers in an indexer clustering environment. This hides both **Exclude current host IPs** and the [**Destinations**](#destinations) section, and displays the following fields:
...
**Validate cluster manager certificates**: By default, Cribl Edge is configured to reject any cluster manager certificate during indexer discovery that is not authorized by your system's Certificate Authority (CA). This behavior is controlled by a toggle set to `Yes` by default. Cribl strongly recommends keeping this setting enabled unless you must allow untrusted (for example, self-signed) certificates.
...
> You can simplify Steps [3](#private-key) and [4](#public-key) below by dragging and dropping (or uploading) the `.pem` files into Cribl Edge's **New Certificates** modal. See [SSL Certificate Configuration](securing-ssl).
>
{.box .success}
