...
page or using the **Target Version** button when you select an individual Fleet. 

Upgrade options are on a per-Fleet basis:
...
![New Fleet upgrade interface](target-software-version-column.png)

> You can also select the **Target Version** button from the Node **List View**.
{.box .success}

3. The **Target Version** window will open. Click the **Upgrade target version**
   drop-down and select a version.
...
The **Upgrade status** icon opens the **Fleet Upgrade Information** modal, where you 
can view Edge Nodes which are eligible for upgrade as well as legacy upgrade jobs.
