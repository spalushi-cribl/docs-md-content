...
This section lists PQ configuration options on Cribl Edge Destinations.
...
The default `Yes` position enables FIFO (first in, first out) event forwarding. When receivers recover, Cribl Edge will send earlier queued events before forwarding newly arrived events. Disabling Strict ordering prioritizes processing new events over those currently residing in the PQ. When you toggle this setting to off, it exposes the **Drain rate limit (EPS)** control. 

#### Using Drain Limits 

Disabling **Strict ordering** setting exposes the **Drain rate limit (EPS)** field. This setting allows you to throttle the rate at which events are drained from the PQ to the destination. The default `0` value disables throttling. This setting ensures that Cribl Edge can keep up with the incoming stream of live data and doesn't get bogged down processing events from the PQ. Additionally, throttling the queue's drain rate can boost the throughput of new/active connections, by reserving more resources for them.
...
1. From Cribl Edge's top nav, select **Monitoring** to access multiple dashboards. 
1. The initial view shows aggregate data for all Fleets and all @{nodes}. Use the drop-downs at the upper right to isolate a single Edge Node.
