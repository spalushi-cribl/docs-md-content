...
When you first log into your Distributed Deployment, product tiles will prompt you to choose a product. The Cribl Edge tile displays basic configuration details, including number of Fleets, Subfleets, Edge Nodes, and events and bytes over time.
...
You must enable health status independently for each Fleet in Fleet Settings. 
...
> If **Minimal** is selected, health status will not be enabled for the Fleet
> and health icons will appear white for all Sources and Destinations.
{.box .info}
...
To review the health status of an Edge Node, teleport into the Node:

1. From the Fleet landing page, click the **List View** tab.
1. Select an Edge Node **GUID** link to teleport from the Leader into the Edge Node. 
1. From the top nav of the Edge Node, click the **Health** tab for monitoring details.

![Health Overview](ed_edge_teleport_health.ea83e81634.png)

> To view the Node health status without teleporting, click anywhere on the row to
...
> Cribl Edge displays metrics in units of KB, MB, GB, and TB. At each level, these are multiples of `1024`.
>
{.box .info}
...
* By default, all integration logging levels are set to `info`. This means logs of level `info` and above (including `error` and `warn`) will be displayed. Manual adjustments are needed to see `debug` and `silly` logs.
* Cribl.Cloud users cannot adjust logging levels for Worker Groups or specific integrations.
