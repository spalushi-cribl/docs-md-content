...
### 2024-09-27 – v.4.0 through Current – `RangeError: offset is out of bounds` and nested field serialization issues in Splunk Load Balanced Destination with S2Sv4 [CRIBL-28080]
...
### 2024-09-16 – v.4.8.2 – Deploying configurations to Edge Subfleets sometimes fail with TLS enabled [CRIBL-27793]
...
### 2024-09-10 – v4.8 through Current – HMAC authentication fails due to empty signature strings [CRIBL-27682]
...
### 2024-09-09 – v4.8.2 – Removing Member from Team removes access after a lag [CRIBL-27680]

**Problem**: As the Owner or Admin of a Cribl.Cloud Organization, if you remove a Member from a Team, that user will retain any Team-associated Permissions on products and product resources for up to 30 minutes. (This lag does not affect Team changes on customer-managed/​on-prem Organizations.)

**Workaround**: The intended access changes will take effect within 30 minutes.

**Fix**: Implemented on Cribl.Cloud after the 4.8.2 release

{{< id `CRIBL-27506` >}}

### 2024-09-04 – v4.8.2 – A Google Cloud Pub/Sub Source authentication tooltip gives incorrect Google authentication information [CRIBL-27506]
...
### 2024-09-05 – v.4.8.2 through Current – Cribl Copilot for KQL and Pipelines is disabled [AI-797]
...
### 2024-09-05 – v.4.8.2 through Current – Cribl Copilot for Pipelines does not work if accessed from Stream Projects [AI-802]
...
### 2024-08-14 – v.4.8.0 through Current – In Cribl Edge, TLS certificate paths in Sources and Destinations are incorrect [CRIBL-26971]
...
### 2024-08-14 – v.4.7.2 through Current – Wiz Source does not correctly paginate [CRIBL-26965]
...
### 2024-08-14 – v.4.8.0 – Some elements are missing from JSON-formatted events collected by the Windows Event Logs Source [CRIBL-26960]
...
You can either:
- Set **Use Windows Tools** to `Yes` in the Windows Event Logs Source [**Advanced Settings**](/edge/sources-windows-event-logs#advanced-settings).
...
### 2024-08-14 – v.4.8.0 – Splunk Destinations incorrectly detect that a downstream Splunk instance is shutting down [CRIBL-26969]

**Problem**: Splunk Single Instance and Splunk Load Balanced Destinations might incorrectly detect that a downstream instance of Splunk is shutting down at frequent intervals. This only affects Destinations with **Max S2S version** set to `v3` and the **Minimize in-flight data loss** toggle enabled. 

**Workaround**: Change the **Max S2S version** setting for the Destination to `v3`. Or, disable the **Minimize in-flight data loss** toggle.

**Fix**: In Cribl Edge 4.8.1.

{{< id `CRIBL-26958` >}}

### 2024-08-14 – v.4.8.0 – Missing in-product documentation for ServiceNow Cloud Observability Destination [CRIBL-26958]
...
### 2024-08-13 – v.4.8.0 - Current – Windows Event Log events are mishandled when they're missing provider metadata [CRIBL-26944]

**Problem**: Events without a valid provider are missing the `_raw` property. This
happens in the Windows Event Logs Source when **Use Windows Tools** is set to `No` in [**Advanced Settings**](/edge/sources-windows-event-logs#advanced-settings),
and [**Render event message strings**](/edge/sources-windows-event-logs/#optional-settings) 
is enabled.

**Workaround**: Set **Use Windows Tools** to `Yes` or disable **Render event
message strings**.

{{< id `CRIBL-26719` >}}

### 2024-08-06 – v.4.7.3 – C.Time Functions can crash Edge Nodes with old timestamps [CRIBL-26719]
...
**Fix**: In Cribl Stream 4.8.

 {{< id `CRIBL-25524` >}}

### 2024-07-31 – v.4.7.3 – Chain Functions in Cribl Stream Projects can lead to infinite loops [CRIBL-25524]
...
### 2024-07-26 – v.4.7.2-4.7.3 – REST Collector state not updated on job cancellation [CRIBL-26485]
...
### 2024-07-26 – v.4.7.2-4.7.3 – When Source PQ is enabled, disk space fills to near capacity and Cribl Edge issues an error [CRIBL-26478]
...
**Fix**: In Cribl Edge 4.8.

 {{< id `CRIBL-26377` >}}

### 2024-07-22 – v.4.7.2-4.7.3 –  `_raw` field expansion is incomplete [CRIBL-26377]
...
### 2024-07-19 – v.4.6.1-4.8.1 – Splunk TCP Source and Destination: Incorrect **Max S2S Version** displayed in UI [CRIBL-26333]
...
**Fix**: In Cribl Edge 4.8.2.

{{< id `CRIBL-24336` >}}
{{< id `CRIBL-26359` >}}

### 2024-07-18 – v.4.7.2-4.7.3 – Collection jobs stuck in "pending" status [CRIBL-24336, CRIBL-26359]
...
### 2024-06-27 – v.4.6.1 through Current – When you teleport into a Windows Edge Node and navigate to **Health**, the **CPU Load Average** panel displays no results. [CRIBL-25879]
...
### 2024-06-26 – v4.5.0-4.7.3 – Unable to Send Data via S2S to Splunk Cloud and Splunk Enterprise [CRIBL-25842]
...
**Fix**: In Cribl Edge 4.7.2. (Closed as a duplicate of CRIBL-25408 which is fixed in 4.7.2.)
...
### 2024-06-05 – v.4.7.0-4.7.3 – Upgrading to 4.7.0 with Google Cloud Security Operations Destination (formerly Google Chronicle) [CRIBL-25371]
...
### 2024-04-18 – v.4.6.0 – Cribl Edge displays a permissions-related error in some areas of the user interface [CRIBL-24310]
...
### 2024-04-17 – v.2.4.4 through Current – Incorrect Source URL in REST Collector events [CRIBL-24280]
...
**Problem**: The **Limits** [settings](settings-group-fleet#limits) at Fleet level that customer-managed (on-prem) deployments use are not available in Cribl.Cloud. Because the Redis settings (including **Reuse Redis connections**) are within that **Limits** UI, Edge Nodes in Cribl.Cloud do not have access to those settings.
...
### 2024-03-22 – v.4.5.0-4.6.0 – Leader incorrectly indicates version mismatch for Edge Nodes [CRIBL-23657]
...
**Workaround**: Manually [restart](cli-reference#restart) Cribl Edge after upgrading with `CRIBL_VOLUME_DIR` set.
...
2. If you wish to run your LDAP server **without** secure LDAP authentication, downgrade or upgrade to run a Cribl Edge version **other than** v4.5.0 or v.4.5.1.

**Fix**: In Cribl Edge 4.6.0.
...
### 2023-11-30 – v4.4.0-4.4.1 – Certain Sources and Destinations are unavailable in Edge and Worker instances [CRIBL-21443]
...
**Problem**: For deployments configured for [Leader high availability/failover](/stream/deploy-add-second-leader/), creating diagnostic bundles via the UI works normally but creating them via the CLI does not. In Cribl Edge v.4.4.0 and newer, the operation fails with a `not a git repository` error. In v.4.4.0 and older, the diag bundle created contains obsolete data. In both cases the underlying cause is that the process that creates the diag bundle cannot access the `CRIBL_CONF_DIR` environment variable. 
...
### 2023-11-06 – v.1.2–4.7.3 – Kafka and Confluent mTLS requirement limits Schema Registry authentication [CRIBL-21047]
...
After an upgrade to v.4.3.0, Edge Nodes for a Cribl Edge Amazon Kinesis Data Streams Source or Windows Event Forwarder (WEF) Source can lose state upon init. This can be especially problematic for customers with large streams and high data ingestion.
...
1. Upgrade to Cribl Edge v.4.3.1 or newer.
...
**Fix**: In Cribl Edge 4.7 (CRIBL-14556) for automatic synchronization between Leaders' hosts.
...
**Workaround**: On the Source whose events are being interrupted, configure an [Always On](persistent-queues-configuring#source) persistent queue. This buffering will cause the Destination to drop fewer events. Note that persistent queuing will not engage until Cribl Edge has exhausted all attempts to send the data to the Kafka receiver. 
