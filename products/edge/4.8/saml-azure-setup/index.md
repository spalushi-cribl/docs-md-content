...
[Snippet not found: content/shared/4.8/snippets/_sso-fallback-access.md]
...
From Microsoft Entra ID's left nav, select **Single sign‑on**, then **SAML**, to open the **Basic SAML Configuration** page.
Then, fill in the form with the information from Cribl.Cloud shown in the screenshot below:

1. In **Identifier (Entity ID)**, select **Add identifier** and enter the **Audience URI** value from the Cribl.Cloud SAML setup page.
1. In **Reply URL (Assertion Consumer Service URL)**, select **Add reply URL** and enter the two **Single Sign‑on URL** values from Cribl.Cloud's SAML setup page.
1. Of these two URLs, identify the one with the `connection` query parameter, and select the check box to make it the **Default**.

![SSO information for configuring Entra ID](cloud-sso-saml-azure-ad-config-4.7.png)
{border="true"}
...
1. When you've verified the connection, click **Save** to complete your submission.
...
1. On the Enterprise Application's **Basic SAML Configurations** page, click **Edit**.
...
[Snippet not found: content/shared/4.8/snippets/_sso-final-step-cloud.md]
