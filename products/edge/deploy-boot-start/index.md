...
Ensure that Cribl Edge starts at boot time.
...
> You must run this command as root, or with `sudo`. For options and arguments, see the [CLI Reference](/edge/cli-boot-start).
> 
...
To **enable** Cribl Edge to start at boot time with **systemd**, you must run the `boot‑start` [command](/edge/cli-boot-start). Make sure you first create any user you want to specify to run Edge. E.g., to run Edge on boot as existing user `cribl`, you'd use:
...
```shell
sudo systemctl [start|stop|restart|status] cribl-edge
```
...
```shell
sudo service cribl-edge [start|stop|restart|status]
```
