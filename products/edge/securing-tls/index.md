...
> In Cribl Edge 4.1 and newer, for enhanced security, Cribl encrypts TLS certificate private keys in configuration files when you add or modify them. Before upgrading to v.4.1.0 (or newer), back up your existing unencrypted key files – see [Safeguarding Unencrypted Private Keys for Rollback](/stream/upgrading#safeguarding/). If you need to roll back to a version older than 4.1, see [Restore Unencrypted Private Keys](#restoring) below.
>
{.box .warning}
...
- For Single-instance deployments, navigate to **Settings** > **Globals** > **Security** > **Certificates** ([Edge](/edge/deploy-linux/), [Stream](/stream/deploy-single-instance/)).
...
If you need to roll back from Cribl Edge 4.1 or later to an earlier version, follow this procedure to restore the unencrypted private key files that you [backed up](stream/upgrading#safeguarding) earlier. 
