...
You can use the following environment variables to configure your Cribl Edge installation.
...
| `CRIBL_BOOTSTRAP_HOST`   | Host name for connecting to the Leader Node when setting up a new Edge Node. |
...
| `CRIBL_K8S_CPU_LIMIT` | (Cribl Stream on Kubernetes only) Automatically sets CPU shares for Cribl Stream based on the `resources.limits.cpu` value, configured by default in the Helm chart. |
| `CRIBL_MAX_WORKERS` | (Cribl Stream on Kubernetes only) Setting a maximum amount of Worker Processes on a Kubernetes node helps to prevent overloading the node with more CPUs than it can handle. |
| `CRIBL_NOEXEC` | When set to `1`, prevents the [Exec Source](sources-exec) from processing any data, effectively putting it into a “disabled” state. To re-enable, remove or unset the `CRIBL_NOEXEC` variable.|

## Distributed Deployment Variables {#distributed}

You can use the following environment variables to configure your distributed Cribl Edge instance.

| Name | Purpose |
| --- | --- |
| `CRIBL_DEPLOYMENT`| Use this variable to set a human-readable name for your on-prem Leader. Example: `linux_production`. The value you set using this variable will appear on the [Connected Environments](cloud-connected-env) page.|
| `CRIBL_CLOUD_WORKSPACE_URL`| URL of the Leader Node in a Cribl.Cloud deployment. Example: `CRIBL_DIST_LEADER_URL=tls://<authToken>@leader:4200`. See [Usage Notes](#cribl_dist_leader_url).| 
| `CRIBL_DIST_LEADER_URL` |URL of the Leader Node. Example: `CRIBL_DIST_LEADER_URL=tls://<authToken>@leader:4200`. See [Usage Notes](#cribl_dist_leader_url).|
...
| `CRIBL_DIST_LEADER_BUNDLE_URL`| Specifies the location of configuration bundles. When set, Cribl Edge {@node}s will attempt to retrieve their configuration bundles from the URL specified by this variable, rather than directly from the Leader Node.|
...
>
> The auth token can include only alphanumeric characters or the underscore (`_`).
> Do not include `<`, `>`, `"`, `` ` ``,  `\r`, `\n`, `\t`, `{`, `}`, `|`, `\`, `^`, or `'`.
{.box .info}

When configuring the node as a Leader, use `0.0.0.0` (or another suitable binding address) for `host`.
Then, set `CRIBL_DIST_MODE` to `leader`.
...
* `tag` – A list of tags that you can use to map ([Stream](/stream/mapping-workers-to-worker-groups), [Edge](/edge/fleets#mapping)) the Worker to a Fleet.
* `disableSNIRouting` – Whether Server Name Indicator (SNI) routing is enabled on the Edge Node. Boolean; defaults to `false`.
  ...
  > determines how Cribl Edge routes connections between Leader Nodes and Edge Nodes.
  > See the [Known Issue](/results/?query=&searchOption=known-issues&tickets=cribl-24451) that resulted in this setting for more details.
  {.box .warning}
...
`CRIBL_DIST_WORKER_PROXY` lets you configure a SOCKS proxy for communication between the Leader Node and Edge Nodes.

The default protocol is SOCKS5, which uses the format:
`socks5://<username>:<password>@<host>:<port>`. Only `<host>:<port>` are required.
...
The `<proxyhost>` can be a hostname, IPv4, or IPv6.

> To configure a SOCKS proxy via the UI, see [Configure SOCKS Proxy](proxy-socks-config).
> To configure an HTTP(S) proxy to use for your Sources and Destinations, refer to [System Proxy Configuration](proxy-config).
>
{.box .info}
...
For example, if you set `CRIBL_BOOTSTRAP=myhost`, then `myhost` will appear in the script modal's **Leader or Outpost hostname/IP** field, instead of the URL used in the browser.
...
## Adding Fallback Leaders {#fallback-leaders}
...
Cribl Edge provides the following environment variables to facilitate using [GitOps](/stream/gitops) in Cribl Stream.
...
| `CRIBL_GIT_REMOTE_URL`               | Configure simple Git remote URL. Takes precedence over `CRIBL_GIT_REMOTE` for URL setting. |
...
Cribl Edge uses the following variables for multiple purposes.
