...
Federal Information Processing Standards [FIPS](https://www.nist.gov/standardsgov/compliance-faqs-federal-information-processing-standards-fips) represent a set of US government standards and guidelines for information security. You can deploy Cribl Edge in **FIPS mode**. This mainly restricts the cryptographic algorithms used within Cribl Edge and also enforces FIPS compliant password requirements. 

> Do not start Cribl Edge without FIPS mode enabled, or else you will be unable to run it in FIPS mode later. You must enable FIPS mode as described in this section, after installing **but before starting** Cribl Edge. 
{.box .warning}
...
To run Cribl Edge in FIPS mode, your environment must satisfy the following requirements.

#### Operating Systems 

Must support FIPS [140-2](https://csrc.nist.gov/pubs/fips/140-2/upd2/final) or FIPS [140-3](https://csrc.nist.gov/pubs/fips/140-3/final). Several Linux distributions meet this standard. See NIST's list of tested configurations [here](https://csrc.nist.gov/projects/cryptographic-module-validation-program/certificate/4282).

> While RHEL 9 can be configured to run in FIPS mode at the operating system level, this **does not automatically enable FIPS mode within Cribl Edge.** You must still follow the procedures outlined in this document to explicitly enable Cribl Edge's FIPS mode. Cribl Edge relies on its own configuration to ensure FIPS compliance, independent of the OS-level FIPS setting.
{.box .warning}
...
- **For Cribl Edge versions 4.5.0 through 4.8.1**: The FIPS provider version must be 3.0.8 or 3.0.9, and the system-wide OpenSSL version must also be [3.0.8](https://www.openssl.org/blog/blog/2023/05/29/fips-3-0-8/) or [version 3.0.9](https://www.openssl.org/blog/blog/2024/01/23/fips-309/index.html), with its associated certificate.
- **For Cribl Edge starting with version 4.8.2**: The FIPS provider version requirement must be >= 3.0.5, and the system-wide OpenSSL version is no longer a factor. Cribl Edge now verifies the OpenSSL version bundled within the binary.

Enterprise Linux 9 is the only version of Enterprise Linux that includes OpenSSL version 3.0.9 out of the box. While it's possible to manually install compatible versions of OpenSSL on older Enterprise Linux versions, this process is complex and generally not recommended. Therefore, the minimum recommended Enterprise Linux version for running Cribl Edge in FIPS mode is 9.

Even when running Cribl Edge on a FIPS-enabled RHEL 9 system, you must still explicitly enable Cribl Edge's FIPS mode as described in the [Running Cribl Edge in FIPS Mode](#running-fips) section below. The OS being in FIPS mode is not sufficient for Cribl Edge to operate in a FIPS-140 compliant manner.

### FIPS Mode Restrictions on Cryptographic Algorithms

When running in FIPS mode, Cribl Edge uses only those cryptographic algorithms that satisfy the FIPS standards. This means that Cribl Edge operates exclusively with FIPS-compliant cryptographic algorithms, disallowing the use of non-compliant algorithms like MD5 (message-digest) or CRC-32 (Cyclic Redundancy Check 32) algorithms.
...
- The [Amazon Kinesis Data Streams Source](/stream/sources-kinesis-streams) will not perform checksums and the **Verify KPL checksums** setting will not be available.
...
- The following Destinations will not perform checksums:
  ...
  - [Amazon Kinesis Data Streams](/stream/destinations-kinesis-streams)
...
Cribl Edge's FIPS mode requires [RBAC](roles#rbac-concepts) to be enabled for enhanced security. This ensures proper access management within FIPS-compliant environments. To enable RBAC, the Cribl Edge instance has to be running in Distributed mode.
FIPS mode is available on certain license tiers. For details, see [Pricing](https://cribl.io/pricing/).

If FIPS mode is already enabled and RBAC is subsequently disabled, Cribl Edge will automatically disable FIPS mode and log a message stating:
...
When Cribl Edge is in FIPS mode, all passwords must:
...
## Running Cribl Edge in FIPS Mode {#running-fips}

To begin using Cribl Edge with FIPS enabled, complete either the [non-systemd](#fips-non-systemd) or the [systemd](#fips-systemd) procedure below, **before you start Cribl Edge for the first time after installing**. 

### Enabling FIPS Mode on a Non-systemd Distribution {#fips-non-systemd}

If the Linux distribution on which you run Cribl Edge **does not** use systemd:
...
3. Ensure [RBAC](#rbac) is configured in Cribl Edge. 
...
If the Linux distribution on which you run Cribl Edge **does** use systemd:
...
2. Run the following command, replacing the placeholders with the relevant paths from your environment. (This assumes that you do not already have a `nodejs.cnf` file. If you **do** have one, complete the [workaround](#nodejs-cnf-workaround) below instead of this step.)
    ...
    This command creates `nodejs.cnf` – the configuration file that NodeJS uses to run in FIPS mode as Cribl Edge starts. 
...
### Verify FIPS Mode {#verify-fips}
   
- On the Leader or Edge Node, search `cribl.log` for this message:
...
When you log in as `admin` – you will be prompted to enter a FIPS compliant password.
