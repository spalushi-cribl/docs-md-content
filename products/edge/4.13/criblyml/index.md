...
api: # [object]
  ...
  listenOnPort: # [boolean] Enable/disable exposing the API service to the network on the configured API port. Defaults to true.
  disabled: # [boolean] Flag to enable/disable local UI access. Defaults to false.
  workerRemoteAccess: # [boolean] Enable teleporting to Edge Nodes'. Defaults to false.
  ...
  idleSessionTTL: # [number] How long (in seconds) Cribl Edge will observe no user interaction before invalidating user's session tokens. Default is 1 hr.; minimum is 60 sec.
  ...
  ssoRateLimit: # [string] Rate limit for SSO and SLO callback endpoints. Expressed as maximum number of requests per second, minute, hour, or day, e.g., 3/second, 5/minute. When limit is reached, the Cribl Edge sends 429 Too Many Requests HTTP responses.
...
auth: # [object] Authentication Settings
  ...
  groupMemberValField: # [string] Group membership attribute - Attribute name of LDAP user object, such as dn. Determines group member attribute's value, which defines group's allowed users.
  ...
  spEntityId: # [string] Audience (SP entity ID) - Unique URL that identifies this SP (Cribl Edge) instance as the recipient of SAML requests. Must be the base URL of your Cribl Edge server, such as https://yourDomain.com:9000.
  ssoCallbackUrl: # [string] Sign-on callback URL - URL where the SP (Cribl Edge) will consume assertions (that is, where it will receive authentication responses)
  sloCallbackUrl: # [string] Logout callback URL - URL where the SP (Cribl Edge) will receive logout responses
  metadataUrl: # [string] Metadata URL - URL that exposes a description of the SP (Cribl Edge), including endpoints, certificates, and metadata
  spSigningCert: # [object] 
    certificateName: # [string] Request certificate - Certificate the SP (Cribl Edge) should use to sign requests. Create a new certificate, or choose one that you imported as described [in Cribl Docs](https://docs.cribl.io/stream/securing-communications/).
  idpEntityId: # [string] Issuer (IDP entity ID) - Unique ID of the identity provider
  ssoUrl: # [string] Single sign-on (SSO) URL - Endpoint to which authentication requests will be sent (that is, the identity provider's single sign-on service)
  sloUrl: # [string] Single logout (SLO) URL - Endpoint to which logout requests will be sent (that is, the identity provider's single logout service). Setting this will enable SP-initiated single logout.
  requestBinding: # [string] Request binding - Type of binding for SAML requests sent to the identity provider's SSO/SLO services
  idpSigningCert: # [string] Response validation certificate - The certificate that the SP (Cribl Edge) should use to validate signed responses. Provided by the IDP; contains a public key.
  encryptCert: # [object] 
    certificateName: # [string] Response decryption certificate - Certificate the SP (Cribl Edge) should use to decrypt encrypted responses. Create a new certificate, or choose one that you imported as described [in Cribl Docs](https://docs.cribl.io/stream/securing-communications/).
  messageSigningOrder: # [string] Order of encrypting and signing - Specifies the order when the IDP both signs and encrypts responses
  usernameField: # [string] Username field - In the SAML response, the attribute that specifies the user identifier. Defaults to "NameID".
  groupnameField: # [string] Group name field - In the SAML response, the attribute that specifies the user groups. Defaults to "groups".
  fallback: # [boolean] Allow login as Local User - Allow users to log in without SAML (as an alternative, or as a fallback if SAML login fails)
  groups: # [object] 
    default: # [string] Default role - Default role assigned to groups not explicitly mapped to a role
    mapping: # [object] Mapping - Add a mapping for each external user group that you want to map to one or more Cribl Edge roles. Then enter the group and select the role(s).
...
tls: # [object] Default TLS Settings
  ...
  defaultCipherList: # [string] Default cipher list - Default suite of enabled and disabled TLS ciphers. Defaults to:
      ...
      !SRP:
      !CAMELLIA
  defaultEcdhCurve: # [string] ECDH curve - The curve name, or a colon-separated list of curve NIDs or names, to use for ECDH key agreement. For example: 'P‑521:P‑384:P‑256'. Defaults to 'auto'.
...
git: # [object] 
  ...
  commitDeploySingleAction: # [boolean] Collapse actions - When enabled, Commit & Deploy will be collapsed into a single action. If you've configured a remote, Commit & Git Push will also be collapsed. Your Default Commit Message below will be used for all commits.
  ...
  copilotAutoGitCommitMessages: # [boolean] Generate commit messages automatically with Cribl Copilot - Cribl Copilot-generated messages override any default commit message you may have set
fips: # [boolean] 
