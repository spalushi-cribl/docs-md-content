...
Configure the proxy server to handle HTTP/S requests on Windows.
...
There are a two ways to do it, using Windows system settings and Group Policy Objects.
...
you need to set the [`CRIBL_DIST_WORKER_PROXY`](environment-variables#cribl_dist_worker_proxy) environment variable:
...
You can use HTTP Basic authentication on HTTP or HTTPS proxies.
To do this, [during installation](deploy-windows#env-var) set the `HTTP_PROXY` and `HTTPS_PROXY` environment variables
and specify the user name and password in the proxy URL:

|Variable|Value|
|---|---|
|`HTTP_PROXY`|`http://<username>:<password>@<proxyhost>:<port>`|
|`HTTPS_PROXY`|`https://<username>:<password>@<proxyhost>:<port>`|
