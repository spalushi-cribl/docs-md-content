...
## Disable Exposing the API Service for Edge Nodes {#disable-api-service}
...
1. In the sidebar, select **Fleets**, and then select the Fleet you want to configure.
...
1. Click **Save**.
1. Commit and Deploy the Fleet's new configuration, including the new certificate.
1. Repeat the preceding steps on each Fleet.

![Fleet-level certificates are configured like the Leader's certificate](tls-group-new-cert.df6085a3c1.png)
{border="true"}
...
1. Tunnel through from the Leader to the Edge Node's UI.
...
![Configuring TLS on Edge Node's UI, from the Leader](worker-to-leader-tls-cert-enabling-4101.png)
{border="true"}
