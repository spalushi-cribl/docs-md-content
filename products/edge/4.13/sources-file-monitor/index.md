...
> Cribl Edge currently supports Auto mode only when running on Linux, not on Windows. Windows deployments require using the Manual option. For a Windows-specific configuration, see [Configure the File Monitor Source on Cribl Edge](/edge/usecase-windows-observability#fm-source).
>
{.box .info}
