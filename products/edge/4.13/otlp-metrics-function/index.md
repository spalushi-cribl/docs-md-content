...
Here, we have two incoming events sent by a [OTel Source](sources-otel) with **Advanced Settings > Extract Metrics** turned on.

The Function, with **Batch OTLP Metrics** turned on, batches the Cribl events such that the Cribl Edge OTel Destination can ingest the batch and convert to a batched OTLP metric event.
