...
IPFIX fields are derived from the [IANA IPFIX Information Elements](https://www.iana.org/assignments/ipfix/ipfix.xhtml#ipfix-information-elements) list. These fields are predefined and included in the `ipfix-information-elements.yml` configuration file.
...
## Leader and Edge Node Interactions 

The NetFlow & IPFIX Source uses the Leader Node to keep all Edge Nodes in sync with NetFlow v9 and IPFIX template information. If the Leader is unavailable, Edge Nodes can still process data using the templates they already have. However, new or updated templates for v9 flows will only be learned by the specific Edge Node receiving that traffic. Other Edge Nodes won't get these updates and will drop any flowsets that use those new templates.
