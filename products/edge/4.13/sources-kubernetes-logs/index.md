...
> 
> You can't configure custom Kubernetes Logs Sources.
> 
{.box .info}
...
To visualize and analyze these Kubernetes logs in more detail, consider using the [Kubernetes Explorer UI](/edge/explore-edge-linux#kubernetes) in Cribl Edge. You can preview logs by navigating to the **Logs** view, filtering logs, and defining event breakers.
...
1. Select the default `in_kube_logs` to open the configuration modal.
...
**Enable load balancing**: To handle high volumes of Kubernetes logs, 
enable [load balancing](load-balancing). 
This improves the events per second (EPS) ingest rate, and
is helpful when you need to ingest more Kubernetes logs EPS than
a single Worker Process can handle. 

When you enable this toggle, you should also increase the number of
[Worker Processes](fleet-settings#processes) in Fleet Settings. 
The number of Worker Processes for load balancing depends on
your Kubernetes log ingest volume. To ingest 40k EPS, try starting with two or four Worker
Processes, then increase if needed.
