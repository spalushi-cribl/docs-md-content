...
- **Add Pipeline with Cribl Copilot**: Use the Cribl Copilot Editor to create a Pipeline using AI. See [Create Pipelines Using Cribl Copilot Editor](copilot-editor) to learn more.
...
Toggle **Final** off (default) to process events by this Function, **and** then pass them on to the next Function below.

Toggle **Final** on if you want the Function to "consume" the results, meaning they will not pass down to any other Function below.
...
Functions in a Pipeline are equipped with their own filters. Even though filters are not required, we recommend using them as often as possible. See [Build Custom Logic to Route and Process Your Data: Filter Expressions](filter-and-transform-data#filters) for more information.
