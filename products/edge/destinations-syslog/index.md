...
**Enable Source IP spoofing**: Cribl Edge displays this option only when **Protocol** is set to `UDP`. This feature is available only for on-prem or hybrid Worker Groups (not available in Cribl.Cloud). Toggle on to use the event Source IP and port for outgoing UDP packets. 
...
Enabling IP spoofing requires creating raw network sockets, which demands elevated system privileges. To maintain the Cribl Edge security model, this operation is isolated into a dedicated helper program.
