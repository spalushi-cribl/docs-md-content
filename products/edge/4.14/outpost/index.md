# Connect Edge Nodes to Leader Through Cribl Outpost
...
by acting as a relay between individual Edge Nodes and the Leader Node.

Once you establish an Outpost and configure it to accept traffic from Edge Nodes,
...
between Edge Nodes and the Leader, for example due to restrictions in opening up firewall ports.
...
> the Outpost node will close the listener for communication incoming from Edge Nodes,
> but will be shown as healthy on the Leader.
{.box .info}

## How Outpost Works

An Outpost is a special type of Node that rests between Edge Nodes and the Leader.

![Overview of how Outpost connects Leader with Edge Nodes](outpost-diagram.png)
{border="false" scale="90%"}

For an individual Edge Node, an Outpost is indistinguishable from the Leader:
...
You can connect a maximum of 28,000 Edge Nodes to a single Outpost.
The specific number of possible connections up to this limit
depends on the Outpost Node specifications, in particular on CPU and RAM.

The recommended specifications for serving a given number of Edge Nodes are as follows:

| Number of Edge Nodes | vCPU | RAM | Connection listener processes |
| --- | --- | --- | --- |
| up to 10,000 | 2 | 4 GiB | 1 |
| 10,000 – 25,000 | 4 | 8 GiB | 2 | 

While the maximum limit is 28,000 Edge Nodes, if you need to connect more than 25,000 Nodes,
Cribl recommends using more than one Outpost to share the load.
