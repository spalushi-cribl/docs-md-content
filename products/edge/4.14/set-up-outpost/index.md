...
1. Set the installation mode to `mode-outpost` to connect the Outpost to the designated Leader
   and allow Edge Nodes to connect to it in turn:
   ...
   - `<token>` must match the Leader token, to ensure a connection with the Leader.
     ...
     In Cribl.Cloud, get the token for your Workspace from the Edge Node modal (**Edge Nodes > Add/Update Edge Node**) from the **Auth token** field.
...
and you can start configuring Edge Nodes to connect to it.

## Connect Nodes to an Outpost {#connect}

Edge Nodes see Outpost in the same way as a Leader Node.
