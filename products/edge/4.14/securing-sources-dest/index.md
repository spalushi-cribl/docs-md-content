...
You can secure connections between Cribl Edge and external systems by using TLS (Transport Layer Security) certificates or custom HTTP headers. These extra security measures are optional, but TLS certificates or custom HTTP headers help authenticate data senders and receivers, protect data in transit, and prevent unauthorized access.

## Certificate-Based Authentication {#worker-group-certs}

Cribl Edge stores and manages certificates at the Fleet level. When you add a certificate to a Source or Destination, Cribl Edge creates it within the same Fleet where you configured the integration.

> For information about how certificate references work in Packs, see [Use Named TLS Certificates](packs#named-certificates).
>
{.box .info}

### Add a TLS Certificate to a Fleet {#add-certificate-to-group}

To add a TLS certificate that will be available to all Sources and Destinations at the Fleet level:

1. On the top bar, select **Products**, and then select **Cribl Edge**. Under **Fleets**, select a Fleet.

1. Navigate to **Fleet Settings**. On the side menu, expand **Security** and select **Certificates**.
...
1. Select **Save**. The certificate now appears in the list of certificates for the Fleet.
...
1. Use the **Certificate name** drop-down to add a certificate:
   ...
   - **To add a new certificate:** Select the **Create** button next to the field. See [Add a TLS Certificate to a Fleet](#add-certificate-to-group) for field descriptions.
...
Content Security Policy (CSP) helps protect Cribl Edge from Cross-Site Scripting (XSS) and other web injection attacks by controlling which resources the browser is allowed to load. You can implement CSP using the [Custom HTTP Headers](#custom) setting described above.
