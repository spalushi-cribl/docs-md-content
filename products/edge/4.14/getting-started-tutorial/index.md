...
Learn basic Cribl Edge functionalities.
...
and a free Cribl license will be enough to go through all the steps.
...
1. In Cribl Edge, select **Manage** > **Collect**. 
   You can use the QuickConnect screen to configure how data flows between Sources and Destinations.

   > If you're on Cribl.Cloud or any other Distributed mode, first select the `default_fleet` in its tile to select it.
   {.box .cloud}

1. From the list of Sources, expand the File Monitor tile and select **Configure** for **in_file_varlog**.
...
1. When set to a Manual discovery mode, the **Search path** field in File Monitor specifies
   ...
   **in_file_varlog** contains the `/var/log/` path, which works for this tutorial.
   ...
   ![File Monitor configuration modal with default settings.](edge-tut-configure-file-monitor-2-4101.png)
   {caption="The default settings for the Source are good enough for this tutorial." border="true"}
...
> If you're on Cribl.Cloud or any other Distributed mode, select **Commit** before proceeding.
> Then, in the resulting dialog, type a commit message (for example, "Added Source") and select **Commit and Deploy** to confirm.
> You'll see a **Commit successful** message.
{.box .cloud}

Now select **Capture** in the **FileMonitor** Source to go to the **Live Data** tab.
...
{caption="Data from the File Monitor is already flowing." border="true"}

Hover over the **DevNull** Destination to see a shortcut menu.
You can select **Capture** to jump to a preview of data flowing into this Destination.

If you do not see any events, the reason may be that no file changes were registered in this period.
In that case you can select **Capture...** and increase the **Capture Time (sec)** value
...
1. From Cribl Edge's top menu, select **More** > **Destinations**. 
   ...
   > If you're on Cribl.Cloud or any other Distributed mode, first select the `default_fleet` in its tile.
   {.box .cloud}

1. From the **Manage Destinations** page's tiles or left menu, select **Cribl HTTP**.
   (You can use the search box to jump to the **Cribl HTTP** tile.)
1. Select **Add Destination** to enter a **New Destination** modal.
1. Provide your Destination's **Output ID** (for example, `cribl-stream`).
1. In **Cribl Worker endpoints** you will provide the endpoint of the Cribl Stream instance
   ...
   ![Cribl HTTP Destination modal with endpoint configured.](edge-tut-create-http-dest-4101.png)
   {border="true"}

1. Confirm with **Save**.

> If you're on Cribl.Cloud or any other Distributed mode, select **Commit** at Cribl Edge's upper right before proceeding.
> Then, in the resulting dialog, type a commit message (for example, "Added Destination") and select **Commit and Deploy** to confirm.
> You'll see a **Commit successful** message.
{.box .cloud}
...
1. Access your Cribl Stream's UI, and select the `default` (or any other) Worker Group.
2. In the top menu, select **Data** > **Sources**.
3. Search for **Cribl HTTP** and select it. You will see a built-in `in_cribl_http` Source that may be disabled by default.
...
4. In the Source's configuration modal, make sure that the **Address** Field points to `0.0.0.0`,
   ...
   ![Cribl HTTP Source modal in Cribl Stream.](edge-tut-http-source-4101.png)
   {caption="This Source will pick up events sent by Cribl Edge." border="true"}

5. Enable the Source and exit the modal.
6. Select **Commit** at Cribl Stream's upper right.
   In the resulting dialog, type a commit message and select **Commit and Deploy** to confirm.
   You'll see a **Commit successful** message.
...
{caption="The data now flows where it should: to the Cribl HTTP Destination." border="true"}
...
Wait a few moments for the data to start flowing, and then in Cribl Stream select the `in_cribl_http` Source's **Live** button.
...
- [Explore Cribl Edge](explore-edge-linux): Learn more about the various capabilities of Cribl Edge by taking a tour through its UI.
