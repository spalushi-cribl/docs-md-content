...
The Cribl Internal Source enables you to capture and send Cribl Edge's own internal [logs](internal-logs) and [metrics](internal-metrics) through Routes and Pipelines. 
...
3. Configure the **General Settings** for your selected Source.
   - For Cribl Logs, configure the following:
     ...
     - **Input ID**: This is prefilled with the default value `CriblLogs`, which cannot be changed.
   ...
   - For Cribl Metrics, configure the following:
     ...
     - **Input ID**: This is prefilled with the default value `CriblMetrics`, which cannot be changed.
     - **Description**: Optionally, enter a description.
