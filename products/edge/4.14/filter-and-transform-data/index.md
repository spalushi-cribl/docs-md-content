...
Throughout Cribl Edge, many settings let you use custom JavaScript code and [Cribl Expressions](cribl-reference) to control how data gets routed or processed. You will encounter JavaScript-enabled fields like these in several parts of Cribl Edge:
...
You can use JavaScript syntax in any field that allows JavaScript expressions. You can also use [Cribl Expressions](cribl-reference), which are pre-built helpers designed for common operations in Cribl Edge. Cribl Expressions begin with `C.`, such as `C.Lookup` or `C.Time`.
...
Wildcard lists appear in various Functions, such as [Eval](eval-function), [Mask](mask-function), [Publish Metrics](publish-metrics-function), [Parser](parser-function), and [Serialize](serialize-function).
...
Wildcard fields (`*`) support two use cases:

- Wildcards (`*`) to include multiple fields.
- Nested addressing, where you can reference a parent field and all its children using a wildcard. For example, if you convert `_raw` into an object, use `_raw*` to refer to `_raw` and all nested fields inside it.
...
> You cannot use wildcards to target Cribl Edge internal fields that start with `__`  (double underscore). You must specify these fields individually. For example, `__foobartab` cannot be removed by specifying `__foo*`.
>
{.box .warning}
