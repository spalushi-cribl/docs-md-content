...
![Options to add or update an Edge Node](edge-node-add-update-4.14.png)
...
- Linux: For details, see [Installing Cribl Edge on Linux](deploy-linux).
- MacOS: For details, see [Install Cribl Edge on MacOS](deploy-macos).
...
1. Fill and modify the deployment option fields:
    ...
    - As needed, correct the target **Fleet**, as well as the **Leader or Outpost hostname/IP**
     (URI).
...
1. To allow Edge to update or add Edge Nodes, configure the environment variable
  ...
  - You can add the restart command to the environment variable in the Linux
    ...
    variable each time you restart the server:
      ...
      1. In the CLI, enter `sudo systemctl edit cribl-edge` on the Edge Node.
...
updates by using the specified [**Target Software Version**](upgrade-nodes-fleet)
for your Fleet, enhancing the reliability and consistency of updates across all
supported environments: Docker, Kubernetes, Linux, macOS, and Windows. 
...
- For details on collocating multiple Cribl products, see [Cribl Edge and Cribl Stream on the Same Host](deploy-linux#collocated).
