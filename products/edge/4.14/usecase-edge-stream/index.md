...
Cribl Edge automatically discovers logs, metrics, application data, and so on – in real time – from your configured endpoints, and delivers them to Cribl Stream or any supported destination. Meanwhile, Cribl Stream can help collect, reduce, enrich, transform, and route data from Cribl Edge to any destination. And using a Cribl HTTP Source, you can collect and route data from Edge Nodes to Stream Worker Nodes without incurring additional cost.
...
- Configure [Cribl HTTP Source](/stream/sources-cribl-http) on Cribl Stream to receive data from the Edge Node.
- Configure the [Exec Source](/edge/sources-exec) on Cribl Edge to collect data on the Edge Node.
- Configure the [Cribl HTTP Destination](/edge/destinations-cribl-http) on Cribl Edge to send data to Cribl Stream.
- Configure a [Route](/edge/routes) to Send the Data.

And finally, we will confirm the data flow. 

## Configure the Cribl HTTP Source on Cribl Stream {#source}

In Cribl Stream, start by configuring and enabling a [Cribl HTTP Source](/stream/sources-cribl-http#configure-the-criblhttp-source). The key requirement here is to set the **Port** to listen on. For this example, we'll use the default `10200`. (Optionally, you can also configure TLS, enable persistent queue, and/or a pre-processing Pipeline.)
...
> On Cribl-managed Workers in Cribl.Cloud, make sure that TLS is either disabled on both the Cribl HTTP Source and the Cribl HTTP Destination it's receiving data from, or enabled on both. Otherwise, no data will flow. In the Source, TLS is enabled by default. 
>
{.box .cloud} 

## Configure the Exec Source on Cribl Edge

Next, we'll configure the [Exec Source](/edge/sources-exec) on your Edge Node. This Source will break the incoming streams of data into discrete events, and send them to Cribl Stream. 
...
On the Exec Source configuration modal's left tab, select [**Event Breaker**](/edge/sources-exec#event-breakers). In the **Event Breaker rulesets** drop-down, select `Cribl – Do Not Break Ruleset`.
...
To get the data flowing, we'll configure the Cribl HTTP [Destination](/stream/destinations-cribl-http) on your Edge Node. A few things to note when configuring this Destination:
...
- Optionally, [define](/stream/destinations-cribl-http#optionalsettings) your **Compression**, **Throttling**, and **Backpressure behavior** requirements.
...
Finally, configure a [Route](/edge/routes) to send your data to Cribl Stream. For this example, we can use the `passthru` [Pipeline](/edge/pipelines). 
...
You can also check the [Monitoring](/stream/monitoring#data-monitoring) page > **Data** submenu, to isolate the throughput on your Source.