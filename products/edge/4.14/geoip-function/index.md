...
1. Determine where to place the database file. We recommend the `$CRIBL_HOME/state/` subdirectory, which is already listed in the default `.gitignore` file that ships with Cribl Edge.
    ...
    You always have the option to upload the file to Cribl Edge's [Lookups Library](lookups-library). In a [Cribl.Cloud](/stream/deploy-cloud) deployment, this is currently the only option. For details, see [About Lookups](/stream/lookups-about).

1. Place the database file on your Edge Node(s). In a [distributed deployment](/stream/deploy-distributed), we [recommend](/stream/usecase-lookups-ip-addresses#recommended) having the file on the Leader and all Edge Nodes. Smaller deployments can [get away with](/stream/usecase-lookups-ip-addresses#copy-the-database-file-only-to-the-leader-alternative) hosting the file only on the Leader Node.
