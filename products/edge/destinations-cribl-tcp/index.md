...
initial ingest to the first Fleet (depending on your Leader setup – see
...
You can use the Cribl TCP Destination to transfer data between Edge Nodes and Workers. If the Cribl TCP Destination sends data to its [Cribl TCP Source](sources-cribl-tcp) counterpart, you're billed for ingress only once – when Cribl first receives the data.
