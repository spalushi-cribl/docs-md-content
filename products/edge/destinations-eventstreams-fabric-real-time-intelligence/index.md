...
>
> Before you create a Fabric Real-Time Intelligence Destination in Cribl Edge, [create an Eventstream and custom endpoint source](#create-eventstream-custom-source) in Microsoft Fabric. You will then retrieve the connection values (like the bootstrap server) to use when you configure a Fabric Real-Time Intelligence Destination in Cribl Edge.
>
{.box .info}

## Create an Eventstream and Custom Endpoint Source in Microsoft Fabric {#create-eventstream-custom-source}

To create an Eventstream that you can use to send data from Cribl Edge, create a new [workspace](https://learn.microsoft.com/en-us/fabric/fundamentals/create-workspaces) and [Eventstream](https://learn.microsoft.com/en-us/fabric/real-time-intelligence/event-streams/create-manage-an-eventstream) as follows:
...
**SASL JASS password**: The stored secret that references the **Connection string-primary key** in the **Cribl Source** custom endpoint source in Microsoft Fabric. The secret can reside in Cribl Edge's [internal secrets manager](securing-kms-config) or (if enabled) in an external KMS. A **Create** link is available if you need a new secret.
...
To add a **Cribl Source** custom endpoint source in Microsoft Fabric and retrieve the values necessary to [configure](#configure-fabric-dest) the Fabric Real-Time Intelligence Destination in Cribl Edge, you must have **Contributor** (or higher) Fabric workspace permissions.
