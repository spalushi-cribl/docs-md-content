...
| `CRIBL_DIST_LEADER_BUNDLE_URL`| Specifies the location of configuration bundles. When set, Cribl Stream {@node}s will attempt to retrieve their configuration bundles from the URL specified by this variable, rather than directly from the Leader Node.|
...
* `tag` – A list of tags that you can use to map ([Stream](/stream/mapping-workers-to-worker-groups), [Edge](/edge/fleets#mapping)) the Worker to a Worker Group.
