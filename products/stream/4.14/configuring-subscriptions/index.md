...
A Project can contain multiple Subscriptions, and each Subscription can be associated with multiple Projects. Each Project's copy of the Subscription will share the same configuration, but will have independent data flow.
...
- Subscriptions lack a Route's **Output** or **Destination Expression** field. (Instead, you associate Subscriptions with Projects, where Project editors can process the data and send it to Destinations).
...
## Access Subscriptions {#subs-tab}

From your Worker Group's submenu, select **Projects**, and then select **Subscriptions**.
...
![Configuring a new Subscription](project-subs-config.1ea2946b4f.png)
{border="true"}

## Configure a New Subscription {#configure-new}

1. Select **Add Subscription** to open the configuration options described below.

1. Assign this Subscription a unique **Subscription ID**. (This can include letters, numbers, `_`, and `-`. IDs are case-sensitive, so entering the same characters with different capitalization will create a separate ID.)

1. Modify the **Filter** by replacing the default `true` with a JavaScript expression that selects a subset of events to filter through this Subscription. (Your expression can specify an upstream sender, a data type, the presence of particular fields, and more. For examples, see [Build Custom Logic to Route and Process Your Data: Filter Expressions](filter-and-transform-data#filters).)

1. Modify (as needed) the Source's default `passthru` [Pre‑processing Pipeline](pipelines#input-conditioning-pipelines).

1. Optionally, add a **Description** of the Subscription's purpose.

1. Optionally, assign the Subscription to one or more Projects. (You can also import Subscriptions from individual [Projects' configurations](configuring-projects#configure-new), and can control Subscriptions' visibility in the [Project view](configuring-projects#configure-new).)

1. Select **Save** to save the new Subscription to the Subscriptions table.

1. Commit and deploy your changes.

    > A Subscription must specify a Pre‑processing Pipeline, even if this is the default `passthru`, which performs no processing.
    {.box .warning}

## Add and Preview More Subscriptions {#configure-more}

On the **Subscriptions** page, you can select **Add Subscription** to define multiple Subscriptions. These stack on the left, like Routes in Cribl Stream's [Routing table](routes).
...
## Edit a Subscription {#editing}

To update an existing Subscription, just select its accordion in the Subscriptions table. This will reopen the same configuration options outlined [above](#configure-new).
