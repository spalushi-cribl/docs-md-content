...
Once you’ve configured a [Collector](collectors), you can run it in one of two ways:

* **Ad hoc**: Execute a one-time collection job immediately. This is useful for testing configurations, previewing data, or replaying a specific dataset.

* **Scheduled**: Configure a job to run on a recurring interval, effectively turning a static data source into a continuous stream.

Before running a job, it's important to understand when your configuration edits are applied. Certain Collectors update automatically without a **Commit & Deploy**, while others require it. See [Collector Changes](collectors#changes) for details.
...
In the Preview mode, a collection job will return only **a sample subset** of matching results (for example, 100 events). This is useful in cases when users need a data sample to:
...
**Lower task bundle size**: Limits the bundle size for small tasks. For example, bundle five 200KB files into one 1MB task bundle. Defaults to `1MB`.

**Upper task bundle size**: Limits the bundle size for files above the **Lower task bundle size**. For example, bundle five 2MB files into one 10MB task bundle. Files greater than this size will be assigned to individual tasks. Defaults to `10MB`.
...
Cribl Stream processes time-based tokens as follows:
  ...
  * Paths may contain more than one partition. For example, `/my/path/2020-04/20/`.
...
The behavior of a collection job during a Cribl Stream shutdown depends on how the job was started and how the shutdown was initiated.

**Explicit Restart or Stop**: If you explicitly restart or stop Cribl Stream, this will cancel any currently running jobs. This applies to executing the `./cribl restart` or `./cribl stop` CLI commands, as well as to selecting **Settings** > **Global** > **Controls** > **Restart**.

**Unintended Shutdowns**: 
* **Ad hoc** jobs can be configured to automatically resume if they were interrupted by an unexpected shutdown.  
* **Scheduled** jobs will **not** resume automatically after any type of shutdown. They will wait for their next scheduled run time.
