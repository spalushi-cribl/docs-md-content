...
2. In the **New Source** modal, configure the following under **General Settings**:
     ...
     - **Topic ID**: ID of the Pub/Sub topic from which to receive events. If **Monitor subscription for new messages** is enabled, this setting is set to a default value and ignored. Otherwise, it is required for topic or subscription creation.
     - **Subscription ID**: ID of the subscription to use when receiving events. When **Monitor subscription for new messages** is enabled, the fully qualified subscription name must be entered. For example: `projects/myProject/subscriptions/mySubscription'`
3. Next, you can configure the following **Optional Settings**:
   - **Monitor subscription for new messages**: Toggle on to monitor and pull messages directly from the specified Pub/Sub subscription, without requiring a **Topic ID**. Use this when you only have access to a subscription or do not need to manage the topic. When enabled, **Create topic** and **Create subscription** are not relevant and are disabled.
   - **Create topic**: If toggled on, Cribl Stream will create the topic on Pub/Sub if it does not exist. Disabled when **Monitor subscription for new messages** is enabled.
   - **Create subscription**: If toggled on (default), Cribl Stream will create the subscription on Pub/Sub if it does not exist. Disabled when **Monitor subscription for new messages** is enabled.
...
Use the **Google authentication method** drop-down to select a Google authentication method:
