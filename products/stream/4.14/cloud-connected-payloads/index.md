...
The heartbeat interval is determined by the **Task heartbeat period** field, which specifies the number of seconds between each event. In the provided example below, the heartbeat is sent every `10` seconds. This interval is configurable, meaning administrators can adjust it to optimize performance based on their needs. For details, see [Task Manifest and Buffering Limits](group-settings#task-manifest-and-buffering-limits) in Worker Group Settings.
