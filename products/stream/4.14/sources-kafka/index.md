...
2. In the **New Source** modal, configure the following under **General Settings**:
     ...
     - **Bootstrap servers**: Enter each Kafka bootstrap server you want to use. Specify the hostname and port (such as `mykafkabroker:9092`) or just the hostname (in which case Stream will assign port `9092`).
     - **Topic**: Enter the name(s) of topics to subscribe to. Press `Enter`/`Return` between multiple entries.
...
3. Next, you can configure the following **Optional Settings**: 
   ...
   - **From beginning**: Whether to start reading from the earliest available data. Relevant only during initial subscription. Defaults to toggled on.   
...
4. Optionally, you can adjust the [TLS](#tls), [Authentication](#auth), [Schema Registry](#schema), [Processing](#processing), [Retries](#retries), and [Advanced](#advanced-tab) settings, or [Connected Destinations](#connected) outlined in the sections below.
5. Select **Save**, then **Commit & Deploy**. 

### TLS Settings (Client Side) {#tls}

**Use TLS**: Toggled off by default. When toggled on:

**Validate server certs**: Reject certificates that are not authorized by a CA in the **CA certificate path**, or by another trusted CA (such as the system's CA). Default is toggled off.
...
This section details the SASL (Simple Authentication and Security Layer) and OAuth authentication settings for connecting to bootstrap servers. For enhanced security, using [TLS](#tls_client) is highly recommended.

To begin, toggle **Enabled** on (the default is off). This presents you with options to configure authentication via `SASL` or `OAuth`.

#### SASL Authentication

Use the **SASL mechanism** dropdown to select your authentication method from the available options: `PLAIN`, `SCRAM-256``, `SCRAM-512`, and `GSSAPI/Kerberos`. The controls displayed will vary based on your selection.

##### PLAIN, SCRAM-256, or SCRAM-512 

For these methods, you can provide credentials either through:
...
> Kerberos authentication is not supported for Cribl-managed Workers in Cribl.Cloud, but it is enabled for customer-managed (hybrid or on-prem) Worker Groups, whether the Leader is based in Cribl.Cloud or on-prem.
>
{.box .cloud}

If you select Kerberos, you must provide the following:
...
#### Enable OAuth Authentication {#oauth}

> Enabling OAuth will replace any existing authentication settings with placeholder values. Old configurations will continue to work without modification after an upgrade, and disabling OAuth will restore your original settings.
>
{.box .warning}

Toggle **Enable OAuth** on to configure this method. To configure, you must provide the following: 
 
- **Token URL**: URL of the token endpoint to use for OAuth authentication.

- **Client ID**: ID to use for OAuth authentication.

- **Client secret (text secret)**: Select or create a stored text secret.

- **Add OAuth Parameters**: Optionally, click **Add parameter** for each additional OAuth paramater. These are parameters sent to the authentication provider in the body of the token request. They are often used for `scope` (for example, in Amazon Cognito) or `audience` (for example, in `Auth0`).

In the resulting table, enter the **Name** of each field per row. The corresponding Value should be a literal value (such, a string, a number, or a boolean).

- **Add SASL Extension Fields**: Optionally, click **Add Field** for each additional SASL extension fields. These are fields that are set as extensions when authenticating with the Kafka cluster. For example, Confluent Cloud uses fields like `logicalCluster` or `identityPoolId` for granular access control. You do not need to include the `extension_ prefix` when configuring these fields in Cribl.

In the resulting table, enter the **Name** of each field per row. The corresponding Value should be a literal value (such, a string, a number, or a boolean). 

### Schema Registry {#schema}

This section governs Kafka Schema Registry Authentication for data encoded with Avro or JSON schemas, with schemas stored in the Confluent Schema Registry. 
...
**Schema type**: Select the schema format for event serialization and deserialization. This is the schema used from your configured Schema Registry.
   * `Avro`: (Default) Encodes and decodes events using Avro schemas.
   * `JSON`: Encodes and decodes events using JSON schemas. Choose this option for native compatibility with environments that use the JSON schema standard.
...
**TLS enabled**: When toggled on, displays the following TLS settings for the Schema Registry.
...
**Byte limit, per partition**: The maximum amount of data that the server will return per partition. Must equal or exceed the maximum message size the server allows. (Otherwise, the producer will be unable to send messages larger than the consumer can fetch.) If not specified, defaults to `1048576`.

**Byte limit**: Maximum amount of bytes to accumulate in the response. The default is `10485760 (10 MB)`.

**Error limit, per socket**: Maximum number of consecutive request errors that can occur on a single socket connection before the connection is discarded and reestablished. This mitigates issues with idle connections, particularly those used for sending heartbeats, thereby reducing the number of rebalances in a consumer group. Default is `0`, which disables this feature. Accepts values `1-100`.
...
* If the incoming Kafka message contains a `_time` field whose value is **not** a timestamp in UNIX epoch time (such as an ISO or UTC timestamp), that becomes the value of the emitted event's `__origTime` field, **and** the message's timestamp becomes the value of the emitted event's `_time` field.
...
[Snippet not found: content/shared/4.14/snippets/_common-errors-kafka.md]
