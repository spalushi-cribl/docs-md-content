...
> Cribl Edge on macOS offers support for only a subset of System State collectors (see [Cribl Edge on MacOS](/edge/edge-macos#sources-and-destinations)).
>
{.box .cloud}

## Configure Cribl Stream to Collect System State Events 
...
1. On the top bar, select **Products**, and then select **Cribl Stream**. Under **Worker Groups**, select a Worker Group. Next, you have two options:
     ...
     - To configure via [Routes](routes), select **Data** > **Sources** (Stream) or **More** > **Sources** (Edge). Select the Source you want. Next, select **Add Source**.
1. Configure the following under **General Settings**:
   - **Enabled**: Toggle on to enable the Source. 
   - **Input ID**: Enter a unique name. The default Source is prefilled with the value `in_system_state`, which can't be changed via the UI. If you clone this Source, Cribl Stream will add `-CLONE` to the original **Input ID**.
   - **Description**: Optionally, enter a description.
1. Next, you can configure the following **Optional Settings**: 
   - **Polling interval**: How often to collect metrics, in seconds. Defaults to `300`. Each run of the state collector generates events with identical `_time` values.
...
1. Optionally, you can adjust the [Collector](#collector), [Processing](#processing), and [Advanced](#advanced-tab) settings, or [Connected Destinations](#connected) outlined in the sections below.
...
> On Cribl Edge, this option is available for Windows and Linux. You must create a different Worker Group for each OS. For details see, [Mapping Edge Nodes to Fleets](/edge/mapping-edge-nodes/).
>
{.box .info}
...
When Cribl Edge is deployed in a container, the **DNS** Collector does not collect any events.
...
| `source` |The source of the data used to populate the event. |
| `dnsClient` | Reports `MSFT_DnsClient` when the native WMI method is used (default). Reports `Get-DnsClient` when the **Use Windows Tools** toggle is enabled in **Advanced Settings**. |
...
With the **Enabled** toggle on (the default), Cribl Stream will create events for each configured service (for example `systemd` and `initd`) along with their enabled and running status.
...
When Cribl Edge is deployed in a container, the **Services** Collector does not collect any events.
...
**Use Windows Tools**: Toggled off by default.
This legacy setting will be removed in a future release.
...
This setting affects Windows Edge Nodes only, it has no effect on Linux Edge Nodes or in Cribl Stream.

- If toggled off, the System State Source interacts directly with the Windows Management
    Instrumentation (WMI) API, resulting in faster data collection.
