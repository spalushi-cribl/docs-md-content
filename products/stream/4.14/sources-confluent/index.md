...
Cribl Stream supports receiving [Kafka](https://kafka.apache.org/documentation/) topics from the [Confluent Cloud](https://docs.confluent.io/cloud/current/overview.html) managed Kafka platform. As of Cribl Stream 3.3, this Source automatically detects compressed data in `Gzip`, `Snappy`, or `LZ4` format.
...
## Ingest Kafka Topics from Confluent Cloud
...
2. In the **New Source** modal, configure the following under **General Settings**:
    ...
    - **Bootstrap servers**: List of Confluent Cloud bootstrap servers to use, for example, `myAccount.confluent.cloud:9092`.
...
**Validate client certificates**: Reject certificates that are not authorized by a CA in the **CA certificate path**, or by another trusted CA (such as the system's CA). Defaults to toggled on.
...
> Enabling OAuth will replace any existing authentication settings with the word `placeholder`. 
>
{.box .warning}
...
**Enabled**: Defaults to toggled off. When toggled on, displays the following controls:
