...
You can assign different access and capabilities (Permissions) to each user
([Member](members)) or user group ([Teams](teams)) at multiple levels:
...
Assign Organization-level Permissions per individual user when you
[invite them](members#invite-members) to your Organization as Members or by
updating an **Organization Permissions** assigned to a user after they
join.

If you [configured SSO](sso-initial-steps), grant Organization-level Permissions
by managing group membership within your IdP. Make sure that your IdP group
names align with the core patterns for [automatic Permission
mapping](sso-initial-steps#idp-group-name-mapping).

Cribl.Cloud does not support globally predefining Permissions, as with on-prem Cribl Stream.
...
> Cribl.Cloud does not use the earlier access-management model of Local Users and Roles/Policies that on-prem Cribl Stream still supports. You can create and configure only Members and Permissions – and optionally, [Teams](teams) – on Cribl.Cloud Organizations and applications ([Cribl Search](/search/cloud-managing#search-member-roles) and [Cribl Lake](/lake/lake-permissions)). References elsewhere in Cribl docs to Local Users, Roles, and Policies do not apply to Cribl.Cloud Organizations or applications.
>
{.box .info}
...
> For more-specific Product-level Permissions in Cribl Search, see [Search Member Permissions](/search/cloud-managing#search-member-roles).
> For Product-level Permissions in Cribl Lake, see [Lake Permissions](/lake/lake-permissions).
{.box .success}

| Permission                                                                               | User | Read Only | Editor | Admin |
| ---------------------------------------------------------------------------------------- | ---- | ----- | ----- | ----- |
| Can be assigned to Worker Groups and resources | ✓    | ✓     | ✓     | ✓     |
| View all Members, Worker Groups, Settings, Leader commits, and legacy Local Users and Roles |      | ✓     | ✓     | ✓     |
| View all Groups and Monitoring pages |      |       | ✓     | ✓     |
| Create, view, update, and delete all Worker Groups and resources |     |      |      | ✓     |
| Manage Worker Group Mappings |     |      |      | ✓     |
| Add, update, restart Worker Nodes |     |      |      | ✓     |
...
- A **Read Only** Permission automatically [inherits](#inheritance) **Read Only** Permissions on all Worker Groups and lower-level resources.
- An **Editor** Permission [inherits](#inheritance) the **Editor** Permission on Worker Groups, and the **Maintainer** Permission on lower-level resources.
...
| ---------------------------------------------------------------------------------------- | ---- | ----- | ----- | ----- | ----- |
| Can be assigned to resources within the Worker Group | ✓    | ✓     |       | ✓     | ✓     |
| View all Worker Group-level Settings, encryption keys, certificates, secrets, scripts  |      | ✓     |      | ✓     | ✓     |
...
| Run Collection jobs on the Worker Group |     |      | ✓     | ✓     | ✓     |
| Create, view, update, and delete all Worker Group-level encryption keys, certificates, secrets, scripts, Sources, Destinations, Pipelines, Packs, Routes, QuickConnect connections, Knowledge objects, and Notifications and Notification targets |     |      |       | ✓     | ✓     |
| Commit configuration changes  |     |      |       |✓     | ✓     |
| Create, view, update, and delete all Worker Group-level access management (Members' Permissions), Settings, encryption keys, key management system (KMS) settings, certificates, secrets, scripts |     |      |      |      | ✓     |
...
| Add, update, and restart Worker Nodes. |     |      |      |      | ✓     |
...
Worker Group-level Permissions inherit Permissions of a different level in the following manners:
...
1. Make sure this Member has a [**Workspace permission**](#workspace) level sufficient to support the Permission you want to assign on the Worker Group.
1. In the sidebar, select **Settings**, then **Stream**/**Edge**, then **Members and Teams**.
1. Select each applicable Member row to open their **Member Details** drawer.
1. Grant the Member the Permission level you want on this Worker Group.
...
> The v.4.1.x `project_user` Role and `ProjectSourceSubscribe` Policy, which provided access to all Projects, are retired in Cribl Stream 4.2.x and newer.
...
- Members with the `Admin` Permission at the [Organization](#permissions-org) level will be locked to the `Admin` Permission on [Products](#permissions-product), to the `Editor` Permission on [Worker Groups](#group), and to the `Maintainer` Permission on lower-level [resources](#resource).
