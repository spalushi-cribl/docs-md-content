...
Variables are scoped to different levels of the application: Single-instance deployment, Worker Group, or Pack.
...
### Worker Group Variables {#group-variables}

To access variables specific to a given Worker Group:

1. Select **Worker Groups** from the sidebar, and choose a Worker Group.
1. On the **Worker Groups** submenu, select **Processing**, then **Knowledge**.
1. In the sidebar, select **Variables**.
...
Variables can be accessed via `C.vars` – which can be called anywhere in Cribl Stream that JS expressions are supported. Typeahead is provided. See [Cribl Expressions](cribl-reference) for more information.
...
> the value returned will be the `redis` Pack variable, not `redis` as defined in the Worker Group.
{.box .info}
