...
This document outlines how to configure Cribl Stream to use a proxy server for outbound HTTP/S requests.

There are two ways to define proxy variables in Cribl Stream:

- **Using environment variables**: This method is independent of the systemd service manager and can be used regardless of how Cribl Stream is installed or started.
- **Using systemd override files**: This method is specific to systemd and requires that Cribl Stream is started as a systemd service.
Choose the method that best suits your deployment environment and preferences.

### Prevent Proxy, Config, and Case Conflicts
...
### Set Up Proxy Environment Variables (Using Environment Variables)
...
Configure the `HTTP_PROXY` and `HTTPS_PROXY` environment variables, either with your proxy's IP address, or with an FQDN that resolves to that IP address. 

Optionally, follow either convention with a colon and the port number to which you want to send queries. Some `HTTP_PROXY` examples:
...
### Set Up Proxy Configuration with systemd {#systemd-proxy}

If you are proxying outbound traffic and starting Cribl Stream using systemd, add your proxy environment variables to the systemd override file (see [Persisting Overrides](run-stream#persisting-overrides)). Add statements of this form:
...
> Cribl v.4.6.0 introduces direct S3 downloads for configuration bundles, bypassing the Leader Node and scaling deployments with many Worker Nodes. However, if your S3 bucket is firewalled or requires access controls, a proxy might be needed. In such scenarios, you need to exclude the S3 bucket address from the "NO_PROXY" list for successful downloads. 
>
{.box .info}
...
### Destinations
  ...
  * [Google Cloud Chronicle API](destinations-google-chronicle-api)
