...
**Use the AppScope Source IP/port or UNIX domain socket**: Toggle on to apply the transport options defined in the associated [AppScope Source](sources-appscope). Toggle off to expose the following fields in this section.
...
**Emit metrics**: Defaults to toggled on, which exposes the following options.
...
**Emit events**: Defaults to toggled on, which exposes the following controls in this section.
