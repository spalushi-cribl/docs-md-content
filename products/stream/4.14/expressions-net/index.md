...
```js
C.Net.cidrMatch(cidrIpRange: string, ipAddress: string): boolean
```
...
```js
C.Net.communityIDv1(sourceIP: string, destIP: string, sourcePort: number | undefined, destPort: number | undefined, proto: string | number, seed: number = 0): string
```
...
- For network flows coming from the Cribl Stream [NetFlow & IPFIX Source](sources-netflow), use the protocol name as-is – the Source will provide it in correct form.
...
```js
C.Net.isIp(value: string): boolean
```
...
```js
C.Net.isIpV4(value: string): boolean
```
...
```js
C.Net.isIpV6(value: string): boolean
```
...
```js
C.Net.isIPV4Cidr(value: string): boolean
```
...
```js
C.Net.isIPV6Cidr(value: string): boolean
```
...
```js
C.Net.isIpV4AllInterfaces(value): boolean
```
...
```js
C.Net.isIpV6AllInterfaces(value): boolean
```
...
```js
C.Net.ipv6Normalize(address: string): string
```
...
```js
C.Net.isPrivate(address: string): boolean
```
...
```js
C.Net.parseAddressOrRangeString(source: string, mask?: string | number)
```
