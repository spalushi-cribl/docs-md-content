...
> A Cribl.Cloud deployment on a [certain plan](https://cribl.io/pricing/) also
> allows you to create Cribl-managed Worker Groups in Cribl.Cloud. See [Managing Cribl.Cloud Worker Groups](cloud-workers) for more details. 
{.box .cloud}
...
1. Enter a unique **Worker Group name**.
...
1. Optionally, type in a **Description** and select [**Tags**](group-settings#worker-group-configuration).

![Creating a New Group](new-group-4.8.png)
{scale="60%" border="true"}

You can also create a new Worker Group by selecting **Products** on the top bar, hovering over **Worker Groups** in the sidebar, and selecting the **+** button.

![Creating a New Worker Group from **Products** Sidebar](product-create-worker-group.png)
{scale="60%" border="true"}

> You can also use the [Cribl API](/cribl-as-code/api/) to [create a new Worker Group](/cribl-as-code/configure-worker-groups#configure-worker-groups-api).
>
...
1. In the sidebar, select **Worker Groups**, then select **Add Worker Group**.
1. Set **Worker Group name** to be exactly the same as the deleted group. Fill in the remaining information and **Save**.
