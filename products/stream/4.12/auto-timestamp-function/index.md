...
> The Auto Timestamp Function uses the same basic algorithm as [Event Breakers](event-breakers#timestamp-settings) and the [C.Time.timestampFinder()](expressions-time#time-finder) native method.
>
{.box .success}
...
**Additional timestamps**: To extract additional timestamp formats, select **Add Timestamp** to define each format. Each row will provide these fields: {{< id `add_timestamps` >}} 
...
> For details about Cribl Stream's Library (native) time methods, see: [C.Time – Time Functions](expressions-time).
>
{.box .info}
...
In order to use auto timestamping upon ingestion, the formatting used must match the `%Z` parameters above. For example, this Function will automatically parse all of these formats:
...
Create a Pipeline with one Auto Timestamp Function. Leave the settings of the Function to their defaults.

Sample event: 

`Jan 20 12:03:55 PA-VM 1,2025/01/20 13:03:58,CRIBL,TRAFFIC,end,2049,2024/09/20 14:03:58,314.817.108.226,10.0.0.102,314.817.108.226,10.0.2.65,cribl,,,incomplete,vsys1,untrusted,trusted,ethernet1/3,ethernet1/2,log-forwarding-default,2024/09/20 13:03:58,574326,1,53722,8088,53722,8088,0x400064,tcp,allow,296,296,0,4,2024/09/20 13:03:45,7,any,0,730277,0x0,United States,10.0.0.0-10.255.255.255,0,4,0,aged-out,0,0,0,0,,PA-VM,from-policy,,,0,,0,,N/A,0,0,0,0`

To add this sample (after creating an Auto Timestamp Function with the above **Filter** expression), in the Pipeline page go to **Sample Data** > **Import Data**, and paste the data snippet above. Do not make any changes to timestamping or line breaking, and select **Save as Sample File**.
...
- `_time: 1737374635`
- **GMT**: Jan 20, 2025 12:03:55 PM GMT
- **Your Local Time**: Jan 20, 2025 1:03:55 PM GMT+01:00

Because no explicit timezone has been set (under **Default Timezone**), `_time` will inherit the **Local** timezone, which in this example is `GMT +01:00`.

![Auto Timestamp Basic Example](auto-timestamp-timezones-4.10.png)
{border="true"}
