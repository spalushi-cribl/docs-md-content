...
**Automatic schema**: Toggle on to automatically generate a Parquet schema based on the events of each Parquet file that Cribl Stream writes. Toggle off (default) to expose the following additional field: 
...
> Cribl Stream will close files when **either** of the `File size limit (MB)` or the <code>Max&nbsp;file open time (sec)</code> conditions are met.
>
{.box .info}
