...
1. In the **Data Generator File** menu, select `businessevent.log`.
   ...
   ![Configure a datagen Source](tutorial-source-configure-datagen-4101.png)
...
1. Verify that the datagen Source is ready to generate sample data by confirming that the **Enabled** column next to the `businessevent` toggle is toggled on.

   ![Verify datagen Source](tutorial-businessevent-enabled-4101.png)
   {scale="100%" border="true"}
...
## Step 3: Add a Pipeline {#pipeline}
...
Now that you've set up your Pipeline, you will learn how to capture and filter sample data in the next step. You can use sample data to test and validate your Pipeline as you build it.

## Step 4: Capture and Filter Sample Data to Test the Pipeline {#filter}

In this step, you will learn how to capture sample data from the Pipeline you added in the previous step. The Data Preview tool is useful for previewing and testing data transformations as you build them. It gives you confidence that your Pipeline configurations results in the intended output. The Data Preview allows you to get data from multiple places, such as inbound streams, uploaded files, and from your computer's clipboard. See [Data Preview](data-preview) for more information.
...
1. In the Data Preview, find at least one entry with a `marketState` key-value pair and take note the typical values, which will be the abbreviation for a U.S. state. You may need to select the **Show more** link to expand events to see this key-value pair.
...
As you build Pipelines, generate, or upload sample data and check the Data Preview when working with Pipelines to validate your changes.
...
1. Save your changes. Then, confirm that the Data Preview pane for the `tutorial-sample-data.log` has switched from the **IN** tab to the **OUT** tab. This tab shows you the outbound effect of the Pipeline you just saved. The `_raw` field for each event has changed color, indicating that it has undergone some redactions.
...
1. Confirm that the Data Preview pane now displays many more fields parsed from `_raw`. While in this view, notice also that many fields have `NA` (not applicable) values.
...
1. Confirm that the Data Preview pane now no longer shows any fields with the value `NA`, such as `credits`, `EventConversationID`, and `ReplyTo`. The remaining fields should display meaningful values.

   > If you didn't see the fields change, toggle the Parser Function off in the Pipeline. Then, select **Save** and watch the Data Preview pane change again. By toggling Functions on and off, you can test and troubleshoot each Function's effect. Remember to toggle it on again before moving on to the next step in the tutorial.
   >
   {.box .info}
...
In this step, you will use the Eval Function to enrich and streamline your data. The Eval Function adds or removes fields from events. See [Eval Function](eval-function) for more information.
...
1. Save the changes. In the Data Preview pane, confirm that data events now contain a `phoneCompany` field that matches its `phoneType`.

1. In the Eval Function in the **Remove Fields** option, type `_raw`.

1. Save the changes and confirm that the Data Preview now shows the `_raw` field removed from each event.
...
1. Save the changes. In the Data Preview pane, confirm that the `conversationId` field has changed to the `ID` instead.
...
1. In the Data Preview pane, confirm that events with the `accountType` of `PrePaid` are now struck out and faded, which indicates that Cribl Stream will drop them before forwarding the data to a downstream receiver. Verify also that events with the `accountType` of `PostPaid` remain.

   > As a reminder, you can turn any Function off by toggling it off in the Pipeline. Then, select **Save** and watch the Data Preview pane change again. By toggling Functions on and off, you can test and troubleshoot each Function's effect. Remember to toggle it on again before moving on to the next step in the tutorial.
   >
   {.box .info}
...
1. Leave **Enable Expression** toggled off (default).

   > Toggle **Enable Expression** on to change the **Destination** field to a **Destination Expression** field where you can enter a JavaScript expression for your **Destination** name.
   >
   {.box .info}

1. Set **Destination** to either `devnull` or `default`.
...
1. Leave the **Description** empty and leave **Final** toggled on.
...
1. With the Route configured, you now need to move the Route so that it processes events first. Select the drag handle for this Route and drag it above the `default` Route.
   ...
   ![Move the Route](tutorial-route-move-4120.png)
   {border="true"}
