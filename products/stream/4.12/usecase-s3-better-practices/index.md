...
- [Write](#writing-to-s3-dest) to a Cribl Amazon S3 Destination.
- [Read](#reading-from-sqs-source) from Cribl's Amazon SQS Source.
- [Replay](#replaying) from Amazon S3 buckets, via Cribl's S3 Collector.

Then, this guide will go on to detail 10 "better practices" (and a bonus one!) as we cover four broad topics:

- Write to object storage, [generically](#generic).
...
![Write to object storage – example](st-usecase-best-practices-ss1.865e49e5fc.png)
{border="true"}

#### Write to a Cribl Amazon S3 Destination {#writing-to-s3-dest}
...
#### Read from Cribl's Amazon SQS Source {#reading-from-sqs-source}
...
#### Replay from Amazon S3 Buckets {#replaying}
...
- Each Destination writes files to its staging directory until the number of files exceeds the configured maximum, or the Destination reaches any of the following limits: **File size limit (MB)**, **Idle time limit (sec)**, or **File open time limit (sec)**.
...
| **# of S3 Destinations in Cribl** | **Max cardinality per Dest.** | **Open file limit in Cribl Dest.** | **# Worker Process on each Worker Node** |**Min Linux per-process open file limit** | **Min Linux system plus Cribl user open file and 1k reserved for the system** |
| --- | --- | --- | --- | --- | --- |
...
A partitioning expression and file prefix expression should produce fewer than 2,000 potential combinations. When a Worker Process exceeds the configured **Open file limit**, this generates errors; all open files for that Worker Process are simultaneously closed; and, new files will be created.
...
#### Analyze Partitioning Expressions
...
#### See How Cardinality Changes Over Time 
...
- Setting **Open file limit** high enough.
...
- Toggle **Advanced Settings** > **Remove staging dirs** on. This means that any empty directory that has not been written to for an hour, is deleted.
...
#### Determine Staging Directory Disk Size {#disk-size-formula}
...
#### Verify S3 Access {#verifying-s3-access}
...
```
#### Access Amazon S3 from Cribl.Cloud {#access-from-cribl-cloud}

Every Cribl.Cloud Organization exposes its own unique AWS ARN. 

From the main Cribl.Cloud portal, navigate to the **Workspace** you are working in, and select **Trust** from the sidebar to expose the Cribl.Cloud ARN.

![Cribl.Cloud ARN](st-usecase-best-practices-ss4.png)
{border="true"}
...
2.  **Max file open time (sec)**: First, determine your cardinality over 1 minute, 3 minutes, 5 minutes, and 10 minutes. Adjust this setting to match the lowest-cardinality period. E.g., if cardinality is 10,000 over 5 minutes, but 2,000 over 90 seconds, change this setting to 90 seconds. If cardinality is 2,000 over 5 minutes, then the default 5 minutes (300 seconds) is acceptable.
3.  **Max idle time (sec)**: The default 30 seconds is generally acceptable. In cases with high cardinality, lowering this setting can result in fewer open files when processing bursty data.
4.  **Data format**: If using a Passthru Pipeline, change this setting to `Raw`. Otherwise, leave it at the JSON default to preserve all metadata added to the events.
5.  **Storage class**: For Amazon S3 Destinations, consider the "infrequent access tier" or "intelligent tier" to reduce overall cost if the data will be rarely searched. Examples of rarely-searched data include threat-hunting or compliance-reporting use cases.
...
![Configuring S3 Destination Settings](st-usecase-best-practices-ss5-4101.png)
{border="true"}
...
## Improve Partitioning Expressions {#expressions}
...
- Open file limit within Cribl Destination set to 2,000.
- File open time limit of 5 minutes.
