# Configure Cribl Stream for Non-Root User
...
- **Using setcap (Linux with POSIX capabilities)**: This method directly modifies the executable file to grant it specific privileges. For details, see [Persisting Overrides on initd](stream/run-stream#persisting-overrides-initd).
...
For details about setting these capabilities, see [Persisting Overrides on systemd](run-stream#persisting-overrides).
