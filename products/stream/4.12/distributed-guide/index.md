...
This tutorial builds on our [Cribl Stream Tour](tour) and [Getting Started Guide](getting-started-guide) by walking you through a Distributed deployment – Cribl Stream's typical deployment type for production.
...
To deploy a Leader Node, plus (for example) two Workers already configured and wired up to the Leader, use this command:
...
For example: `curl 'http://localhost:9000/init/install-worker.sh?token=<token>&tag=dev&tag=test''`
...
1. See these AWS instructions for SSH access:
...
3. In the sidebar, select **Settings**, then **Global**.
4. Select **Distributed Settings**, then **General Settings**.
5. In **Mode**, choose `Leader`.
6. Select **Leader Settings**, and make sure the **Port** is set to `4200`.  
   (This port must be open for Workers to communicate with the Leader.)
7. Select **Save** to restart the Cribl server in Leader mode.
...
![Distributed > Leader Settings](leader-settings-4.9.png)
{border="true"}

8. {{< id `worker-ui-access` >}} Optional but recommended: In the sidebar, select **Worker Groups**. Then enable **Teleport** for your Group(s). This way, you will be able to click through from the Leader's **Workers** page to view and manage each Worker's UI, distinguished by a [purple border](#teleport-border). (This option handles authentication for you, so you don't need to manage or enter the Workers' credentials.)
...
1. In **Mode**, choose `Stream: Managed Worker (managed by Leader)`.
...
1. Select **Leader Settings**, and make sure the **Address** matches the domain of the Leader you [previously configured](#leader). 
1. Still in **Leader Settings**, display the **Auth token** value. 
   Optionally, you can change this from the default.
1. Leave other settings unchanged, and select **Save** to restart this instance as a managed Worker.
...
1. Return to the Leader instance you [previously configured](#leader).
1. In the sidebar, select **Workers**. 
1. You should now see both Workers, mapped to the `default` **Group**.
...
> - Enable the Leader's **Teleport** option, as [recommended above](#worker-ui-access); or
...
1. In the Leader's sidebar, select **Worker Groups**. 
...
1. Select **Save**. {{< id `datagen-status` >}}  
   In the **Enabled** column, the toggle toggled on indicates that your Datagen Source has started generating sample data.
1. Select **Commit** at the upper right, enter a commit message, and select **Commit & Deploy**.  
   This uses Cribl Stream's `git` integration to record the `default` Group's new configuration and deploy it to your Workers.

![Source (Datagen) configuration](datagen-weblog-config-4101.png)
{border="true"}

> If you'd like to verify that the Datagen is sending events, wait about a minute for them to start accumulating. Then, on the **Datagen** Sources page, select **Live** in the **Status** column for your configured Source. On the resulting **Live Data** tab, you should see events arrive within the default 10-second capture.
>
{.box .success}
...
1. In the Cribl Stream Leader's UI, configure a Splunk Single Instance Destination, following [these instructions](destinations-splunk). For this simulated output:
   ...
   - Set the **Address** to `127.0.0.1` (that is, localhost).
...
![A glorious spoofed Splunk Destination config](splunk-local-config-4.9.png)
{border="true"}

2. In the **Version Control** menu at the upper right, select **Commit**, enter a commit message, and confirm the commit.
3. Select **Deploy** to deploy this new configuration to your Workers.
...
5. Enter `nc -h` to check whether netcat is installed on this Linux instance.
  ...
  If the command fails, follow your Linux distro's steps for installing netcat. (For example, for Ubuntu instructions, see [Docker Notes](#docker-notes) below.)
...
> You can streamline future Commit and Deploy steps by entering a **Default commit message**, and by [collapsing actions](version-control#collapse-actions) to a combined **Commit and Deploy** button. Both options are available at **Settings** > **Global** > **System** > **Git Settings** > **General**.
>
{.box .success}
...
1. Select the **DevNull** tile.
   (You can use the search box.)
1. On the resulting **devnull** row, look for the **Live** indicator under **Status**. This confirms that the **DevNull** Destination is ready to accept events.
1. In the Destinations list on the left, select the **Default** Destination at the top.
1. Verify that **Default Output ID** points to the **devnull** Destination we just examined.
1. Select **Commit** at the upper right, enter a commit message, and select **Commit & Deploy**.  
...
1. From the **Worker Group** submenu, select **Processing**, then **Pipelines**.
1. Select **Add Pipeline**, then **Add Pipeline** again.
1. In the new Pipeline's **ID** field, enter a unique identifier (for example, `slicendice`).
1. Optionally, enter a **Description** of this Pipeline's purpose. 
1. Select **Save**.

![Pipeline saved](pipeline-prompt-4.9.png)
{border="true"}
...
1. At the top of the **Pipelines** page, select **Add Function**.
2. Search for the `Eval` Function, and select it.
3. In the new Function's **Evaluate Fields** section, select **Add Field**. 
4. In the **Name** column, name the field `origin`.
5. In the **Value Expression** column, enter: `host+" "+source`

   This new `origin` field will concatenate the host and source fields from incoming events.

![Adding Eval Function to Pipeline](add-eval-fn-4101.png)
{border="true"}

6. Select **Save** to store the Function's configuration.
7. **Commit** and **Deploy** the new Pipeline configuration.
8. Optionally, open the right pane and select **Capture Data** to verify throughput.
...
1. At the top of the **Pipelines** page, select **Attach to Route**.
1. Select **Add Route**.
1. Enter a unique **Route Name**, like `demo`.
1. Leave the **Filter** field set to its `true` default, allowing it to deliver all events.
1. Set the **Pipeline** drop-down to our configured `slicendice` Pipeline.
1. Set the **Destination** drop-down to the Destination you configured above. If you boldly chose the [Simulated Splunk Destination](#splunk), this will be named something like `splunk:splunk9997`.
1. You can leave the **Description** empty, and leave **Final** toggled on.
1. Grab the new Route by its left handle, and drag it to the top of the Routing table, so that our new Route will process events first. You should see something like the screenshot below.
1. Select **Save** to save the new Route configuration.
1. **Commit** and **Deploy** your changes.
1. Still assuming you configured a simulated Splunk output, look at the terminal where you're running netcat. You should now see events arriving.

![Route attached to Pipeline and Destination](route-splunk-connected-4120.png)
{border="true"}
...
4. Find the **Datagen** Source you configured [earlier](#add-a-source) and select it.
5. Change the **Events per Second per Worker Node** from the default `10` EPS to a high number, like `200` EPS.
6. Select **Save**, then **Commit** and **Deploy** this higher event load.
7. From the **Version Control** menu at the top tight, select **Commit** to commit the Leader's newest config version. In the resulting modal, select **Commit** again to confirm.
...
![Committing Leader's config](changes-commit-all-4.9.png)
{border="true"}

8. In the sidebar, select **Monitoring**.
9. Watch as the following changes unspool over the next few minutes:

   - The **CPU Load Average (1 min)** will spike as the higher event volume floods the system.
   - The **Workers** indicator at th top will drop to `0` as the Workers restart with the new configuration you've deployed to them, and then rebound to `2`.
...
![](monitoring-load-4.9.png)
{border="true"}

10. To confirm that both Workers are `alive`: In the sidebar, select **Workers**. {{< id `workers-page` >}}
...
To add and manage more than one Worker Group – everything in this optional section – you'll need an Enterprise or Sales Trial license for your on-prem Cribl Stream deployment, or an Enterprise (or equivalent) plan for your Cribl.Cloud Organization. For details on adding multiple Worker Groups on Cribl.Cloud, see [Cribl.Cloud Worker Groups](cloud-workers).
...
The **Worker Groups** page now supports multiple Groups. (For now, you still see only the single `default` Group – but we'll change that a few sections down.)

> On Cribl.Cloud, the **Worker Groups** page adds extra columns. You'll see each Group's Worker type (customer-managed / hybrid versus Cribl-managed /  Cribl.Cloud) and, for Cloud Groups, the infrastructure provider (AWS or Azure), anticipated ingress rate and Provisioned status. For details, see [Cribl.Cloud Worker Groups](cloud-workers).
>
{.box .cloud}
...
1. Either select **Workers** in the top right of the **Worker Groups** page, or select **Workers** in the sidebar.
1. Select the desired Worker's link in the **GUID** column.
1. A purple header indicates that you're viewing a Worker's UI.
1. To return to the Leader, select any option in the sidebar.
{{< id `teleport-border` >}}

![Leader's remote view of Worker's UI](st-worker-teleported-4.9.png)
{border="true"}

#### Check/Restart Workers

For the remaining steps, we want to make sure both our Workers (configured earlier) are up. Cribl Stream's top header should indicate `2 WORKERS`. You can verify that they're `alive` by selecting **Workers** in the sidebar, as shown [earlier](#workers-page). If so, proceed to [Map Groups by Config](#map).

If either or both Workers are down, restart them:

1. Make sure you've [enabled Worker UI access](#worker-ui-access). (It's time!)
1. Click through to a dormant Worker's UI.
1. When you see that Worker's UI (purple header), select **Restart Stream**. 
1. Confirm your choice, and wait for a confirmation message to appear for a few seconds.
1. Return to the Leader's UI.
...
1. You now see a default Mapping Ruleset, also literally named `default`. Select it.
...
![Default Mapping Rule](mappings-default-4.9.png)
{border="true"}
...
1. Select **Add Rule**. Then configure the new Rule as shown below:
...
![A specific Mapping Rule by Filter condition](mappings-2nd-rule-4.9.png)
{border="true"}

2. Select **Save** to add this Rule.
3. Confirm the warning that changes will take effect immediately.
4. From the **Version Control** menu, select **Commit** to commit the Leader's new config.

> For more Mapping Rules/Rulesets details and examples, see [Distributed Deployment](mapping-workers-to-worker-groups).
>
{.box .success}
...
1. In the Leaders' sidebar, select **Worker Groups**.
2. Select **Add Group**.
3. Name the new Group `CriblMetrics` to match its purpose.
4. Select **Enable teleporting to Workers**.

![Creating a new Group](group-new-create-4.9.png)
{border="true"}

5. Save the Group.
6. Select **Deploy** on the new Group's row, and confirm your choice. The new Group should deploy immediately.
7. From the **Version Control** menu, select **Commit** to commit the new config on the Leader.

### Add Another Source

On the new Group, we'll now enable a **Cribl Internal: Metrics** Source, representing a second data type.

1. In the sidebar, select **Worker Groups** , and then select the new `CriblMetrics` Group.
2. On the **Worker Groups** submenu, select **Data**, then **Sources**.
3. Select the **Cribl Internal** tile.
4. Enable the `CriblMetrics` Source and confirm.

![CriblMetrics Source enabled](source-cribl-internal-metrics-4101.png)
{border="true"}

5. Select **Commit** to commit the new config on the Leader and Group.
6. From the **Version Control** menu, select **Commit** to deploy this new configuration to your Workers.
...
1. In the sidebar, select **Mappings**.
2. Select the `default` Mapping Ruleset to open it.
3. Select **Add Rule**. Then configure the new Rule as shown below:
...
4. Move the `Default Mappings` rule to the bottom of the Ruleset, reflecting its catch-all function.
5. Select **Save** to store the new configuration.
6. Confirm the warning that changes will take effect immediately.

![Adding a Rule to map the CriblMetrics Worker to its own Group](metrics-rule-config-4.9.png)
{border="true"}

7. From the **Version Control** menu, select **Commit**  to commit the Leader's new config.

We now have two data Sources and two Worker Groups – one each for (Web) logs versus (Cribl Internal) metrics –  along with two Mapping Rules to map data accordingly. To confirm the Workers' assignment to the two Groups, select **Workers** in the sidebar.

To confirm further details about the Workers, in the sidebar, select **Workers**, and on the resulting **Workers** page, click anywhere on the Worker Node's row to reveal more details:
...
1. In the sidebar, select **Worker Groups** , and then select the `CriblMetrics` Group.
1. On the **Worker Groups** submenu, select **Processing**, then **Pipelines**. 
1. Find the `cribl_metrics_rollup` Pipeline, and click it to expand it.
...
1. At the top of the `cribl_metrics_rollup` Pipeline page, select **Attach to Route**.  
1. Select **Add Route**.
1. Enter a unique **Route Name**, like `metrics`.
1. Leave the **Filter** field set to its `true` default, allowing it to deliver all events.
1. Make sure the **Pipeline** drop-down is set to `cribl_metrics_rollup`.
1. As the **Destination**, select our old friend `devnull:devnull`.  
...
![Cribl metrics Route, Pipeline, and Destination wired up](metrics-route-4120.png)
{border="true"}

### Verify the Multi-Group Deployment

From the sparkline on the Route you just configured, you can already see that metrics data is flowing all the way "out" of Cribl Stream – simulated here by the DevNull Destination. 

To verify the whole configuration you've created, in the sidebar, select **Monitoring**. On the **Monitoring** page, use the **All Groups** drop-down (upper right) to toggle between the two Worker Groups to see the division of labor:
...
![Monitoring individual Worker Groups' throughput](monitoring-groups-4.9.png)
{border="true"}
...
- For example: Different Groups can be managed by different teams. You can filter on DNS rules to send relevant data to the relevant team's Group.
...
![Setting arbitrary Tags on a managed Worker](tags-on-workers-4.9.png)
{border="true"}
...
- Navigate to Cribl Stream's `default` Group > **Data** > **Sources** > **Datagen**, and disable the toggle beside `weblog.log`.

- If you configured a second Worker Group: Navigate to Cribl Stream's `CriblMetrics` Worker Group > **Data** > **Sources** > **Cribl Internal**, and disable the toggle beside **CriblMetrics**.
