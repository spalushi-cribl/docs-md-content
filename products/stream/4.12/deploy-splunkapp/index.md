...
In an on-prem Splunk environment, you can install and configure Cribl Stream as a Splunk app (Cribl App for Splunk), on a [Search Head](#run-on-sh). Note the following limitations:
- You **cannot** use Cribl App for Splunk in a Cribl Stream [distributed deployment](deploy-distributed) as a Leader, or as a managed Worker. 
- You **cannot** install the app on Splunk Cloud or on a Splunk Heavy Forwarder. (See [Single-Instance/​Basic Deployment](deploy-single-instance) and [Distributed Deployment](deploy-distributed) for alternatives.)
