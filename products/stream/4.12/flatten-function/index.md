...
The Flatten Function pulls fields out of a nested structure, moving nested key-value pairs (fields) up to a higher level in the object. You can specify: 
...
Under **Select Event Breaker**, choose **ndjson** (newline-delimited JSON), and select **Save as a Sample File**.
...
- [Reducing Windows XML Events](/stream/usecase-win-xml/), which demonstrates flattening within `_raw` events ingested via Windows XML.
