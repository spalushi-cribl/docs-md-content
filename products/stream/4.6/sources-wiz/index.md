...
1. In the Source modal, configure the following under **General Settings**:
    ...
    * **Authentication method**: Enter the Wiz **Client secret** (64-character string) password credentials manually or with a [secret](securing-and-monitoring#secrets). When choosing **Secret**, you'll select a stored text secret in the resulting **Client Secret (text secret)** drop-down, or click **Create** to configure a new secret.
...
**Advanced Settings** enable you to customize post-processing and administrative options.
   ...
   * **Keep alive time (seconds)**: How often Workers should check in with the scheduler to keep job subscription alive.
   * **Worker timeout (periods)**: The number of **Keep alive time** periods before an inactive Worker will have its job subscription revoked.
...
Navigate to and open the Wiz Source, then use the Source's **Job Inspector** tab to see the results of recent collection runs and check the **Logs** tab for errors to investigate.
