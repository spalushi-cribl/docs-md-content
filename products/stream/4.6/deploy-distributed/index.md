...
For on-prem or Cribl.Cloud hybrid Groups, enter a unique **Name** and (optionally) a **Description** and [**Tags**](settings-group-fleet#workergroup-configuration). You can **Enable teleporting to Workers** now or later (details [here](#worker-access)). 
...
In Cribl Stream 4.1 and later, Cribl.Cloud supports multiple Groups of Cribl-managed ("Cloud") Workers. (This expands earlier support for multiple Groups of [hybrid](cloud-enterprise#hybrid) Workers.) With this feature enabled – which requires an Enterprise plan – Cribl.Cloud's **New Group** modal will provide additional controls.
...
![Changing an existing Group's sizing or provisioning](st-manage-groups.edit.3797236cf7.png)
{border="true"}
