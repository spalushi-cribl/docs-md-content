...
> The Trim Timestamp Function, in current Cribl Stream versions, removes timestamps only from events whose `timestartpos` value is set to `0`. If you need to strip timestamps from arbitrary postions within events, instead use an [Eval](eval-function) Function. 
>
{.box .warning}
