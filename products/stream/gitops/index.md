...
> 
> GitOps requires a Distributed Cribl Stream deployment with a [certain license tier](https://cribl.io/pricing/).
>
{.box .success}
...
* You are not using systemd, and need to override the defaults using [environment variables](environment-variables#gitops-variables). 
...
You need to create a dedicated, private Git repo – your Development environment will write to it, and your Production environment will sync to it. Create a private repo named `cribl`.

To do this in GitHub:

1. In GitHub, select **Create new...** on the top bar, and choose **New repository**.
1. Name the repo `cribl`.
1. Change the repo from **Public** to **Private**.
1. Select **Create repository**. The new repo's landing page will open.
...
1. Create a new [Local User](local-users#create-local-user) called `gitops`. 
1. On the Production Leader's local filesystem, temporarily record the password for the `gitops` User.
   This password well be referred to as `GITOPS_USER_PASSWORD` in later steps.
1. Associate the default `gitops` [Role](roles) with the `gitops` User. (See [Adding Roles](local-users#adding-roles).)
    * This Role allows only to `POST` to the `/version/sync` endpoint of the Cribl API (in the [API Reference](/api-reference), see **versioning**).
...
To do this in GitHub:
1. In the GitHub site for your repo, navigate to **Settings** > **Secrets and variables** > **Actions**.
...
In Cribl Stream, in the UI for the Production Leader, select **Settings** in the sidebar, then select **Global** > **Git Settings**.
...
Check your private Git repo – in the `master` branch, you should now see your commit.

#### Setting Up the `prod` Branch {#setting-up-prod}

In this section, we'll set up the Git `prod` branch; later on, we'll set up `dev`.

Recall that your Git repo has one branch, named `master`. Now, rename `master` to `prod`. 

To do this in GitHub:
 
1. In your GitHub repo's landing page (for example,  `https://github.com/<your_Git_username>/cribl`), select the **1 branch** tab. (That's what the branches tab will be named, assuming that the sole branch is `master`).
   
   ![The branches tab](gitops-01a.4e4b353c53.png)
   {border="true"}
2. In the `master` branch's row, select ••• at far right, then select **Rename branch**, and rename `master` to `prod`.

Now, configure your Cribl Stream instance to be using the `prod` branch:

1. In Cribl Stream, select **Settings** > **Global** > **Git Settings**.
1. In the **General** tab, set **Branch** to `prod`.
1. Save.
...
Add `systemctl` overrides to set [environment variables](environment-variables#gitops-variables) that will (a) make the Production Leader read-only upon startup, and (b) set the branch to `prod`.
...
1. In your GitHub repo's landing page (for example, `https://github.com/<your_Git_username>/cribl`), select the **1 branch** tab. (That's what the branches tab will be named, assuming that the sole branch is `prod`).
...
* Any user with the `admin` Role already has ability to push to the remote repo. If this is your preferred approach, skip to the [Pointing Your Development Leader to Its Branch](#pointing-dev-leader-to-branch).
