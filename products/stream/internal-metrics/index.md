...
These `total.` metrics' values could reflect Cribl Stream's health, but could also report low activity simply due to the Source system. For example, logs from a store site will be low at low buying periods.
...
Cribl Stream composite metrics:
...
## Worker Node Resource Metrics {#worker}

The [Cribl Internal Source](sources-cribl-internal) reports two useful metrics on the resource usage of individual Worker Node Processes:

* `system.cpu_perc` – CPU percentage usage.
* `system.mem_rss` – RAM usage in bytes.


## Worker Node Shutdown Metric {#shutdown}
...
The following metrics provide a detailed view of Source and Destination performance, covering connection status, request activity, latency, and data throughput:
  ...
  * `iometrics.consumer_lag` - Offset between the last message in a partition and the consumer's position (Kafka-based Sources only).
...
* `failure_reason` - Whether the failed request is `retryable` or `non_retryable`.
...
* `status_code` - For failed requests, the status code for the error (`4xx` or `5xx`).
* `reject_reason` - Reason for a rejected connection.
...
- Distributed Worker Groups: [Select a Worker Group] > **Worker Group/Fleet Settings** > **System** > **General Settings** > **Limits** > **Metrics**.
...
`/system/metrics` is Cribl Stream's primary public metrics endpoint, which returns most internal metrics. Many of these retrieved metrics report configuration only, not runtime behavior. For details, see our [API Docs](/cribl-as-code/api/).
