...
You can configure Cribl Stream to send Webhook notifications to the [BigPanda](https://www.bigpanda.io/our-product/) IT Ops platform. These notifications arrive in BigPanda as [Alerts](https://docs.bigpanda.io/en/alerts), which BigPanda correlates into [Incidents](https://docs.bigpanda.io/en/incidents).

Before you begin, you should have an Admin account on a BigPanda Cloud instance.

> To supplement this guide, reference the [Cribl Integration](https://docs.bigpanda.io/en/cribl) document from BigPanda.
{.box .success}
...
2. In the **Integrations** tab, select **New Integration**.
...
1.  Navigate to **Products** > **Stream** > **Worker Groups**. Select a Worker Group, then go to **Data** > **Destinations**. Then select **Webhook**. Next, select **Add Destination**.
...
3. In the **Authentication** tab, select an **Authentication type**. 
    ...
    - You can select **Auth token** and provide the 32-character **Authentication token** you got from BigPanda after creating the App Key in the **Token** field. 

    - Alternatively, select **Auth token (text secret)** to expose the **Secret** drop-down, in which you can select a [stored secret](securing-secrets) that references the Access Token. A **Create** link is available to store a new, reusable secret.

4. Select **Save**, then **Commit & Deploy**. You are now ready to test your Webhook Destination's communication with Big Panda.
...
HTTP payloads sent to the BigPanda Alerts API must satisfy rules that are beyond the scope of this topic. For details, see the BigPanda documentation about [Parameters](https://docs.bigpanda.io/en/parameters#primary-and-secondary-properties) and [Troubleshooting an Integration](https://docs.bigpanda.io/docs/troubleshooting-integrations).
...
For more information on this functionality, see the BigPanda pages [Deduplication](https://docs.bigpanda.io/en/alert-intelligence#event-deduplication-86895) and [Incident Identifier](https://docs.bigpanda.io/docs/incident_identifier).
