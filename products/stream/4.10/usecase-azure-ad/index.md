...
Cribl Stream supports setting up SSO using OIDC to provide user authentication (login/password)
...
1. Open the [Microsoft Entra ID portal](https://portal.azure.com/) and log in.
...
1. In **Redirect URI (optional)**, select **Web** and provide the appropriate callback URL for your own Cribl Stream Leader instance, for example: `https://yourDomain.com:9000/api/v1/auth/authorization-code/callback`
...
1. In Cribl Stream, navigate to **Settings** > **Global**.
2. In **Access Management**, select **Authentication**.
3. From the **Type** dropdown, choose `OpenID Connect`.
4. In **Provider name**, enter an arbitrary identifier for this integration.
5. In the **Audience (Relying Party ID)** field, enter your Cribl Stream Leader's base URL. Do **not** append a trailing slash.
...
7. If your Cribl Stream is in Enterprise Distributed mode:
...
8. Change the **Filter type** to `User info filter`. 
...
1. In Cribl Stream, go to **Global Settings**.
1. Under **Access Management**, select **Authentication**.
1. Under **Role Mapping**, under **Mapping**, paste the group's object ID.
