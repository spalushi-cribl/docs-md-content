...
5. In the **Assignments** pane, configure the access granted to Okta users:
   ...
   - To grant access to Cribl Stream to specific Okta groups, select **Limit access to selected groups**.
...
1. In the **Audience (Relying Party ID)** field, enter your Cribl Stream UI base URL.
...
| Cribl Stream field | Okta URL |
| ---------------- | -------- |
| **Authentication URL** | `authentication`
| **Token URL** | `token`
| **User info URL** | `userinfo`. Fill it in if you configured Okta to use groups.
| **Logout URL** | `logout`. Fill it in if you want **Account** > **Log out** in Cribl Stream to log the user out globally.<br> This means that when a user selects the **Accounts** > **Log out** link in Cribl Stream, they are logged out of both Cribl Stream and Okta.
...
4. In the **Groups claim expression**, enter an expression field that matches the groups you want passed to Cribl Stream.
