...
##  Configure Keys with the CLI  {#cli}
...
Keys are added and listed using the `keys` [command](cli-keys):
...
## Configure Keys with the UI {#configuring-keys-with-ui}

In a Single-instance deployment, you can access the key management interface through **Settings** > **Security > Encryption Keys**. In a Distributed deployment, for each Group, select **Worker Groups** > `<group‑name>` > **Group Settings** > **Security** > **Encryption Keys**.

The **Encryption Keys** page lists existing keys, with sortable columns.

![List or add keys through Cribl Stream](encryption-keys-2.7e812d832d.png)
{border="true"}

Select **Add Key** to display the creation modal shown below.
