...
You can use certificates to authenticate Cribl Stream to external data senders and receivers. In the Cribl Stream UI, you configure the certificate and its associated private key differently depending on your deployment type:
...
- For Distributed deployments, navigate to a Worker Group and select **Worker Group Settings** > **Security** > **Certificates** ([Edge](/edge/setting-up-leader-and-edge-nodes/), [Stream](/stream/deploy-distributed/)). Next, select **Add Certificate**.
...
1. Supply a **Passphrase** and/or **CA certificate**, if required by your integration partner's certificate.
1. Select **Save**.
1. Open the configuration modal for the Source or Destination.
1. Select **TLS Settings (Client Side)** or **TLS Settings (Server Side)**, depending on the integration.
1. Toggle **Enabled** on.
1. In the **Certificate name** drop-down, select a certificate that you've preconfigured for this integration. (This will auto-populate the corresponding fields here.)
If you're creating a new certificate, instead select the adjacent **Create** button.
1. Select **Save**.
...
>   *  Review regularly review and renew certificates. 
...
If your organization uses SAML Single Sign-On (SSO), you'll need a modified CSP policy to accommodate the dynamic JavaScript and form submissions required for SAML authentication:
