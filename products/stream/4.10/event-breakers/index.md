...
Event Breakers help break incoming streams of data into discrete events. To see how Event Breakers interact with the rest of Cribl Stream's data flow, see [Event Processing Order](event-processing-order).

## Access Event Breakers {#intro}

To access the Event Breakers management interface navigate to **Products** > **Cribl Stream** > **Worker Groups**. Select a Worker Group, then go to **Processing** > **Knowledge** > **Event Breaker Rules**. On the resulting **Event Breaker Rules** page, you can edit, add, delete, search, and tag Event Breaker rules and rulesets, as necessary.
...
Event Breakers are directly accessible only on Sources that require incoming events to be broken into a better-defined format. (Check individual Cribl Stream Sources' documentation for Event Breaker support.) Also, Event Breakers are currently not supported in [Packs](packs). 
...
![Rules within an example (AWS) ruleset that ships with Cribl Stream](breaker-rules-4.0.ed106b6cbf.png)
{border="true"}
...
On the **Event Breaker Rules** page (see screenshot [above](#intro)), select **Add Ruleset** to create a new Event Breaker ruleset. Select **Add Rule** within a ruleset to add a new Event Breaker.
...
- For Sources that support multiplexed data (like Splunk S2S protocol), Cribl Streamdemultiplexes based on channels. Each channel (typically representing a unique source+sourcetype+host combination in Splunk Sources) will have its own event breaker applied consistently throughout its lifecycle.
...
You can use the JSON Array to extract events from an array in a JSON document (such as an Amazon CloudTrail file). If a Source supports Event Breakers (such as AWS Sources), it is much more efficient to [unroll JSON](json-unroll-function) in a Breaker, versus in a Pipeline Function.
   ...
   * **JSON Extract Fields**: Toggle on to auto-extract fields from JSON events. If disabled, only `_raw` and `time` will be defined on extracted events.
   * **Timestamp Format**: If the **JSON Extract Fields** toggle is off, you **must** set this to **Autotimestamp** or **Current Time**. If the **JSON Extract Fields** toggle is on, you can select any option here.
...
2. In the resulting modal, select **Advanced Mode** to open the JSON editor. 

3. You can now modify the ruleset directly in JSON, if you choose.

4. Select **Export**, select a destination path, and name the file.
...
3. Select **Import**, and choose the file you want.

4. Select **OK** to get back to the **New Ruleset** modal. 
...
You can use [Cribl Edge](/edge/) to apply and author Event Breakers while exploring files – even files that you have not saved as sample files. This option includes remote files that are viewable only from Edge. See [Exploring Cribl Edge on Linux](/edge/explore-edge) or [Exploring Cribl Edge on Windows](/edge/explore-edge-win).
