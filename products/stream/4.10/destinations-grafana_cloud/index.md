...
Obtain an API key, setting its Role to `MetricsPublisher`. If you want Cribl Stream or an external KMS to manage the API key, [configure](securing-kms-config) a key pair that references the API key.
...
## Configure Cribl Stream to Output to Grafana Cloud
