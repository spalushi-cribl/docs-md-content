...
#### Upgrade to v.4.3.x using Leader-Managed Upgrades for Worker Nodes
...
- Before upgrading your Leader to v.4.0 and later, see [Persisting Socket Connections](persisting-socket-connections)
  to prepare the host to keep communications open from Worker Nodes.
...
A slow or unreliable network connection that significantly delays downloading this package will cause the upgrade to fail. The upgrade package size can be around `80` MB.  Ensure adequate network bandwidth and stability for successful upgrades.

### Considerations for Firewalls/Web Proxies

For optimal upgrade performance, it's recommended to configure firewalls to allow access to `cdn.cribl.io`. This enables Workers to download upgrade packages efficiently.

If access to `cdn.cribl.io` is restricted, configure firewalls to immediately deny connection requests. This allows Workers to promptly fall back to the Leader for downloading the upgrade package and avoid timeouts.
