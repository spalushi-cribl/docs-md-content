...
**Lookup file path (.csv, .csv.gz)**: Path to the lookup file. Select an existing file that you've uploaded via Cribl Stream's UI at [Knowledge > Lookups Library](lookups-library), or specify the path. You can reference environment variables via `$`, such as `$CRIBL_HOME/file.csv`.
...
**Use Disk-Based Lookup:** Default: off. Toggle on if the lookup file referenced in the **Lookup file path** is a disk-based lookup. When toggled on, this setting removes the **Match Mode** and **Reload Period (sec)** options, as they do not apply to disk-based lookups.

> Cribl Edge does not support disk-based lookups, only in-memory lookups.
{.box .info}

**Match mode**: Defines the format of the lookup file, and indicates the matching logic that will be performed. Defaults to `Exact`. This option is only available for in-memory lookups and is disabled when **Use Disk-Based Lookup** is selected.

**Reload period (sec)**: For single-instance deployments, changes to lookup files require an application restart unless a reload interval is enabled. Specify how often (in seconds) Cribl Stream should check for updates and reload the file. Use a positive number to enable automatic reloading. Leave the `-1` (default) to disable. This option is only available for in-memory lookups and is disabled when **Use Disk-Based Lookup** is selected.

> In distributed deployments, enabling a **Reload period** can generate conflicts with configuration updates, causing Pipelines to skip executing some Lookup Functions. Cribl recommends that you enable it only for lookup files not managed by Cribl Stream's UI, or lookup files that can be updated by an external process (such as a threat list that you update via a cron job).
>
...
**Lookup fields (.csv)**: Field(s) that should be used to key into the lookup table.
  ...
  * **Corresponding Field Name in Lookup**: The field name as it appears in the lookup file. Defaults to the **Lookup field name in event** value. This input is optional.
  ...
  > For disk-based lookups, the field you reference in the Lookup Function must be indexed, or the lookup will fail. Make sure all lookup key fields are properly indexed before deploying. See [Add Indexes to a Lookup File](/stream/lookups-configure#add-indexes) for more information.
  >
  {.box .info}
...
- [About Lookups](/stream/lookups-about)
- [Configure Lookups](/stream/lookups-configure)
- [Lookups Library](lookups-library)
- [Lookup Function](lookup-function)
- [C.Lookup() Expression](expressions-lookup)
