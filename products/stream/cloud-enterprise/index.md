...
With a Cribl.Cloud Enterprise plan, you have these options and workflow flexibility:

- Configuring and managing multiple [Worker Groups](/stream/worker-groups) and [Fleets](/edge/fleet-management).
- [Notifications](notifications) within Cribl apps, to PagerDuty, and/or to other services via webhook.
- Fine-grained, role-based control of [Member Permissions](permissions#organization) on your Cribl.Cloud Organization and on individual [product resources](permissions#product).
...
> For other Enterprise features, see our [Pricing](https://cribl.io/pricing/) page.
>
{.box .info}
...
- On all Worker Nodes' hosts, firewalls must allow outbound communication on port 443 to the Leader and to `https://cdn.cribl.io`.
This port is also used to bootstrap hybrid Workers from the Leader.
...
- To verify your Leader's region and public URL, open the [Access Details](workspaces#view-access-details) modal.
...
1. Log directly into their UI, then select **Settings** > **Global** > **Distributed Settings**.
  Make sure the **Mode** is set to **Stream: Managed Worker (managed by Leader)** or **Edge: Managed Edge (managed by Leader)** (which might require a restart). 
1. Then select **Leader Settings**, and ensure a consistent entry in the **Address** field.
