...
- Other sensitive attributes throughout Cribl Stream
...
-  Cribl Stream uses this key to encrypt sensitive information in various configuration files.
...
-  Cribl Stream sets and retrieves the `cribl.secret` value via the chosen external KMS provider.
-  The external KMS provider only handles the master encryption key (`cribl.secret`) itself.
-  All other sensitive information (Source/Destination credentials, TLS certificates, database connection strings, etc.) remains in the  Cribl Stream configuration files, encrypted using the externally-managed key.
...
- [Stream Internal](#internal): The only option available without an Enterprise license/plan. With this option, the secrets themselves are configured and maintained in Cribl [Secrets](securing-secrets) section.
...
> ##### External KMS Providers and Worker Groups {{< id `group-ext-kms` >}}
>
...
The **KMS provider** field defaults to `Stream Internal`. With this option, no further configuration here is required (or possible). See [Secrets](securing-secrets) to configure individual secrets.

## HashiCorp Vault {#vault}

Cribl Stream's integration with HashiCorp Vault uses a Vault policy to define the capabilities granted to tokens used for KMS access. This policy ensures tokens have the necessary permissions to interact with Vault paths relevant to KMS functionality.

### Vault Policy Permissions {#policy}

This table outlines the required Vault policy permissions for different access scenarios.

| Path | Capabilities | Description |
|---|---|---|
| `<path to vault secret>` | Create, Update, Read | Grants access to create, update, and read secrets at the specified path. |
| `auth/token/create` | Update | Grants permission to update authentication tokens (only required for Token authentication). |
| `sys/health`  | Read |Allows reading system health information (only needed if enabling health checks). |


> `<path to vault secret>` represents the actual path to the secret within your Vault hierarchy.
{.box .info}

Keep these capabilities in mind as you proceed with the configuration.

Setting the **KMS provider** drop-down to `HashiCorp Vault` exposes the following configuration options:

### KMS Settings

**Vault URL**: Enter the Vault server's URL (for example, `http://localhost:8200`).
...
- **Manual**: If not running on AWS, you can select this option to enter a static set of user-associated IAM credentials directly or by reference. This is useful for Worker Nodes not in an AWS VPC, for example, those running in a private cloud. It prompts you to provide an **Access key** and a **Secret key**.
...
**Enable for Vault Auth**: Toggle on if you want to use your Assume Role credentials to access Vault authentication.
...
**Secret Path**: Enter the path on which the Cribl Stream secret should be stored. For example, `<somePath>/cribl‑secret`.
...
**Enable Health Check**: Whether to perform a health check before migrating secrets data. Defaults to toggled on.
...
**Enable for KMS**: Toggle on if you want to use Assume Role credentials to access the AWS KMS.
