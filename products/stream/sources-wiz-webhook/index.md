...
Cribl Stream supports receiving [Wiz Defend Alert](https://www.wiz.io/platform/wiz-defend) data. Wiz Webhook, an HTTP-based Source, listens on a specific port, captures every HTTP request to that port, and creates a corresponding event that it pushes to its configured Event Breakers.
...
## Configure Cribl Stream to Receive Wiz Defend Alert Data

1. On the top bar, select **Products**, and then select **Cribl Stream**. Under **Worker Groups**, select a Worker Group. Next, you have two options:
     - To configure via [QuickConnect](quickconnect), navigate to **Routing** > **QuickConnect** (Stream). Select **Add Source** and select the Source you want from the list, choosing either **Select Existing** or **Add New**.
     - To configure via the [Routes](routes), select **Data** > **Sources** (Stream). Select the Source you want. Next, select **Add Source**.
2. In the **New Source** modal, configure the following under **General Settings**:
    ...
    - **Port**: Enter the port number to listen on. For Cribl.Cloud, use the `20000` – `20010` [port range](ports#cloud-ports).
3. Under **Authentication**, select **Add Token** to enter the **Auth tokens**:
     - **Token**: Shared secrets to be provided by any client. Click **Generate** to create a new secret. If empty, permits open access.
     - **Description**: Optionally, enter a description.
     - **Fields**: Fields to add to events referencing this token. Select **Add Field** to create a new field. Each field is a **Name/Value** pair, where **Value** is a JavaScript expression to compute field's value, enclosed in quotes or backticks. (Can evaluate to a constant.) For details, see [Authentication Fields](#fields).
...
**Allowed URI paths**: List of URI paths accepted by this input. Supports wildcards, for example, `/api/v*/hook`. Defaults to `*`, which allows all paths.

**Allowed HTTP methods**: List of HTTP methods accepted by this input. Supports wildcards, for example, `P*, GET`. Defaults to `*`, which allows all methods.
...
The examples in this section demonstrate sending HTTP data into a Raw HTTP Source. The token is set to `myToken42` and included in the `Authorization` header of requests.

### Cribl Endpoint – Single Event

Replace `<myCriblHost>` with your [**Public Ingress address**](workspaces#stream-worker-group-details).

```shell {title="Cribl Endpoint, Single Event Example"}
curl -k 'http://<myCriblHost>:10050/postPath' \
...
Replace `<Your-Org-ID>` with your Cribl.Cloud [**Organization ID**](workspaces#view-workspace-details). Cribl.Cloud makes the `20000` – `20010` [port range](ports#cloud-ports) available for configuring Sources that don't have a default port, like the Raw HTTP Source.

```shell {title="Cribl.Cloud, Single Event Example"}
curl -k 'http://default.main.<Your-Org-ID>.cribl.cloud:20001' \
-H 'Authorization: myToken42' \
