...
After applying this setting, monitor the queue's drain rate. If the queue is draining too slowly, carefully increase the **Drain rate limit (EPS)** while ensuring it doesn’t interfere with live data processing. Avoid setting the limit too high, as this can impact the performance of live streams. If needed, consider optimizing Worker startup connections and managing CPU load to further enhance performance at **Group Settings** > [Worker Processes](settings-group-fleet#processes).
...
These notifications always appear in Cribl Stream's user interface and internal logs. You can also send them to external systems. See [Destination-State Notifications](/stream/notifications#destination) for more information.
