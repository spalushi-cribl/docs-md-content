...
Lookups are data tables that can be used in Cribl Stream to enrich events as they are processed by the [Lookup Function](lookup-function). To access the Lookups library, which provides a management interface for all lookups, navigate to **Products** > **Cribl Stream** > **Worker Groups**. Select a Worker Group, then go to **Processing** > **Knowledge** > **Lookups**. 
...
For large lookup files, you'll need to provide extra memory beyond [basic requirements](/stream/requirements) for Cribl Stream and the OS. To determine how much extra memory to add to a Worker Node/Edge Node for a lookup, use this formula:
