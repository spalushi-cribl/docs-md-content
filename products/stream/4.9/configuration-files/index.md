...
All paths below are relative to `$CRIBL_HOME` in a [Single-instance deployment](/stream/deploy-single-instance), or to `$CRIBL_HOME/groups/<group‑name>/` in a [Distributed deployment](/stream/deploy-distributed).
