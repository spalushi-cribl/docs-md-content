...
Next, in the resulting **Commit Changes** modal, you can verify the diffs in the configuration changes. To reuse your preceding commit message, select the **Use Previous Commit Message** button. Other options here include clearing check boxes for individual files to exclude them from the commit (as shown below), and selecting **Undo** to reverse the changes instead of committing them.
...
[Snippet not found: content/shared/4.9/snippets/_global-commit-and-worker-group-commit.md]
...
You can configure a Leader Node with Git remote push capabilities through the [Cribl Stream CLI](cli-reference#mode-master), or through the Cribl Stream user interface. From Settings, select the **Global** tab, then select **System** > **Git Settings**.
...
![Git Authentication Type settings](git-settings-general.png)
{border="true"}
...
![Git Authentication Type settings](git-settings-remote.png)
{border="true"}
