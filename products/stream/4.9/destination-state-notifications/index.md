...
In Cribl Stream, you can configure Notifications on Destinations. You will receive a
Notification to the set target when one of these conditions triggers:

- [Backpressure activates on the Destination](#backpressure)
- [Persistent Queue Usage surpasses a set threshold](#pq)
- [The Destination is unhealthy](#unhealthy)
...
The threshold for the Notification to trigger is: Cribl Stream detected a blocked or dropped state during ≥ 5% of the trailing **Time window** that you configure in the [Configuring Destination Notifications](#configuring-d).
...
Cribl Stream will generate a `Destination <name> is unhealthy` Notification when the Destination's health has been in red status (as indicated on the  [Monitoring](monitoring) page) over the trailing **Time window** that you configure in [Configuring Destination Notifications](#configuring-d). 
...
## Configure Destination Notifications {#configuring-d}
...
1. Access this Destination's **Notifications** tab by one of the following methods: 
    - Select the **Notifications** button on the Destination page's appropriate row.
    - Reopen the Destination's configuration modal and select its **Notifications** tab.
1. Select **Add Notification** to open the **New Notification** modal shown here.

![Configuring a Destination Notification](notification-dest-tab.png)
{border="true"}

### View Count of Configured Destination Notifications

In the **List** view of a Destination, you can quickly see how many
Notifications are configured. The **Notifications** button will display a badge
showing you the number of configured Notifications for the Destination.

![List view of a configured Source with Notifications](notification-indicator.png)
...
Select **Add Target** to send this Notification to additional targets. You can add multiple targets.
  - Use the resulting **Notification targets** drop-down to select any target you've already configured.
  - Select **Create Target** to configure a new target. 
...
You can enter user-defined fields called *metadata*, which Cribl Stream includes in the notification payload. See [Metadata](notifications#metadata) for more information.