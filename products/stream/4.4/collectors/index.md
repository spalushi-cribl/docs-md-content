...
### Importing a Collector Configuration {#importing-json}

You can create a Collector by importing a JSON file that defines a Collector configuration, entering desired values for any placeholders, and saving the new Collector.

To edit or create a Collector configuration, you need to know that placeholders are defined in the form `<Label|Description>`, where `Label` is the input field label and `Description` is the tooltip text.

When you import a Collector configuration, Cribl Stream opens the **Replace Placeholder Values** modal, prompting you to enter desired values for the fields defined as placeholders. Here's an example where the configuration defines placeholders for fields named `Client ID` and `Client Credentials`: 
