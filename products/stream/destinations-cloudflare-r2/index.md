...
The Cloudflare R2 Destination sends data to Cloudflare R2 object storage using an [S3-compatible API](https://developers.cloudflare.com/r2/api/s3/api/).

> Type: Non-Streaming | TLS Support: Configurable | PQ Support: No
>
{.box .info} 

## Configure a Cloudflare R2 Destination
...
1. In the **New Destination** modal, configure the following under **General Settings**:
   ...
   - **R2 endpoint**: Cloudflare R2 service URL. For example: `https://<ACCOUNT_ID>.r2.cloudflarestorage.com`.
   - **R2 bucket name**: Name of the R2 bucket. This value can be a constant, or a JavaScript expression that will be evaluated only at init time. For example, referencing a Global Variable: `myBucket-${C.vars.myVar}`. Ensure that the bucket already exists, otherwise R2 will generate "bucket does not exist" errors.
      > Event-level variables are not available for JavaScript expressions. This is because the bucket name is evaluated only at Destination initialization. If you want to use event-level variables in file paths specify them in the **Partitioning Expression** field, because this is evaluated for each file.
      {.box .info}
   - **Staging location**: Filesystem location in which to locally buffer files before compressing and sending. Cribl recommends that this location be stable and high-performance.
...
1. Next, you can configure the following **Optional Settings**: 
    - **Key prefix**: A constant or JavaScript expression that is prepended to each object key (file name) when writing data to the bucket. The key prefix is evaluated per file and can use global variables or environment variables, but not event-level fields. This enables logical organization of objects within the bucket, simulating directory structures using forward slashes `/`.
    - **Partitioning expression**: JavaScript expression that defines how files are partitioned and organized. Default is date-based. If blank, Cribl Stream will fall back to the event's `__partition` field value (if present); or otherwise to the root directory of the **Output Location** and  **Staging Location**.
      ...
      {.box .info} 
    - **Compression**: Data compression format used before sending. Defaults to `gzip` (recommended). This setting is not available when **Data format** is set to `Parquet`.
...
1. Optionally, you can adjust the [Authentication](#auth), [Processing](#processing), [Parquet](#parquet-settings), and [Advanced](#advanced-tab) settings outlined in the sections below.
1. Select **Save**, then **Commit & Deploy**.

### Authentication {#auth}

Use the **Authentication Method** drop-down to select either **Auto** or **Secret**:
...
> Cribl Stream will close files when **either** of the `File size limit (MB)` or the 
...
**Remove empty staging dirs**: Toggle this on to delete empty staging directories after moving files. This prevents the proliferation of orphaned empty directories. When enabled, exposes this additional option:
...
**Verify if bucket exists**: Toggle off if you can access files within the bucket, but not the bucket itself. This resolves errors of the form: `error initializing...Bucket does not exist`.
