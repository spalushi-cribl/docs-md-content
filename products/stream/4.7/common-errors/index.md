...
> Some of system-level are listed below with the appropriate action to take.
...
Occurs only on Leader Nodes. Each Worker Group relies on a config helper process that runs on its behalf on a Leader Node, and that process is not currently running. This error is usually triggered by restarting Cribl Stream on the Leader Node, and then trying to access one of these UI features before the Cribl Stream server is fully up:
...
Below are listed general errors that can occur with many Sources.
...
Below are listed general errors that can occur with many Destinations.
...
The CA certificates used by the server's leaf certificate must be trusted by the client. See [Configuring CA certs](securing-and-monitoring#env-vars).
...
The self-signed certificate must be trusted by the peer to which it's presented. See [Configuring CA certs](securing-and-monitoring#env-vars).
