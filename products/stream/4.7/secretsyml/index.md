...
`cribl.secret` is unique per Worker Group and resides in the `$CRIBL_HOME/groups/<group-name>/local/cribl/auth` directory on Leader nodes,
or in `$CRIBL_HOME/local/cribl/auth` for Workers or single-instance deployments.

> Cribl automatically encrypts plaintext secrets in secrets.yml on startup, keeping your data secure.
>
{.box .info}
