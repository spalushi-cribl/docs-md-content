...
TCP load-balanced Destinations can be configured with one or multiple receivers. If one or more receivers go down, Cribl Stream will continue sending data to any healthy receivers. Source PQ is designed to help buffer for occasional backpressure; unlike Destination PQ, its role is not to propagate backpressure upstream.
...
> As long as even one of the Destination's receivers is healthy, Cribl Stream will redirect data to that receiver, 
