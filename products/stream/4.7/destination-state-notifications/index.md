...
Cribl Stream will generate a `Destination <name> is unhealthy` Notification when the Destination's health has been in red status (as indicated on the UI's [Monitoring](monitoring) page) over the trailing **Time window** that you configure in [Configuring Destination Notifications](#configuring-d). 
