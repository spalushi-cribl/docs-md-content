...
```shell
-H <host>          # Leader Node Hostname or IP address.
-p <port>          # Leader Node cluster communications port (defaults to 4200).
...
Updates an on-prem Cribl Stream Leader with a new config in the `instance.yml` file called `cloudWorkspace`.
...
encrypt           # encrypt, args:
  ...
  [-s <criblSecretPath>] # This optional argument allows you to define a custom path to a secret key. If omitted, the path to the default Leader file will be used.
```
