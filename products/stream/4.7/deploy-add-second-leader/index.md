...
To handle unexpected outages in customer-managed (on-prem) distributed deployments, Cribl Stream supports configuring a standby Leader for failover. In this High Availability (HA) scenario, if the primary Leader goes down, Collectors and Collector-based Sources can continue ingesting data without interruption. 
...
- Cribl Stream will recover by switching to the standby Leader.
...
> Prior to Cribl Stream 4.7, it was possible for the primary and standby Leaders to be configured differently, potentially causing issues, especially around authentication and authorization.
> 
{.box .info}
