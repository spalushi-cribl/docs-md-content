...
To sustain higher incoming data volumes and/or increased processing, you can
scale from a single instance up to a multi-instance Distributed deployment. In a
Distributed deployment, a single, central Leader Node manages all Worker
instances and is responsible for tracking and monitoring their activity metrics,
as well as for keeping configurations in sync.
...
- For detailed system requirements and constraints not listed on this page, see [Single-Instance/​Basic Deployment](deploy-single-instance).
...
**Worker Node** – a Cribl Stream instance that runs as a **managed Worker**. The Leader fully manages the configuration of each Worker Node. (By default, the Worker Nodes will poll the Leader for configuration changes every 10 seconds.)
...
Cribl prioritizes S3 downloads for Workers, with automatic fallback and firewall allowlisting tips for optimal performance. See, [Optimized Bundle Delivery from S3](#optimized-S3bundle) for details. 
...
See the [FIPS Mode topic](fips-mode) for system and password requirements, and instructions running in FIPS mode. 

## Installing on Linux

See [Single-Instance Deployment](deploy-single-instance#install), as the installation procedures are identical.

### Setting Capabilities for Cribl Stream
...
2. On the **Groups** tab: For each desired Worker Group, toggle **UI Access** to `On`.
  ...
  ![Worker UI access setting](st-manage-ui-access.6e7074c299.png)
  {border="true"}

  > On Cribl.Cloud, the **Groups** page adds extra columns. You'll see each Group's Worker type (hybrid versus Cloud/Cribl-managed) and, for Cloud Groups, the anticipated ingress rate and Provisioned status. For details, see [Managing Cribl.Cloud Worker Groups](cloud-workers).
  {.box .cloud}
...
4. On the **Manage Workers** page: Click the link for any Worker you want to inspect.
  ...
  At the upper right, you can click **Restart** to restart the Worker, or click the **X** close button to return to the **Manage Workers** page.
...
#### Adding Tags to Workers in Teleport View

You can add tags to a Worker Node you've teleported into. Tags help the Leader map
a Worker to a Worker Group. Note that changing the tags on a Worker can cause
the Leader to map the Worker into a new Group.

1. Teleport into a Worker Node.
1. Navigate to **Worker Settings** then **Distributed Settings**.
1. Enter tags for this Worker Node and select **Save**.

>In Cribl.Cloud, you can only add tags to _hybrid_ Worker Nodes. You must accept the
>confirmation modal, as changing the tags on a Worker can affect the Worker
>Group that the Leader maps it into. 
{.box .cloud}
...
When you configure a node to work as the Leader, switching from a Single-instance to a Distributed mode,
...
When you first start a Worker Node, it will use the default license. As a
result, you might see a log entry warning about a limit on Worker Processes:
...
+`CRIBL_DIST_LEADER_URL=tcp://${CRIBL_DIST_TOKEN:-criblmaster}@masterHostname:4200 ./cribl start`
...
`-H` and `-p` are required parameters. For other options, see the [CLI Reference](cli-reference#mode-worker). Here is an example command:
...
Distributed deployments use Unix domain sockets for inter-process communication
(IPC) between a Leader's distributed processes and services. Multiple Unix
domain sockets must exist (created by their respective process) on a Leader's
disk. One of these domain socket files exists to ensure the metrics services
operates correctly. On the host where the Leader is running, the default
location for Unix domain socket files is the operating system temp directory –
for example, `/tmp`.
...
![Distributed deployment: menu structure](worker-groups-main-pipeline.8cd45ff8f7.png)
{border="true"}
...
![Home page in a single-instance deployment](top-menu-single-instance.def31af910.png)
{border="true"}
...
![Editing the Active Ruleset](mappings-default.0d076420c6.png)
{border="true"}
...
However, if the Leader fails, future scheduled Collection jobs will also fail. So will data collection on the Amazon Kinesis Data Streams, Prometheus Scraper, and all Office 365 Sources – which function as Collectors under the hood.
...
- **UI**: When [configuring](#config-leader-ui) **Leader Settings** on a Leader Node, specify an S3 bucket (format: `s3://${bucket}`) for remote bundle storage in the **S3 Bundle Bucket URL** field.
- **YAML**: In the [`instance.yml` config](instanceyml) file, specify the S3 bucket in `master.configBundles.remoteUrl`.
- **Environment Variable**: Use the `CRIBL_DIST_LEADER_BUNDLE_URL` [environment variable](#environment-variables).

#### Optimized Bundle Delivery from S3 {#optimized-S3bundle}

Cribl prioritizes efficient bundle downloads for Workers. Here's how:

- **Direct from S3**: Cribl prioritizes downloading bundles directly from Amazon S3 (S3) whenever possible. This ensures optimal performance.
- **Private Network Fallback**: Workers on private networks with restricted internet access will attempt to retrieve bundles from a CDN. If blocked, they automatically fallback to downloading directly from the Leader Node.
- **Firewall Whitelisting (Optional)**: For Workers with internet access, consider allowlisting S3 access in your firewalls or proxies (specifically `*.s3.amazonaws.com` due to non-static addresses). This proactive step ensures smooth downloads and avoids potential disruptions.
...
Enter a unique **Name** and (optionally) a **Description** and [**Tags**](settings-group-fleet#workergroup-configuration). You can **Enable teleporting to Workers** now or later (details [here](#worker-access)). 

![Creating a New Group](new-group.png)
{scale="60%" border="true"}

> To use the [Cribl API](/api/) to create a new Worker Group, see the [API Reference](/api-reference/).
>
> Configuring multiple Worker Groups, or configuring more than 10 Worker Processes, requires a Cribl Stream Enterprise or Standard [license](licensing) in an on-prem deployment.
> 
> A Cribl.Cloud deployment with an Enterprise license or plan also allows you to create Cribl-controlled Worker Groups.
...
![Manage Groups page: Options menu](st-manage-groups-page.b2526d55ef.png)
{border="true"}
...
* `CRIBL_DIST_LEADER_BUNDLE_URL` – AWS S3 bucket (format: `s3://${bucket}`) for [remote bundle storage](#remote-bundle).
...
* `CRIBL_DIST_WORKER_PROXY` - Communicate to the Leader Node via a SOCKS proxy. Format: `<socks4|socks5>://<username>:<password>@<host>:<port>`. Only `<host>:<port>` are required. 
   ...
   > See [GitOps Variables](environment-variables#gitops-variables) for a separate list of GitOps-oriented environment variables.
   {.box .success}
