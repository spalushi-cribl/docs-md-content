...
1. From the [Cribl Stream](#environment) top nav, select **Data > Sources**, then select **Collectors > Quote Generator** from the **Data Sources** page's tiles or the **Sources** left nav.

![Quote Generator Collector (Missing an Icon)](st-usecase-create-collector2.9ea153ee79.png)
{border="true"}
...
1. In Cribl Stream, navigate to the [Quote Generator Collector](#collectors) and click **Add New** to open the **Quote Generator > New Collector** modal.
...
8. This will open a dialog from the [Job Inspector](/stream/monitoring#job-inspector) with information about the run, including job status, items returned by the Discover call, and logs. Click the **Discover Results** tab to see data returned from the Collector’s Discover call:
...
4. Now, in Cribl Stream, update the existing Collector to use the auto-generate feature. Add these settings, then click **Save**:   
