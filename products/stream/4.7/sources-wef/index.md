...
**Input ID**: A unique name that identifies this Windows Event Forwarder Source. If you clone this Source, Cribl Stream will add `-CLONE` to the original **Input ID**.
...
> The CA certificate that generated the Cribl Stream server certificate must match the root CA
...
**Verify certificate via OCSP**: If toggled to `Yes`, Cribl Stream will use
...
**Strict validation**: If enabled, Cribl Stream will fail checks on **any** OCSP error.
Otherwise, Cribl Stream will fail checks only when a certificate is revoked, and will ignore
...
Cribl Stream's **Manage Sources** page. These tags aren't added to processed
...
- If **Use bookmarks** is set to `No`, then each time a client *forcibly* renews its subscription (for example, after group policy update), all events matching the query will be sent. Restarting Cribl Stream or restarting the client will not cause all events to be re-sent.
...
**Use bookmarks**: If toggled to `Yes`, Cribl Stream will keep track of which
...
**Compression**: Sets whether Windows clients compress the events sent to Cribl Stream, using the
...
**Socket timeout (seconds)**: How long Cribl Stream should wait before assuming that an inactive socket has timed out. The default `0` value means wait forever. See also [Adjusting Timeouts](#adj-timeouts).
