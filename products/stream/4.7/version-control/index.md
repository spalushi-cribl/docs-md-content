...
To leverage this functionality for deployment-critical files, configure `cribl.yml` using the `bundler.bundleGitIgnoredPatterns` setting. This ensures essential files reach Workers, even if they are excluded from GitOps due to customer-specific `.gitignore` rules.
