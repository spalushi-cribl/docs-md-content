...
- **Azure Cloud**: Select the Azure cloud environment to use `Azure Public Cloud` (default) or `Azure China`.
  - `Azure Public Cloud` sets the authority host to `login.microsoftonline.com` and the endpoint suffix to `core.windows.net`.
  - `Azure China` sets the authority host to `login.chinacloudapi.cn` and the endpoint suffix to `core.chinacloudapi.cn`.
...
When configuring an Azure Collector in Cribl Stream, you must grant the necessary permissions for it to interact with your Azure environment. Cribl recommends creating an Azure service principal with the minimum required permissions using Azure role-based access control (Azure RBAC). This approach ensures Cribl Stream has a dedicated identity with precisely the access it needs, following the principle of least privilege.

The minimum permissions required for Collectors are:

- **Storage Blob Data Reader**
- **Storage Blob Data Owner**: Required if **Include Tags** is enabled, which is the default.

See [Authorization](https://learn.microsoft.com/en-us/rest/api/storageservices/list-blobs?tabs=microsoft-entra-id#authorization) in the official Microsoft documentation for more information.
