...
In High Availability Leader proxy mode, UI/API traffic on port `9000` can be routed to either Leader; the standby transparently proxies requests to the elected primary Leader. For details, see [Active‑Active (Proxy) Mode for HA Leaders](deploy-add-second-leader#ha-proxy).
...
The following port is used by [Cribl Outpost](outpost).

Default Port | Protocol | Purpose | Direction
--- | --- | --- | ---
`9000` | TCP | Cribl Outpost UI. Login is disabled, but the [Health endpoint](monitoring#endpoint) is available. | In
...
| `443` | HTTP/S | OIDC Auth (TLS); and [Cribl Lake Destination](/stream/destinations-cribl-lake) on [hybrid](cloud-enterprise#hybrid) Worker Groups that you manage. | Out |
...
| `9514` | TCP/UDP | [Syslog](sources-syslog) Source. | In |
