...
- Each Destination writes files to its staging directory until the number of files exceeds the configured maximum, or the Destination reaches any of the following limits: **File size limit (MB)**, **Max file idle time (sec)**, or **Max file open time (sec)**.
