...
Cribl.Cloud provides a limited number of mixed-use syslog ports for unencrypted UDP, unencrypted TCP, and encrypted TCP, alongside additional configurable ports for any push-based Source. Specifically, Cribl.Cloud provides the following pre-configured syslog ports.

**Encrypted Ports**:
- TCP 6514: Syslog over TCP + TLS

**Unencrypted Ports**:
- 9514: Syslog over TCP or UDP
- 514: Syslog over TCP or UDP (redirected to 101514)

Port 514 is made available for devices that do not support a change from the default syslog port. Note that the only ports available for UDP delivery of syslog traffic are ports 514 and 9514.

In addition to these pre-configured options, Cribl.Cloud also offers a range of configurable TCP ports from 20000 to 20010 for receiving syslog data.

Hybrid Stream Workers support any ports, so they offer greater port flexibility than Cribl.Cloud.
