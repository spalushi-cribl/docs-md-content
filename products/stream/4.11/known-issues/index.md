...
- [Resolved in 4.11](#resolved-in-411)
...
### Cribl Copilot chatbot appears when disabled {#AI-1626}
...
**Problem**: On Cribl.Cloud Organization and Workspace pages, the Cribl Copilot chatbot appears and functions even when it is disabled in AI Settings. Please be assured that your data cannot be accessed by Cribl Copilot as long as it is disabled. 

**Workaround**: None identified.

### Splunk Load Balanced Destinations might experience intermittent health issues {#CRIBL-31564}

|Reported in|Resolved in|Ticket|
|---|---|---|
|4.11.0|TBD|CRIBL-31564|
{.ki-table}
...
**Workaround**: To force propagate this configuration to your Destination, do the following:
  ...
  1. Repeat steps 1 and 2 for each Splunk Load Balanced Destination if you have more than one.
  1. Commit and deploy your change(s) to update the configuration.

### Copying field names with dot notation adds extra characters {#CRIBL-31398}

|Reported in|Resolved in|Ticket|
|---|---|---|
|4.11.0|TBD|CRIBL-31398|
{.ki-table}
...
### Metrics with `_metric` and `_value` fields are merged into a single key-value pair {#CRIBL-31266}

|Reported in|Resolved in|Ticket|
|---|---|---|
|4.11.0|TBD|CRIBL-31266|
{.ki-table}
...
### OpenTelemetry (OTLP) Metrics in batches do not trigger the Metrics View in Data Preview {#CRIBL-29990}

|Reported in|Resolved in|Ticket|
|---|---|---|
|4.11.0|TBD|CRIBL-29990|
{.ki-table}
...
### Lookup Function fails to return all regex matches {#CRIBL-28489}

|Reported in|Resolved in|Ticket|
|---|---|---|
|4.9.0|TBD|CRIBL-28489|
{.ki-table}

**Problem**: The Lookup Function may fail to match all expected values when using Regex MatchMode, especially after switching from Exact MatchMode with Ignore Case enabled. For example, if you enable Ignore Case in Exact MatchMode and then switch to Regex, the Lookup may fail to match the first event or return fewer matches than expected.

This issue occurs because configuration settings from the previous match type (such as Ignore Case) are not fully cleared when switching from Exact to Regex.

**Workaround**: Clear all previous match settings before switching to Regex MatchMode.
...
**Problem**: For deployments configured for [Leader high availability/failover](/stream/deploy-add-second-leader/), creating diagnostic bundles via the UI works normally but creating them via the CLI does not. In Cribl Stream 4.4.0 and newer, the operation fails with a `not a git repository` error. In 4.4.0 and older, the diag bundle created contains obsolete data. In both cases the underlying cause is that the process that creates the diag bundle cannot access the `CRIBL_CONF_DIR` environment variable.
...
### Diag bundles might fail to download from teleported Worker Nodes {#CRIBL-13999}
...
**Problem**: If you configure [persistent queueing](persistent-queues) to both enable **Compression** and set a **Max queue size** limit, set that limit optimistically and monitor the results. Due to an error in computing the compression factor, Cribl Stream will not fully use **Max queue size** values below or equal to the volume's available disk space. This can lead to a mostly empty disk, lost data (either blocked or dropped), and/or excessive backpressure.
...
### Metrics with all zero values are not aggregated correctly {#CRIBL-31233}

|Reported in|Resolved in|Ticket|
|---|---|---|
|4.11.0|4.11.1|CRIBL-31233|
{.ki-table}

**Problem**: When using the Aggregate Metrics or Aggregations Function with StatsD-formatted metrics data, Cribl may not produce any output if all the metric values are zero. This happens because the system incorrectly treats zero as if no value is present. In configurations that include a fallback value (such as an alternate field to use if no value is found), the system mistakenly uses the fallback, even though the original value is valid and zero.

This issue also affects the automated configurations generated in Data Preview when selecting metrics to aggregate. The expression Cribl generates includes a fallback, which causes it to treat zero as missing and use a string instead, resulting in missing or invalid aggregation output.
...
JSON file contents. When this option is set, Cribl Stream will first check whether
...
incorrect, Cribl Stream checks for credentials in two additional fallback
locations, as described in the official Google Cloud documentation:
[How Application Default Credentials works](https://cloud.google.com/docs/authentication/application-default-credentials).

### C.Time Functions can crash Worker Nodes with old timestamps {#CRIBL-26719}
...
### When Source PQ is enabled, disk space fills to near capacity and Cribl Stream issues an error {#CRIBL-26478}
...
### Cribl Stream displays a permissions-related error in some areas of the user interface {#CRIBL-24310}
...
**Problem**: The **Limits** [settings](group-settings#limits) at Worker Group level that on-prem deployments use are not available in Cribl.Cloud. Because the Redis settings (including **Reuse Redis connections**) are within that **Limits** UI, Worker Nodes in Cribl.Cloud do not have access to those settings.
...
### Leader incorrectly indicates version mismatch for Worker Nodes {#CRIBL-23657}
...
**Workaround**: Manually [restart](cli-restart) Cribl Stream after upgrading with `CRIBL_VOLUME_DIR` set.
...
1. Open the Developer Tools network tab in your browser and copy the authorization bearer token from one of your requests. Alternatively, you can follow the docs steps [here](/api/#criblcloud) to create a Cribl.Cloud API credential and obtain a token. Copy the token.
...
### Rapid logging causes Worker Node memory spikes and prevents logging {#CRIBL-20607}
...
### Worker Nodes cannot be upgraded in Leader-managed upgrades {#CRIBL-20086}
...
### File Monitor Source causes memory leaks and Worker Node crashes {#CRIBL-19154}
...
### Credential encryption errors preventing user access to Cribl Stream Sources and Destinations {#SAAS-4823}
...
### Worker Nodes don't update from Cribl Stream Leader {#CRIBL-17792}
...
**Workaround**: On the Source whose events are being interrupted, configure an [Always On](persistent-queues-sources) persistent queue. This buffering will cause the Destination to drop fewer events. Note that persistent queuing will not engage until Cribl Stream has exhausted all attempts to send the data to the Kafka receiver.
...
### Worker Nodes integrated with `systemd/initd` can fail upon reconfig  {#CRIBL-17264}
...
### On-prem Worker Nodes automatically upgrade, ignoring Leader settings {#CRIBL-15367}
...
### Worker Nodes show incorrect configurations after upgrade or commit/deploy from Leader {#CRIBL-13863-CRIBL-15507}
...
### Changes to a Lookup table on the Leader don't always propagate to Worker Nodes {#CRIBL-14299}
...
### Teleported Worker Nodes don't display Distributed Settings or Diagnostics Settings {#CRIBL-13868}

|Reported in|Resolved in|Ticket|
|---|---|---|
|4.0.0|4.0.1|CRIBL-13868|

**Problem**: When displayed via remote access from the Leader ("teleporting"), a Worker's/Edge Node's **Settings** UI omits the **Distributed Settings** and **Diagnostics** left-nav links. This blocks remote access to features like [configuring TLS communications](securing-import-certs) between Worker Node and Leader.

**Workaround**: Access the Worker's UI directly on its host's port 9000 (or other configured port). As a precondition, you might need to undo any [Disable UI Access](securing-communications#ui-access) setting on the parent Worker Group.
...
### Sorting by column on Manage > Worker Node page results in blank list {#CRIBL-13505}
