...
**Path**: Path, within the bucket, from which to collect data. Templating is supported (e.g., `/myDir/${host}/${year}/${month}/`). Time-based tokens are also supported, e.g., `myOtherDir/${_time:%Y}/${_time:%m}/${_time:%d}/`. This value can be a constant or a JavaScript expression. More on templates and Filters [here](collectors-schedule-run#tokens).
