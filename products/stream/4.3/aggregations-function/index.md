...
**Flush on stream close**: If set to `Yes` (the default), aggregations will flush when an input stream is closed. If set to `No`, the [Time Window Settings](#tws) will control flush behavior; this can be preferable in cases like the following: 
...
- `histogram(expr:FieldExpression, buckets: number[])`. Returns the average of the values of the parameter and generates a field with the same name as the aggregate output field, suffixed with `_data`.

  - `buckets` must contain at least one numeric value.

  The `_data` field in the output contains three pieces of information:

  - `_sum` – the sum of all values of the parameter.

  - `_count` – the number of values seen.
  ...
  For example:
    ...
    `histogram (request_duration, [50, 100, 500])`
...
- `list(expr:FieldExpression[, max:number, excludeNulls: boolean = true])`: Returns a list of values of the parameter.
...
- `mode(expr: FieldExpression[, excludeNulls: boolean = true])`: Returns the single most frequently encountered value of the `<expr>` parameter.
...
- `summary(expr:FieldExpression)[, quantiles: number[]])`. Returns the average of the values of the parameter and generates a field with the same name as the aggregate output field, suffixed with `_data`. 
  ...
  - `_quantiles` – an object whose keys are the quantiles defined in the function, and whose values are the quantile value across the values of the parameter. For example, a quantile key `0.5` and value `500` would indicate that the 50th percentile (or median) of the values seen was 500.

  For example:

    `summary (request_duration, [0.1, 0.5, 0.95])`

- `sumsq(expr:FieldExpression)`: Returns the sum of squares of the values of the parameter.

- `top(expr: FieldExpression, count: number[, excludeNulls: boolean = true])`: Returns the most frequently encountered values of the  `<expr>` parameter, up to  `<count>` number of results.
