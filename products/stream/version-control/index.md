...
> You can also use the Cribl API to automate commit and deploy commands. Read [Commit and Deploy Changes](/cribl-as-code/commit-deploy/) in the API documentation for more information.
>
{.box .info}
...
Distributed mode includes Worker Group-specific **Commit** and **Deploy** controls. These controls function differently from the global **Commit** button.

The global **Commit** button:

- Commits configurations for *all* Worker Groups and for the Leader Node.
- Does not deploy configurations.

The Worker Group **Commit** and **Deploy** buttons:

- Commit and deploy configurations for the currently selected Worker Group only.
