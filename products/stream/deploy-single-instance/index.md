...
![](sources-destinations-june-2025.png)
{border="false"}

## Install on Linux {#install}

> For system requirements, see [OS and System Requirements](requirements). To launch Cribl Stream in compliance with secure Federal Information Processing Standards (FIPS), see [FIPS Mode](fips-mode).
>
{.box .info}
...
You can run an Edge Node with a Leader that is managing Cribl Stream, or an Edge Node and a Worker Node on the same host. For details, see [Installing Cribl Edge and Cribl Stream on the Same Host](/edge/deploy-linux#collocated).
