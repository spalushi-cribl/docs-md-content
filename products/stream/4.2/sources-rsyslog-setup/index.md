...
1. Commit, then deploy, this configuration change. 
  Now we're listening for the syslog protocol, on its `514` UDP incoming port. (The port would differ for a specific syslog source. E.g., an Cisco ESXi firewall would send to TCP port `1517`, its default destination.)
