...
With the **Enabled** slider on (the default), Cribl Stream will collect events from host’s defined firewall rules.
...
With the **Enabled** slider on (the default), Cribl Stream will create events from the list of listening ports and their associated process identifier (pid).
...
With the **Enabled** slider on (the default), Cribl Stream will collect entries from currently logged-in users.
...
With the **Enabled** slider on (the default), Cribl Stream will create events for each configured service (e.g. `systemd` and `initd`) along with their enabled and running status.
