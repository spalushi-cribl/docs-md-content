...
See [Working with a Tracking Column](collectors-database#tracking-column) to learn how to craft a Collector query for state tracking. 

**Enabled**: Slide to `Yes` if you want to use a **tracking column** (also known as a **rising column**) when running scheduled jobs.

**Tracking Column**: A numeric column whose values increase monotonically. The Collector will track the column's greatest seen value.

> State tracking is available only for Database Collectors.
> 
> Cribl recommends that when you enable state tracking on a Database Collector, you configure the schedule to allow only one job to run at a time.
> 
> This prevents one run from starting before another has finished, thus avoiding duplicate records being collected and/or state being tracked incorrectly.
>
{.box .success}
