...
Cribl Stream supports SSO/OIDC user authentication (login/password) and authorization (user's group membership, which you can map to Cribl [Roles](#role-mapping)). Using OIDC will change the default `Log in` button on the login page to a button labeled `Log in with OIDC` which redirects to the configured Identity Provider (IDP).
...
## Begin Configuring OIDC Auth in Cribl Stream {#config-ls}

> The only OAuth 2.0 flow that Cribl Stream supports is the [Authorization Code Grant](https://developer.okta.com/blog/2018/04/10/oauth-authorization-code-grant-type) flow.
> 
...
## Finish Configuring OIDC Auth in Cribl Stream {#finish-in-cribl}
...
- **Allow login as Local User**: Toggle to `Yes` to also users to log in using Cribl Stream's local authentication. This enables an extra button called `Log in as Local User` on the Cribl Stream login page. (This option ensures fallback access for local users if SSO/OpenID authentication fails.)
