...
7. {{< id `worker-ui-access` >}} Optional but recommended: From Cribl Stream's top nav, click **Manage** > **Groups**. Then enable the **UI access** slider for your Group(s). This way, you will be able to click through from the Leader's **Manage** > **Workers** tab to view and manage each Worker's UI, distinguished by a [purple border](#teleport-border). (This option handles authentication for you, so you don't need to manage or enter the Workers' credentials.)
...
![UI for multiple Worker Groups](st-manage-ui-access.d4eb70bb8f.png)
{border="true"}
