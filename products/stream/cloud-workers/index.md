...
Using Cribl.Cloud's in-app controls, you can provision Cribl-managed Worker Nodes in
Cribl.Cloud to match the throughput capacity you need. With [certain
plan/license tiers](https://cribl.io/pricing/), you have the option to add and
configure multiple Cribl-managed and/or customer-managed (hybrid) Groups.

## Cloud Worker Group Providers {#providers}

  > Cribl.Cloud Government Worker Groups are supported exclusively on AWS US
  > East/West Regions. Microsoft Azure is not supported.
{.box .cloud}
...
| --------------------- | ---------------- | -------------------- | -------------------- |
| Asia Pacific (Singapore) | `ap-southeast-1` | Southeast Asia |   | `southeastasia`      |
...
| Europe (Zurich)       | `eu-central-2`   | Switzerland North    | `switzerlandnorth`   |
...
Cribl will draw down your Cribl.Cloud credits to cover the base cost of
maintaining the infrastructure, as well as your normal cost for ingested data.
When you create a Workspace, Cribl provisions Workers for you in the `default`
Worker Group, but you can deprovision the Worker Group to control infrastructure costs.

You can do so either when creating the Worker Group, or at any later time when
you are ingesting no data. With this setting, the Worker Group will go dormant,
and your base infrastructure cost for this Worker Group will be `0`. In this
state, the Worker Group will contain no Worker Nodes and will ingest no data.

#### Deprovision a Worker Group

To deprovision the Worker Group:

1. Select your Worker Group from the **Worker Groups** menu.
1. Select **Worker Group Settings**.
1. On the **General Settings** tab, select **Overview**.
1. Uncheck the **Provision infrastructure** option.
1. Select **Save** to confirm, and agree to the confirmation modal with
   **Deprovision**.

![Deprovision a Worker Group from Worker Group Settings](worker-group-provision-infra.png)
{border="true" scale="75%"}

> When you deprovision a Worker Group, Cribl Stream retains the configuration so you
> can reprovision it if needed. However, any persistent queue data on the Worker
> Nodes will be permanently deleted.
>
{.box .warning}
