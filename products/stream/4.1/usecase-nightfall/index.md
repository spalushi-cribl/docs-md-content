...
## Configuring the Detection Engine {#configuring}
...
## Installing the Nightfall Pack {#installing}

1. From Cribl Stream's top nav, select **Processing** > **Packs**.

    ![Selecting Packs](st-nightfall-integ-01.2aae6e1636.png)
    {border="true"}
1. Click **Add Pack**, then select **Add from Dispensary** to open the [Packs Dispensary](https://packs.cribl.io/) drawer.

    ![Adding a new Pack from the Dispensary](st-nightfall-integ-02.117b56c18f.png)
    {border="true"}
1. Navigate to the **DLP by Nighfall AI** Pack's tile.
...
## Configuring the Nightfall Pack {#pack-config}

1. From the **Manage Packs** page, click the new Pack. This opens the Pack's **Routes** tab.
1. In the **Pipeline** column, click `Nightfall`.
1. From the resulting Nightfall Pipeline configuration, expand the **DLP Scanner with Nightfall AI** Function.

    ![Selecting the Nightfall Pipeline](st-nightfall-integ-04.5c9ed75071.png)
    {border="true"}
1. In the **Nightfall API Key** field, enter the API key you created in the Nightfall Dashboard.
1. Set the **Scan with a Policy or Multiple Detection Rules** drop-down to match your [detection engine](#configuring) configuration.
    ...
    ![Scanning with a Policy](st-nightfall-integ-05.8da3fd3ea3.png)
    {border="true"}
1. (Optionally:) Label sensitive events. If you toggle **Label Log Lines with nightfall_violations** to `Yes`, each event where Nightfall has detected sensitive information will have an added `nightfall_violations: true` field.
...
![Disabled steps](st-nightfall-integ-06.107b8a14db.png)
{border="true"}
...
![Enabling log batching](st-nightfall-integ-07.5058cc89aa.png)
{border="true"}
