...
The Cribl HTTP Destination is available only in [distributed deployments](/stream/deploy-distributed). It enables Worker Nodes to send data to peer Nodes, as long as all Nodes are connected to the same Leader.

You'll find the Cribl HTTP Destination/[Source](sources-cribl-http) pair especially valuable in a [hybrid Cloud deployment](/stream/deploy-cloud#hybrid), where it ensures that you're billed for ingress only once – when Cribl first receives the data. All data subsequently transferred to other Workers via the Cribl HTTP Destination/Source pair is not charged. However, the Cribl HTTP Destination/Source pair can similarly reduce your metered data ingress in other scenarios, such as on-prem Edge to on-prem Stream. 

In [single‑instance mode](/stream/deploy-single-instance), or for testing, you can substitute the [Webhook](destinations-webhook) Destination. (However, this substitution will not facilitate sending [all internal fields](#internal-fields), as described below.)

> Type: Streaming | TLS Support: Configurable | PQ Support: Yes 
>
{.box .info}

You might choose this Destination over the [Cribl TCP](destinations-cribl-tcp) Destination in certain circumstances, such as when a firewall or proxy blocks raw TCP egress.
...
- To configure the Leader Address via the [instance.yml](instanceyml) file, the `host` values on the connecting Worker Nodes must be identical. In this example, both Worker Nodes must point to `cribl-leader`:
...
- Cribl 3.5.4 was a breakpoint in Cribl HTTP Leader/Worker communications. Worker Nodes running the Cribl HTTP Destination on Cribl Stream 3.5.4 and later can receive data only from Worker Nodes running v.3.5.4 and later. Worker Nodes running the Cribl HTTP Destination on Cribl Stream 3.5.3 and earlier can receive data only from Worker Nodes running v.3.5.3 and earlier.
...
**Load balancing**: Set to `No` by default. When toggled to `Yes`, see [Load Balancing Settings](#lb) below.
...
**Exclude Current Host IPs**: This slider determines whether to exclude all IPs of the current host from the list of any resolved hostnames. Defaults to `No`. With the default `No` setting, if you notice that Cribl Stream is not sending data to all possible IP addresses, enable **Advanced Settings** > **Round-robin DNS**.
