...
> ##### Troubleshoot Root Access or SSL Errors {{< id `sudo` >}}
>
...
> To use these options, you must have the `admin` [Role](roles) on a [Distributed deployment](deploy-distributed)'s Leader Node, with a [certain license tier](https://cribl.io/pricing/). You must also create the Worker Groups before using the following instructions to add or reassign Workers to them.
>
{.box .success}

### Add New Worker Node {#add-bootstrap}

1. In the Cribl Stream sidebar, select **Workers**.

2. On the **Workers** page, select **Add/Update Worker Node**. 

3. Select **Linux** > **Add** from the menu.

4. In the **Add Linux Worker** modal, the **Install package location** defaults to `Cribl CDN`. If desired, change this to `Download URL`. (For details about this option, see [Add Download URL](#url).)

5. As needed, correct the target **Group**, **User**, and **User Group**, as well as the **Leader or Outpost hostname/IP** (URI).

6. Copy the resulting script to your clipboard and select **Done** to close the modal.

8. Paste the script onto your Worker Node's command line and execute it.
    - As needed (see the note [above](#sudo)), prepend `sudo` to the generated **Script** field's contents, and/or append the `-k` flag.

### Update Existing Worker Node {#update-existing}
...
3. Select **Linux** > **Update** from the menu.

1. In the **Update Configuration on Linux Workers** modal, select the new target **Group** for this Worker Node.

1. As needed, correct or change the **Leader or Outpost hostname/IP** and/or **Leader port number**.
...
1. Copy the resulting script to your clipboard and select **Done** to close the modal.

1. Paste the script onto your Worker Node's command line and execute it.
    - As needed (see the note [above](#sudo)), prepend `sudo` to the generated **Script** field's contents, and/or append the `-k` flag.
...
--- | ---| ---
`token` | optional | Leader Node’s shared secret (`authToken`). By default, this is set to a random string. You can find this secret in the Leader Node's  **Distributed Settings** section.</br>In versions older than 4.5.0, the default token was `criblmaster`. This token is still in use in instances upgraded from an earlier version.
...
To bootstrap Workers in an internet-disconnected (airgapped) environment, you can separate downloading the installation package from Cribl's repository versus running the `curl` command. Here's an example:
...
### Tag to Assign Workers to Worker Groups {#map-and-tag}

Cribl Stream uses [Mapping Rulesets](mapping-workers-to-worker-groups) to map Workers to Worker Groups. When you create a Worker from a bootstrap script, you can take advantage of Mapping Rulesets to specify which Worker Group you want the newly-created Worker to join. This is done by adding tags to the download URL, in the form `&tag=tag1&tag=tag2`.
