...
![Collector configuration for basic HTTP GET](rest-ex-1b-4.9.png)
{border="true"}
...
![Event Breaker Configuration (Pagination via URL Attribute)](rest-ex-2a-4101.png)
{scale="30%" border="true"}

![Collector configuration for HTTP GET, paginated via URL Attribute](rest-ex-2b-4.9.png)
{border="true"}
...
![Event Breaker Configuration (Pagination via Response Body Attribute)](rest-ex-3-0-breaker-config-4101.png)
{scale="30%" border="true"}
...
![Collector configuration for HTTP GET, paginated via Response Body Attribute](rest-ex-3a-4.9.png)
{border="true"}
...
You need [special syntax](filter-and-transform-data) to reference the attributes because they have dots in the name: `__e['attribute.name.with.dots']`.
...
![Collect URL](nested-response-body-ex3-4.9.png)
{border="true"}
...
![Collect parameters](nested-response-body-ex4-4.9.png)
{border="true"}
...
![Collect headers](nested-response-body-ex-headers-4.9.png)
{border="true"}
...
![Collect POST body and Last-page expression](nested-response-body-ex5-4.9.png)
{border="true"}
...
![Collector configuration for HTTP GET, paginated via Response Header URL](rest-ex-4a-4.9.png)
{border="true"}
...
![Event Breaker Configuration (Discover and Collect with Login Authentication)](rest-ex-5a-4101.png)
{scale="40%" border="true"}

![Collector configuration for HTTP Discover and Collect with Login authentication](rest-ex-5b-4101.png)
{border="true"}
...
![Collector configuration for Discovery via Item List](rest-ex-6a-4.9.png)
{border="true"}

![Fields configuration](rest-ex-6b-4101.png)
{border="true"}
...
![Collector configuration for JSON Response Discovery](rest-ex-7a-4.9.png)
{border="true"}
...
1. On the **Worker Groups** submenu, select **Processing**, then **Knowledge**, then **Event Breaker Ruleset**.
...
![Collector Config - Collector Settings](usecase-rest-state-tracking-conf1-4.9.png)
{border="true"}

![Collector Config - Event Breakers](usecase-rest-state-tracking-conf2-4101.png)
{border="true"}
...
3. Expand the **State Tracking** section, then set **Enabled** on. We can use the default values for **State update expression** and **State merge expression**. For more information, see [Understanding State Expression Fields](collectors-rest#state-tracking-expression-fields).
...
![First Run Results](usecase-rest-state-tracking-results1-4.9.png)
{border="true"}
...
![Second Run Results](usecase-rest-state-tracking-results2-4.9.png)
{border="true"}

## 11. State Tracking by Multiple Discovered Results

This example demonstrates tracking state for multiple discovered results during a REST Collector run. Here's a detailed breakdown of the process:

1. During the Discovery phase, the REST API returns a list of identifiers, such as `ids: [1, 2, 3, 4]`. These identifiers represent unique data sources. When the **Discover data field** is set to `items`, the Collector extracts each identifier (`id`) from the API response and passes them to the collection phase as a [`__collectible`](collectors-rest#internal-fields). Here's an example API response:
...
2. During the Collection phase with state tracking enabled, the **State update expression** tracks the latest time for each discovered identifier individually. This expression updates each identifier's state with the most recent timestamp only if `__timestampExtracted` is not `false`. It saves the collectible ID to the state and determines the latest time by comparing the existing `latestTime` with the current time. The higher value is stored as the `latestTime`.

    ```
    __timestampExtracted !== false && {
      id: __collectible.id, // Identifier
      latestTime:
        (state[__collectible.id]?.latestTime || 0) > _time
          ? state[__collectible.id].latestTime
          : _time,
    }
    ```

3. The **State merge expression** combines the state from multiple tasks, ensuring that the state for each identifier is correctly merged. The following expression merges state information from multiple tasks. It assigns a new property to the previous state object using the `newState.id` as the key. The value of this property is determined by comparing the `latestTime` values in the previous and new states for the same ID. The state with the higher `latestTime` is included in the merged state.

    ```
    Object.assign({}, prevState, {
      [newState.id]: // Identifier
        (prevState[newState.id]?.latestTime || 0) > newState.latestTime
