...
**S3** is a non-streaming Destination type. Cribl Stream does **not** have to run on AWS to deliver data to S3. 

> Type: Non-Streaming | TLS Support: Yes | PQ Support: No
>
{.box .info} 

Stores that are S3-compatible will also work with this Destination type. For example, the S3 Destination can be adapted to send data to Snowflake, for which Cribl Stream currently has no preconfigured Destination. 
