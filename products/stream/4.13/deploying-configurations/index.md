...
Cribl Stream configurations are deployed after the [bundle](config-bundle-management) containing them is saved and committed.
*Deploying* here means **propagating updated configurations to Worker Nodes.**

The typical workflow for deploying Cribl Stream configurations is the following:

1. Configure.
2. Save.
3. [Commit (and optionally push)](config-bundle-management).
4. Deploy.
...
>
> When a Worker Node pulls its first configuration, the admin password will be randomized, unless specifically changed. This means that users won't be able to log in on the Worker Node with default credentials. For details, see [Set Worker/Edge Node Passwords](authentication#worker_pwd).
>
{.box .warning}
...
For other options, see [About Lookups](lookups-about).
