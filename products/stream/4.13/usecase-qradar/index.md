...
* Incoming data must adhere to a predefined schema, based on IBM LEEF format. You can't reformat events, nor reduce the size of an event, without breaking the correlation rules within the QRadar Event Processor.
...
> Use sample data to validate the reduction methods described below. Don't apply these methods to your production SIEM until you are confident they won't interfere with the usability of your system. 
>
{.box .warning}
...
3. [Add the Regex Extract Function](usecase-regex-filtering) to your Pipeline to create fields for `EventCode` and `Logon Type`. These fields will be the foundation for a Filter that drops only Windows events of ID 4624, logon type 3.
  ...
  ![Regex Function on Pipeline](st-usecase-qradar-regex-4101.png)
  {border="true"}
...
4. [Add the Drop Function](stream/drop-function) to your Pipeline using the Filter below. 
  ...
  ![Drop Function](st-usecase-qradar-filter-4101.png)
  {border="true"}
