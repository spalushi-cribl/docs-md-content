...
- [Parquet schemas](#parquet-schemas) for writing data from a Cribl Stream Destination to Parquet files, described on this page.
...
When logging is set to `debug`, you will see what schema is associated with the Parquet files that Cribl Stream writes out. On Linux, you can use the Cribl Stream CLI's `parquet` [command](cli-parquet) to view a Parquet file, its metadata, or its schema.

If you turn on a Destination's **Parquet Settings** > **Automatic schema**, Cribl Stream will automatically generate a Parquet schema based on the events of each Parquet file it writes. The automatically-generated schema preserves the exact structure of the ingested events, with no data lost. In Cribl Stream 4.4.2, every field in the generated schema will have a data type of `JSON`, `STRING`, `INT_64`, `UNSIGNED_INT_64`, or `DOUBLE`.
...
To access the Parquet schema library, select **Worker Groups** from the sidebar, and choose a Worker Group.
Then, on the **Worker Groups** submenu, select **Processing**, then **Knowledge**, then **Parquet Schemas**.
...
## Working with Parquet in Cribl Stream {#working-with-parquet}
...
* If the data contains JSON, the JSON must be [stringified](https://www.w3schools.com/js/js_json_stringify.asp). Otherwise, Cribl Stream treats this as a data mismatch, and does not write out the row. For example, this valid (but not stringified) JSON will trigger a data mismatch:  
`{ "name": "test"}`.  
...
Converted types have been superseded by logical types, as described in the [Apache Parquet docs](https://github.com/apache/parquet-format/blob/master/LogicalTypes.md). Cribl Stream can read Parquet files that use converted types, but will write out the same data using corresponding logical types.
...
Cribl Stream does not support reading or writing Parquet files which employ the [deprecated](https://issues.apache.org/jira/browse/PARQUET-323) `INT96` data type.
