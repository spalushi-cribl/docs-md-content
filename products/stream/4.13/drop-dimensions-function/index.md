...
- Cribl Stream does not support aggregation of histogram, summary, or distribution metric types. The Function handles these metrics in special ways:
...
> While `nameExpr` usually references a field (such as `_metric`), it can also be an expression. In such cases, Cribl Stream uses the evaluated result of the expression as the output field name.
>
{.box .info}
