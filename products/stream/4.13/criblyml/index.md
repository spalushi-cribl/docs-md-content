...
auth: # [object] Authentication Settings
  ...
  spEntityId: # [string] Audience (SP entity ID) - Unique URL that identifies this SP (Cribl Stream) instance as the recipient of SAML requests. Must be the base URL of your Cribl Stream server, such as https://yourDomain.com:9000.
  ssoCallbackUrl: # [string] Sign-on callback URL - URL where the SP (Cribl Stream) will consume assertions (that is, where it will receive authentication responses)
  sloCallbackUrl: # [string] Logout callback URL - URL where the SP (Cribl Stream) will receive logout responses
  metadataUrl: # [string] Metadata URL - URL that exposes a description of the SP (Cribl Stream), including endpoints, certificates, and metadata
  spSigningCert: # [object] 
    certificateName: # [string] Request certificate - Certificate the SP (Cribl Stream) should use to sign requests. Create a new certificate, or choose one that you imported as described [in Cribl Docs](https://docs.cribl.io/stream/securing-communications/).
  ...
  idpSigningCert: # [string] Response validation certificate - The certificate that the SP (Cribl Stream) should use to validate signed responses. Provided by the IDP; contains a public key.
  encryptCert: # [object] 
    certificateName: # [string] Response decryption certificate - Certificate the SP (Cribl Stream) should use to decrypt encrypted responses. Create a new certificate, or choose one that you imported as described [in Cribl Docs](https://docs.cribl.io/stream/securing-communications/).
  ...
  groups: # [object] 
    default: # [string] Default role - Default role assigned to groups not explicitly mapped to a role
    mapping: # [object] Mapping - Add a mapping for each external user group that you want to map to one or more Cribl Stream roles. Then enter the group and select the role(s).
