...
* Decide which [API calls](https://learn.microsoft.com/en-us/graph/api/overview) you expect to use, then, for each of those API calls:
  * Configure the corresponding [API permissions](https://learn.microsoft.com/en-us/graph/permissions-reference) for your app.
...
You'll create the Event Breaker in the **Results** section. But first, you must save a sample file, and create a ruleset for the Event Breaker to use.

### Saving a Sample File

On the **REST** page, select **Run** beside the REST Collector you configured. This opens the **Run configuration** modal, in **Preview** mode. Select **Run** to open the **Capture Sample Data** modal.
...
Select **Save as Sample File** and note the file name and location.
...
1. Enter an ID and a Description for the ruleset, then select **Rules** > **Add Rule** to open the **Rules** modal.
...
* Compare the **In** and **Out** panes. You should see output similar to this:
  ...
  ![Audit Logs data broken into individual events](rest-ms-graph-1.5.fdf4ef874a.png)
  {border="true"}
* Select **OK** to return to the **New Ruleset** modal.
* You should see a result similar to the screenshot below. Select **Save**.

![Creating an Event Breaker Ruleset for Audit Logs data](rest-ms-graph-2.eacb7953bf.png)
{border="true"}

### Event Breakers {#event-breakers}

In the Collector's **Results** section:
