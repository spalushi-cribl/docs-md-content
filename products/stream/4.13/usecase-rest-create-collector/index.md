...
![Script Collector Configuration Screen](st-usecase-create-collector.9fff2f69ea.png)
{border="true"}
...
1. For a standalone install of a running Cribl Stream instance, add new directories and files to the `$CRIBL_HOME/local/cribl/collectors` directory (example: `/opt/cribl`). 
...
4.  Connect to the container:  
    ...
    (example: `docker exec -it 544370698fb5 bash`)
...
11.  After Cribl Stream restarts, you must log out and log back in for the new Collector tile to display under [Sources](sources).
...
1. Navigate to **Products** > **Stream** > **Worker Groups**. Select a Worker Group, then go to **Data** > **Sources**. Select the **Collectors** button to display the Collectors tiles.

![Quote Generator Collector (Missing an Icon)](st-usecase-create-collector2.png)
{border="true"}

1. Select the **Quote Generator** > **New Collector** modal.

2. Enter the following in the **Collector Settings** tab, then select **Save**:

   - **Collector ID**: `Hello_World`
   - **Discover Script**: `echo "hello world"`
   - **Collect Script**: `echo "{ message: \"$CRIBL_COLLECT_ARG\" }"`

![Collector Settings Tab](st-usecase-create-collector3.e29f5b8bd2.png)
{border="true"}

3. On the **Manage Quote Generator Collectors** page, select **Run** next to the Collector. 

![Run the Collector](st-usecase-create-collector4.764f52f53c.png)
{border="true"}

4. In the **Run configuration** modal, select **Run** again. Note that you can set the Collector's debug level in the modal's **Advanced Options** section. For details, see [Run Configurations and Shared Settings](/stream/collectors-schedule-run#run-config).

![Run the Collector](st-usecase-create-collector5.6d1daef271.png)
{border="true"}

5. After the Collector runs, the following event will be generated:
...
6. On the **Manage Quote Generator Collectors** page, select the check box next to your Collector, then select **Delete selected Collectors**.

![Delete the Collector](st-usecase-create-collector7.9ca6fa12bf.png)
{border="true"}
...
1. In Cribl Stream, navigate to the [Quote Generator Collector](#collectors) and select **Add New** to open the **Quote Generator > New Collector** modal.

2. Type the following into the **Collector Settings** tab, then select **Save**:

   - **Collector ID**: `firstCollector`
   - **Auto generate names**: Toggle off.
   -  **Names**: `Jane, John, Rover`

   ![New Collector Settings](st-usecase-create-collector8-4101.png)
   {border="true"}

3. On the **Manage Quote Generator Collectors** page select **Run** next to the Collector. 

    ![Run the Collector](st-usecase-create-collector9.665ea68631.png)
    {border="true"}

4. In the **Run configuration** modal:

   - For **Mode**, select **Preview**.
   - For **Log Level**, select **debug**.
   - Select **Run** again.

    ![Run Collector Settings](st-usecase-create-collector10-4101.png)
    {border="true"}
...
7. On the **Manage Quote Generator Collectors** page, select **Latest ad hoc run**.

8. This will open a dialog from the [Job Inspector](/stream/monitoring#job-inspector) with information about the run, including job status, items returned by the Discover call, and logs. Select the **Discover Results** tab to see data returned from the Collector’s Discover call:
...
9. Select the **Logs** tab to view logs from the run. This is where you can locate anything logged by `job.logger`.
...
4. Now, in Cribl Stream, update the existing Collector to use the auto-generate feature. Add these settings, then select **Save**:   

    - **Auto generate names**: Toggle on.    
    -  **Num names:** `10`

    ![Update Existing Collector](st-usecase-create-collector15-4101.png)
    {border="true"}   

5. Run the Collector, notice that 10 Events are now displayed and each username is randomized:

    ![Run Updated Collector](st-usecase-create-collector16.c645dd1426.png)
    {border="true"}  
...
6.  Run the Collector to randomly retrieve the quotes from the list:

    ![Run the Collector](st-usecase-create-collector17.ec542d1ba2.png)
    {border="true"}  

7. In the next few steps, you'll back up the files, by opening a shell window on your local machine and running the indicated commands. 
   
8. List Docker containers by running `docker ps`, as shown here:
...
9.  Run: `docker exec -it 208db8d1d8f7 tar cvzf /tmp/container_source.tgz /opt/cribl/local/cribl/collectors/quote_generator`

10. Run: `docker cp 208db8d1d8f7:/tmp/container_source.tgz`  
...
2.  `cd $CRIBL_INSTALL/groups/default/local/cribl`  
    ...
    `$CRIBL_INSTALL` refers to the directory where Stream is running. Example: `/opt/cribl`
