...
A Notification target allows you to receive alerts using your preferred channel or alert delivery method. Every Notification requires one or more targets.
...
When you create a new Notification, its default target type will be **System Messages**. When an event occurs in Cribl Stream to trigger a Notification condition, you will see an indicator in the <img className='inline' src="/images/page/icon-messages.svg"></img> **Messages** icon. Select the icon to open the **Messages** drawer and view the Notification details.

## Add a Notification Target

To add a new Notification target:

1. In the Cribl Stream sidebar, select **Notifications**, and then select the **Targets** tab.
...
> options will be hidden or disabled in Cribl Stream's UI.
>
{.box .info}
