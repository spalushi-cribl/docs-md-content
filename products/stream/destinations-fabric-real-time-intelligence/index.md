...
>
> Before you create a Fabric Real-Time Intelligence Destination in Cribl Stream, [create a **Cribl** data source in Microsoft Fabric](https://aka.ms/criblrtidoc). Microsoft Fabric will then provision the connection values (like the bootstrap server) that you must use when you configure a Fabric Real-Time Intelligence Destination in Cribl Stream.
>
{.box .info}
...
**SASL JASS password**: The stored secret that references the **SASL JASS password-primary** or **SASL JASS password-secondary** in the **Cribl** data source in Microsoft Fabric. The secret can reside in Cribl Stream's [internal secrets manager](securing-kms-config) or (if enabled) in an external KMS. A **Create** link is available if you need a new secret.
...
To [add a **Cribl** data source in Microsoft Fabric](https://aka.ms/criblrtidoc) and retrieve the values necessary to [configure](#configure-fabric-dest) the Fabric Real-Time Intelligence Destination in Cribl Stream, you must have **Contributor** (or higher) Fabric workspace permissions.
