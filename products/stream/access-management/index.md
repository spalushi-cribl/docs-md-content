...
- [Permissions](permissions): Fine-grained access control configurable at separate levels (Organization, Workspace, product, Worker Group, and lower-level resources like Cribl Stream Projects and Cribl Search Datasets).
...
## Which Access Model Should I Use? {#restrictions}

> **Cribl.Cloud** relies only on [Members](members)/[Permissions](permissions).
>
{.box .cloud}

For on-prem deployments, Cribl supports both the Members/Permissions and the legacy Roles/Policies models. These models are cross-compatible for many use cases. However, certain purposes require you to choose a specific model:
...
- Use [Workspace-level](permissions#workspace) Permissions to manage overall access to an on-prem deployment.
