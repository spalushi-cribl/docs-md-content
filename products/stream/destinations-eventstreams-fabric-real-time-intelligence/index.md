# Fabric Eventstreams and the Fabric Real-Time Intelligence Destination


> If the **Cribl** data source is not yet available in Microsoft Fabric, follow the instructions on this page. If the **Cribl** data source *is* available, follow the instructions in [Fabric Real-Time Intelligence](destinations-fabric-real-time-intelligence) instead.
>
{.box .warn}

Cribl Stream supports sending data to [Eventstreams](https://learn.microsoft.com/en-us/fabric/real-time-intelligence/event-streams/overview) in [Microsoft Fabric Real-Time Intelligence](https://learn.microsoft.com/en-us/fabric/real-time-intelligence/overview).

> Type: Streaming | TLS Support: Configurable | PQ Support: Yes
>
> Before you create a Fabric Real-Time Intelligence Destination in Cribl Stream, [create an Eventstream and custom endpoint source](#create-eventstream-custom-source) in Microsoft Fabric. You will then retrieve the connection values (like the bootstrap server) to use when you configure a Fabric Real-Time Intelligence Destination in Cribl Stream.
>
{.box .info}

## Create an Eventstream and Custom Endpoint Source in Microsoft Fabric {#create-eventstream-custom-source}

To create an Eventstream that you can use to send data from Cribl Stream, create a new [workspace](https://learn.microsoft.com/en-us/fabric/fundamentals/create-workspaces) and [Eventstream](https://learn.microsoft.com/en-us/fabric/real-time-intelligence/event-streams/create-manage-an-eventstream) as follows:

1. Log in to [Microsoft Fabric](https://app.fabric.microsoft.com).

1. Select **New workspace**. Enter a name (`Cribl`) and an optional description for the workspace.

1. If you are not using a trial account, select **Advanced > Fabric capacity** for License mode.

1. Select **Apply** to create the workspace.

1. Select **New item > Eventstream**. Enter a name (`Cribl`) for the Eventstream.

1. Select **Create** to open the homepage for the new Eventstream.

In the new Eventstream, create a custom endpoint source to enable sending data to Eventstream via Kafka protocol:

1. On the homepage for the new Eventstream, select **Use custom endpoint**.

1. Enter a name for the custom endpoint source (`Cribl Source`) and select **Add**.

1. Select **Publish** to enable the custom endpoint.

Once the custom endpoint is enabled, you can copy the connection values that you need to [configure the Fabric Real-Time Intelligence Destination](#configure-fabric-dest):

1. Select the new `Cribl Source`.

1. In the **Details** pane, select **Kafka**.

1. Select **SAS Key Authentication**.

1. Copy the values from the **Bootstrap server**, **Topic Name**, and **Connection string-primary key** fields.

## Configuring Cribl Stream to Output to Microsoft Fabric Real-Time Intelligence {#configure-fabric-dest}

1. On the top bar, select **Products**, and then select **Cribl Stream**. Under **Worker Groups**, select a Worker Group. Next, you have two options:
   - To configure via [QuickConnect](quickconnect), navigate to **Routing** > **QuickConnect** (Stream) or **Collect** (Edge). Select **Add Destination** and select the Destination you want from the list, choosing either **Select Existing** or **Add New**.
   - To configure via the [Routes](routes), select **Data** > **Destinations** or **More** > **Destinations** (Edge). Select the Destination you want. Next, select **Add Destination**.
2. In the **New Destination** modal, configure the following under **General Settings**:
   - **Output ID**: A unique name to identify this Fabric Real-Time Intelligence Destination. If you clone this Destination, Cribl Stream will add `-CLONE` to the original **Output ID**.
   - **Description**: Optionally, enter a description.
   - **Bootstrap server**: The value of the **Bootstrap server** setting for the **Cribl Source** custom endpoint source in Microsoft Fabric.
   - **Topic name**:  The value of the **Topic name** setting for the **Cribl Source** custom endpoint source in Microsoft Fabric. Can be overwritten using the [`__topicOut` field](#internal-fields).
...
**SASL mechanism**: SASL (Simple Authentication and Security Layer) authentication mechanism to use. Defaults to `PLAIN`, which exposes [Basic Authentication](#plain) options from the **Cribl Source** custom endpoint source in Microsoft Fabric. Select `OAUTHBEARER` to enable [OAuth Authentication](#oauthbearer) via a different set of options.
...
**SASL JASS password**: The stored secret that references the **Connection string-primary key** in the **Cribl Source** custom endpoint source in Microsoft Fabric. The secret can reside in Cribl Stream's [internal secrets manager](securing-kms-config) or (if enabled) in an external KMS. A **Create** link is available if you need a new secret.
...
- `cribl_group` - Cribl Stream Worker Group that processed the event.
...
**Reauthentication threshold (ms)**: Time window during which Cribl Stream can reautheticate if needed, measured backward from the moment when credentials are set to expire. Defaults to `10000` (10 seconds).
...
To add a **Cribl Source** custom endpoint source in Microsoft Fabric and retrieve the values necessary to [configure](#configure-fabric-dest) the Fabric Real-Time Intelligence Destination in Cribl Stream, you must have **Contributor** (or higher) Fabric workspace permissions.
