...
* Notice that the API structures Azure Active Directory (AD) Audit Logs data as a single JSON object with events nested under the `value` field. With an Event Breaker, you can [split](#event-breakers) this into individual events for each AD Audit Log activity.
...
**Collect URL**: An expression that produces a URL for the collect operation. The URL can be any of the Graph API [endpoints](https://docs.microsoft.com/en-us/graph/api/overview?view=graph-rest-1.0). In this example, we want to retrieve Azure AD Audit logs, so we use the Audit Logs endpoint:
