...
## Configuring a Database Collector {#configuring-db-collector}
...
1. For **SQL Query**, input the following string, including the backticks:
   ...
   `` `select * from <table-name> where ${state && state.id ? `id > ${state.id.value}` : '1 = 1'} order by <id>` ``
...
## Managing Collector State {#manage-state}

For the Database Collector, **state** identifies the last row that the Collector has read from the database, as expressed by that row's value in the [tracking column](#tracking-column). You can view, edit, refresh, or delete that state – and that is what we mean by managing state. Testing or debugging a Collector is the main reason to do this.

The **Manage State** button is enabled whenever your Collector's state is not empty, and is available from two locations: 

* In the **Manage** > **Data** > **Manage Sources** / **Collectors** / **Database** list view of your Database Collectors, at the end of each Collector's row.
...
If you want to start again with no state, click **Delete State**. To learn how and why to use this operation, see [Deleting Collector State](#delete-state) below. 

### Editing Collector State {#edit-state}
...
### Deleting Collector State {#delete-state}
...
#### Deleting Collector State in a Single-Instance Deployment
...
#### Deleting Collector State in a Distributed Deployment
