...
Keeping the **Keep-alive timeout** low (such as the default 90 seconds) will minimize Cribl Stream memory usage (due to long-lived open connections), but it will result in an additional network round-trip on each batch of events delivered.
