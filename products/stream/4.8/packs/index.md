...
For example, where a Pipeline configuration references Lookup file(s), Cribl Stream will import the Pipeline only if the Lookups are available in their configured locations. A Pack can consolidate this dependency, making the Pipeline portable across Cribl Stream instances. If you import the Lookups into the Pack, you can develop and test a configuration, and then port it from development to production instances, or readily deploy it to multiple Worker Groups.

### What Is a Pack?

Each Pack provides you with a user interface (described in the first half of the [Using Packs](#using-packs) section below) and can also be imported and otherwise treated as a file in `.crbl` format.
...
- Packs can't include Event Breakers, which are associated with Sources. (However, you can instead use the [Event Breaker Function](event-breaker-function) in Pipelines included with your Pack.)
...
![A Pack snaps into Cribl Stream like an enhanced Pipeline](pack-context-exploded.c40453ad38.png)
...
In a single-instance deployment, Packs are global. From the top-level **Manage** tab, select **Processing** > **Packs**. On the filesystem, Packs (including those that you add) are stored at `$CRIBL_HOME/default/`.
...
In a [distributed deployment](/stream/deploy-distributed), Packs are associated with (and installed within) Worker Groups. Select **Manage**, and then click into the Worker Group you want to manage (for example, `default`). Next, from that Worker Group's top nav, select **Processing** > **Packs** (Stream) or **More** > **Packs** (Edge).
...
> By design, you can readily share Packs **across** Worker Groups by [copying](#copy) them between each other.
>
{.box .success}
...
Once loaded, each Pack displays a submenu with familiar links: **Routes**, **Pipelines**, **Knowledge**, and **Settings** on the left pane, along with **Sample Data**, and **Preview Simple** on the right. The Pack submenu is a subset of the top nav.

![Configuring a Pack](se-pack-config.495596b977.png)
{border="true"}

The left pane submenu links give you access to configuration objects specific to this Pack.

> There is a limit of 200 Pipelines allowed per Pack. This limit is in place to ensure optimal performance and prevent potential errors. It helps maintain stability and avoids issues that can arise from managing very large packs.
>
{.box .warning}
...
Cribl provides the [Cribl Packs Dispensary](https://packs.cribl.io/) as a resource for you to find and share Packs. Packs are developed by Cribl or by our partners and users and loaded to the Cribl Packs Dispensary for easy sharing. Cribl will test and curate submissions to ensure the quality of the repo contents.

You can install Dispensary Packs directly through Cribl Stream's user interface, as outlined in [Add from Packs Dispensary](#score) below.
...
1. Click any Pack tile to display its details page with its README. This will typically outline the Pack's purpose, compatibility, requirements, and installation.
...
1. Optionally, give the pack an explicit, unique **New Pack ID**. (See [Upgrading an Existing Pack](#upgrading).)
2. Optionally, enter a **Branch or tag** to filter the import source using the repo's metadata. You can specify a branch (such as `master`) or a tag (such as a release number: `0.5.1`, and so forth).
...
You can install Dispensary Packs directly through Cribl Stream's user interface, as outlined in [Import from Git Repos](#git) above. However, if you prefer, you can click through to any Dispensary repo release page, download the corresponding `.crbl` file, and then [upload the file](#file) into Cribl Stream.
...
> If you toggle this option to `Yes`, the imported Pack will completely overwrite your existing Pack configuration.
> 
...
- If the Pack includes any user–modified Knowledge objects (for example, Lookup files), be sure to copy the modified files locally for safe keeping before upgrading the Pack. After installing the upgrade, copy those files over the Pack upgrade's default versions.
- Test, test, and test!
- Commit and Deploy.
...
7. Use the standard Cribl Stream controls (see [above](#controls)) to configure and save the infrastructure you want to pack up. As you save changes in the UI, they're saved to the Pack.
...
You can update Pack metadata (Version, Description, Author, and so forth) and display settings. If you're developing a new Pack to share, you'll want to use this interface to populate the Pack README and display logo.

1. From the Pack's submenu, select **Pack Settings**. The left **README** tab will gain focus.
2. To populate the Pack README file, toggle **View** to **Edit**, replace the placeholder markdown content, and **Save**.
...
You can easily share Packs in a [distributed deployment](/stream/deploy-distributed) by copying them between Worker Groups as described here or [using the Cribl API](/api/copy-export-install-packs#copy-packs).
...
1. Select the Worker Groups to copy the Pack to.
1. Optionally, toggle **Overwrite** on to replace existing Packs with the same **Pack ID**.
1. Select **Copy**.
