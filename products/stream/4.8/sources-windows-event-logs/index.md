...
Cribl Stream supports collecting local Windows Event Logs. 
...
For more information about how to locate Windows event logs on a Windows server, see [Locate Windows Events Logs in the Server](usecase-windows-observability#locate-windows-event-logs-in-the-server).

## Configuring Cribl Stream to Collect Windows Event Logs
...
- Events include the `__winEvent` property in both XML and JSON format. This
  property is one of the default **Excluded fields** in the [Cribl TCP Destination](destinations-cribl-tcp#advanced-settings).

> The `__winEvent` property contains a JSON representation of the native XML
> data obtained from the Windows Event API. The Windows Event Logs Source
> attempts to convert the JSON representation to the JSON schema that Windows Tools uses
> – however, there are some cases where the automatic conversion
> process doesn't perfectly match the JSON schema. The `__winEvent` property
> provides the raw JSON representation, allowing you to extract any necessary data
> if the conversion process isn't perfect.
{.box .info}

**Render event message strings**: Toggle to `Yes` to display rendered message
strings in the XML and JSON event output.

- This toggle only appears when **Use Windows Tools** is set
  to `No`. 
- Defaults to `Yes` when JSON is the selected event format and `No` when XML 
  is the event format.
- Events will not contain a `Message` property when this is set to `No`.
...
**Use Windows Tools**: Set to `No` by default.
...
>Batch size and Polling interval only apply when **Use Windows Tools** is set to `Yes`.
{.box .info} 

**Max event bytes**: The maximum number of bytes that can be in an event before Cribl Stream sends it to the Pipelines. Defaults to `51200`.
...
long as the **Use Windows Tools** toggle is set to `No` in [Advanced Settings](#advanced-settings).
