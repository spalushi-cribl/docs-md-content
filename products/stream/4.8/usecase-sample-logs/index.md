...
Though not always the easiest, this is the best option, because samples are captured exactly as they will look when handed off to Cribl Stream in production. You'll set Cribl Stream up as a [passthrough](aggregations-function#output-settings) which gets samples “right off the wire” and dumps them to [DevNull](destinations#available-internal-destinations).
