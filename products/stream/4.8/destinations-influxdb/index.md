...
**Dynamic value fields**: When enabled, Cribl Stream will pull the value field from the metric name. (For example, `db.query.user` will use `db.query` as the measurement and `user` as the value field). Defaults to `Yes`.
