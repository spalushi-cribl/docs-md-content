...
**Domain name**: WaveFront domain name, for example, `longboard`. Required.
...
- **Secret**: This option exposes an **Auth token (text secret)** drop-down, in which you can select a [stored secret](/stream/securing-secrets) that references the API auth token described above. A **Create** link is available to store a new, reusable secret.
