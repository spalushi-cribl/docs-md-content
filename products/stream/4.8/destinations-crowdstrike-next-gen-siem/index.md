...
For example, consider a scenario where you generate three terabytes of firewall logs daily, with an average event size of 1,250 bytes. To determine your EPS, you can utilize the Cribl Stream [monitoring](monitoring) view to track peak EPS over a 24-hour period. Alternatively, if your data rate is consistent, you can calculate the average EPS using the following formula:
...
1. **Extract sample `_raw` data**: In Cribl Stream, while editing the relevant Pipeline, open the [sample data](/stream/usecase-sample-logs) output and copy the content of the `_raw` field.
