...
To optimize Cribl-managed Worker Nodes on Cribl.Cloud, navigate to the desired Worker Group's **Group Settings** > **Group Configuration** section. Then adjust the [Sizing Calculator](cloud-workers#calc) to determine the appropriate size for your Worker Group.

With a customer-managed [single-instance deployment](deploy-single-instance), you govern resource allocation through the **Settings** > **Global Settings** > **System** >  **Service Processes** > **Processes** section.

With a customer-managed [distributed deployment](deploy-distributed) or [hybrid group](cloud-enterprise), you allocate resources per Worker Group. Navigate to **Manage** > `group-name` > **Group Settings** > **Worker Processes**.

In any of the above customer-managed scenarios, these controls are available:
...
1. Enable [Worker UI Access](setting-up-leader-and-worker-nodes#teleporting) if you haven't already.
...
1. Select **Worker Settings** from that Worker Node's top nav.
