...
```js
Crypto.encrypt(value: Buffer | string, keyclass: number, keyId?: string, defaultVal?: string): string
```
...
```js
Crypto.createHmac(value: string | Buffer, secret: string, algorithm: string = 'sha256', outputFormat: 'base64' | 'hex' | 'latin1' = 'hex'): string
```
