...
## Configuring Cribl Stream to Output to Microsoft Sentinel
...
**Validate server certs**: Reject certificates that are not authorized by a trusted CA (for example, the system's CA). Toggle this to `No` if you want Cribl Stream to accept such certificates. Defaults to `Yes`.
