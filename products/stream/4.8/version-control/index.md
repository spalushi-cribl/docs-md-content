...
Once Git is installed, you can commit configuration changes using the `git` CLI. You can also commit changes interactively, using the Cribl Stream UI. 
...
Next, in the resulting **Commit Changes** modal, you can verify the diff'ed configuration changes. Other options here include clearing check boxes for individual files to exclude them from the commit (as shown below), and clicking **Undo** to reverse the changes instead of committing them. 
...
Once Git is installed, you can revert to a previous commit using the `git` CLI. You can also restore a previous commit for a Worker Group using the Cribl Stream UI: 
...
### Connecting to a Remote with SSH {#connecting-to-github-with-ssh}
...
![Cribl Stream Git settings](git-settings-annotated-3.4.d543e96464.png)
{border="true"}
...
![Git Authentication Type settings](git-settings-general.17ad9bd01e.png)
{border="true"}
...
![Git Authentication Type settings](git-settings-remote.62857f8526.png)
{border="true"}
...
### Worker Deployment: Handling Customer-Defined `.gitignore` Exclusions {#bundler}

The `bundler` object in `cribl.yml` allows you to deploy files to GitOps that are usually excluded via gitignore, like `cribl.secret` and `secrets.yml`. This facilitates loading custom secrets per environment and Group. However, these files are never committed to GitOps. Instead, a customer-managed logic (outside GitOps) ensures the appropriate secret files are dropped during deployment for each environment and group. This approach safeguards sensitive information while leveraging the `bundler's` functionality within your GitOps workflow.

To leverage this functionality for deployment-critical files, configure `cribl.yml` using the `bundler.bundleGitIgnoredPatterns` setting. This ensures essential files reach Workers, even if they are not tracked with version control due to customer-specific `.gitignore` rules.

##### Example: Using `.gitignore` with `bundler`

For example, if you want to exclude your [Cribl secret](authentication#the-criblsecret-file) from Git,
add `local/cribl/auth/cribl.secret` to `.gitignore`.

You can then add the same path to `bundler` to ensure it is deployed to GitOps during deployment.
...
If you want to avoid tracking custom certificates as well,
use the path for the directory in which the secrets/certificates reside: `local/cribl/auth`.
