...
1. Go to **Distributed Settings**. You can find them either in **Settings** > **Global Settings**, or in **Worker Settings**.
1. Select **General Settings**.
1. In **Mode**, choose the mode you want the instance to run as:
   - **Leader**
   - **Stream: Managed Worker (managed by Leader)**.
1. On the **Leader Settings** left tab, configure the required Leader settings:
...
Remote UI access to Workers (also known as teleporting) enables you to click through from the Leader's **Manage Worker Nodes** page
...
1. Select the top nav's **Manage** tab, then make sure the submenu's **Groups** tab is selected.
2. In the row of the Worker Group you want to enable teleporting for, enable **UI Access**.
  ...
  > On Cribl.Cloud, the **Groups** page adds extra columns.
  > You'll see each Group's Worker type (hybrid versus Cloud/Cribl-managed) and, for Cloud Groups,
...
1. Select the top nav's **Manage** tab, then the **Workers** tab.
...
You can add tags to a Worker Node you've teleported into.
...
> The mode for a Leader Node is called `master`, which corresponds to an earlier, outdated name for the mode.
{.box .success}
...
For other options, see the [CLI Reference for mode-worker](cli-reference#mode-worker).
