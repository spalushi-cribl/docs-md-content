...
From the top nav, click **Manage**, then select a **Worker Group** to configure. Next, you have two options:
  * To configure via [QuickConnect](quickconnect), click **Routing** then **QuickConnect** (Cribl Stream) or **Collect** (Cribl Edge). Next, click **Add Source** and from the resulting drawer's tiles, select **Syslog**. Next, click either **Add Source** or (if displayed) **Select Existing**.
  * To configure via [Routes](routes), click **Data** then **Sources** (Cribl Stream) or **More** then **Sources** (Cribl Edge). Select **Syslog** from the list of tiles or the **Sources** left nav. Next, click **Add Source** to open a **Add Source** modal.
...
>
> For Syslog devices that are hardcoded to send to port 514, you can listen on this port on Cribl.Cloud, which is normally available only with privileged access.
...
**Address**: Enter the hostname/IP on which to listen for data., For example `localhost` or `0.0.0.0`.
...
**Enable TCP load balancing**: Toggle to `Yes` if you want the Source to load balance traffic across all Worker Processes, as explained [below](#tcp-lb). (This setting is available only in Cribl Stream deployed in Distributed mode.)
...
For customer-managed and hybrid Worker Groups, consider reducing the [Process count](/stream/scaling#worker-processes) by 1, if possible. This will leave a core free to run the load balancer process.
