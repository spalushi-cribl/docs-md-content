...
You can use Cribl Edge to apply and author Event Breakers while exploring files – even files that you have not saved as sample files. This option includes remote files that are viewable only from Edge. See [Exploring Cribl Edge on Linux](/edge/explore-edge) or [Exploring Cribl Edge on Windows](/edge/explore-edge-win).
...
On the **Event Breaker Rulesets** page (see screenshot [above](#intro)), click **+ Add New** to create a new Event Breaker ruleset. Click **+ Add Rule** within a ruleset to add a new Event Breaker.
