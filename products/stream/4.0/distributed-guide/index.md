...
6. {{< id `worker-ui-access` >}} Optional but recommended: From Cribl Stream's top nav, click **Manage** > **Groups**. Then enable the **UI access** slider for your Group(s). This way, you will be able to click through from the Leader's **Manage** > **Workers** tab to view and manage each Worker's UI, distinguished by a [purple border](#teleport-border). (This option handles authentication for you, so you don't need to manage or enter the Workers' credentials.)
...
1. Click **New Source** to open a **New Source** modal.
...
1. Click **Save**. {{< id `datagen-status` >}}  
    ...
    In the **Enabled** column, the slider set to **Yes** indicates that your Datagen Source has started generating sample data.

1. Click **Commit & Deploy** at the upper right, enter a commit message, and click **Commit & Deploy**.  
...
1. At the **Pipelines** pane's upper right, click **+ Pipeline**, then select **Create Pipeline**.
...
To add and manage more than one Worker Group – everything in this optional section – you'll need an Enterprise or Sales Trial license for your on-prem Cribl Stream deployment, or an Enterprise (or equivalent) plan for your Cribl.Cloud Organization. 
...
![UI for multiple Worker Groups](groups-ui.126c41b558.png)
{border="true"}
...
![Select a Worker's GUID link to tunnel to that Worker](st-worker-ui-access.70497a71ad.png)
{border="true"}
{{< id `teleport-border` >}}
...
![Default Mapping Rule](mappings-default.bae399249f.png)
{border="true"}
...
4. Set the **UI access** slider to `Yes`.

![Creating a new Group](group-new-create.625b02b3a7.png)

4. Save the Group.

5. Click **Deploy** on the new Group's row, and confirm your choice. The new Group should deploy immediately.

![Second Groups saved, ready to Deploy](group-2nd-saved.ebd52cc303.png)
{border="true"}
...
![Manage Groups page confirms Workers' mapping and assignment](group-2nd-saved.ebd52cc303.png)
{border="true"}
...
- Navigate to Cribl Stream's `default` Group > **Data** > **Sources** > **Datagen**, and switch off the slider beside `weblog.log`.

- If you configured a second Worker Group: Navigate to Cribl Stream's `CriblMetrics` Group > **Data** > **Sources** > **Cribl Internal**, and switch off the slider beside  **CriblMetrics**.
