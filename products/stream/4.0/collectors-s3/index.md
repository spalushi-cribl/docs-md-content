...
From the top nav, click **Manage**, then select a **Worker Group** to configure. Next, select **Data** > **Sources**, then select **Collectors** > **S3** from the **Manage Sources** page's tiles or left nav. Click **New Collector** to open the **S3** > **New Collector** modal, which provides the following options and fields.
...
`/${index}/${year}/${month}/${day}/${host}/${category}/${hour}/${minute}/${file}`

![Using path extractors to transform timestamps](path-extractor-s3-expressions.7c06537098.png)
{border="true"}

Token | Extractor Expression | Purpose
--- | --- | ---
`index` | `{"indexUppercase" : value.toUpperCase()}` | Uppercase the `index`
`year`  | `{"yearShort" : value.splice(-2)}` | Convert long year to short 2-digit year
`month` | `{"monthLongName" : new Date(Date.UTC(1970, value-1)).toLocaleString('en-US',{month: 'long'})}` | Convert month from numbers to name
`day`   | `{"dayLongName" : new Date(Date.UTC(1970, 1, value-1)).toLocaleString('en-US',{weekday: 'long'})}` | Convert day-of-week from numbers to name
...
`hour`  | `{"hourStandardTime" : (value + 11) % 12 + 1}` | Convert military time to standard time
