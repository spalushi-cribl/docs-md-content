...
Cribl Stream supports sending events to [SignalFx](https://docs.signalfx.com/en/latest/).
...
Or, to configure via the [Routing](routes) UI, click **Data** > **Destinations** (Stream) or **More** > **Destinations** (Edge). From the resulting page's tiles or the **Destinations** left nav, select **SignalFx**. Next, click **New Destination** to open a **New Destination** modal that provides the options below.
...
- **Manual**: Displays an **Auth token** field for you to enter your SignalFx API access token. See SignalFx's [Manage Tokens](https://docs.signalfx.com/en/latest/admin-guide/tokens.html#working-with-access-tokens) topic.
...
For details on integrating with SignalFx, see the [SignalFx Developers Guide](https://developers.signalfx.com/), with particular reference to the [SignalFx HTTP Send Metrics Reference](https://developers.signalfx.com/ingest_data_reference.html#tag/Send-Metrics).
