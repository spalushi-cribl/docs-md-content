...
Or, to configure via the [Routing](routes) UI, click **Data** > **Destinations** (Stream) or **More** > **Destinations** (Edge). From the resulting page's tiles or the **Destinations** left nav, select **Confluent Cloud**. Next, click **New Destination** to open a **New Destination** modal that provides the options below.
...
Authentication parameters to use when connecting to brokers. Using [TLS](#tls_client) is highly recommended.
