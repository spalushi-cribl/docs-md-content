...
![Monitoring page](monitoring-page-3.1.6e596ac70b.png)
{border="true"}

> Metrics displayed in the Cribl Stream UI are in units of kibibytes (KB), mebibytes (MB), gibibytes (GB), and tebibytes (TB).
>
{.box .info}
...
Click a **View Details** button to access a Worker Node's **Worker Settings**, with details per Worker Process. (To enable this button, you must first enable the parent Group's [Remote UI Access](deploy-distributed#worker-access).)
...
![Monitoring > Data submenu (Pipelines selected)](monitoring-data-submenu.f84d3b9b3d.png)
{border="true"}
...
![Sparklines and fly-out](Sparklines-composite.5d55f94c07.png)
{border="true"}
...
- Leaders (see [Second Leader](deploy-add-second-leader))
- Queues (see [Persistent Queues](persistent-queues))
- [Licensing](#licensing)
- Jobs (and tasks in-flight, see [Collector Sources](collectors))
- [Job Inspector](#job-inspector)

![Monitoring > System submenu (Jobs in‑flight selected)](monitoring-system-submenu.ad3f6eb3ab.png)
{border="true"}

##### Leaders

Select **Monitoring** > **System** > **Leaders** to view the status of your Leader Nodes. For more information on how to configure a second Leader Node for failover/durability, see [Second Leader](deploy-add-second-leader)).

![Monitoring Leader Nodes](se-monitoring-leaders.e524e221a4.png)
{border="true"}
...
![Monitoring > Licensing](st-monitoring-license.b7df2e4f79.png)
{border="true"}
...
Select **System** > **Job Inspector** from the Monitoring page's top nav to view and manage pending, in-flight, and completed collection jobs and their tasks. For details about the resulting page, see [Monitoring and Inspecting Collection Jobs](collectors#inspect).
...
![Monitoring > Reports/Top Talkers](st-monitoring-top-talkers.d82697db61.png)
{border="true"}
...
1. On the **CriblLogs** and/or the **CriblMetrics** row, slide the **Enabled** slider to `Yes`. Confirm your choice in the resulting message box.
...
1. Click and drag the plus sign on the right side of the Source to your desired Destination. A **Connection Configuration** modal will prompt you to select a **Passthru**, **Pipeline**, or **Pack connection**. See [QuickConnect](quickconnect#bottom-drawer) for details.
...
![Logs page (controls highlighted)](Log-Screen-annotated.b61c365951.png)
{border="true"}
...
2. **Fields selector**: Click the **Main | All | None** toggles to quickly select or deselect multiple check boxes below. Beside these toggles, a Copy button enables you to copy field names to the clipboard in CSV format.
   ...
   ![Monitoring - Copy Fields Icon](monitoring-copy-fields-icon.aa8914affc.png)
...
![Manage Logging Levels page](log-levels-composite.826fca10ae.png)
{border="true"}
