...
```shell
CRIBL_DIST_LEADER_URL=tls://<token>@stream-<Organization-name>.cribl.cloud:4200
```
...
By default, the above command pulls the freshest stable image (tagged `cribl/cribl:latest`) from [Cribl's Docker Hub](https://hub.docker.com/r/cribl/cribl/tags). Our Docker images are built on Ubuntu 20.04.
