...
> If a request fails, Cribl Stream will resend the data to a different endpoint. Cribl Stream is designed to block only if **all** endpoints are experiencing problems. See [here](#grace) to tune this behavior.
>
{.box .info}
...
Or, to configure via the [Routing](routes) UI, click **Data** > **Destinations** (Stream) or **More** > **Destinations** (Edge). From the resulting page's tiles or the **Destinations** left nav, select **Splunk** > **Load Balanced**. Next, click **New Destination** to open a **New Destination** modal that provides the options below.
