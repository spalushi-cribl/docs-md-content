...
![Preview options](se-data-preview-4.0.c580e071ea.png)
{border="true"}
...
![Pipeline status tab](se-pipeline-status-tab.408591103e.png)
{border="true"}
...
The **Capture Data** button opens a slightly different modal – it does not require event breaking. In the composite screenshot below, we've already captured some events using the **Capture** drop-down. 
...
To prevent in-memory samples from getting unreasonably large, samples input by any means (Paste, Attach, Remote File, or Capture New) are constrained by a limit set at **Settings** > **Global Settings** > **General Settings** > **Limits** > **Max sample size**. The default limit is `256KB`, and you can adjust this upward or downward.
...
![Managing sample files](se-samples-mgmt-tab-4.0.3ef811163e.png)
{border="true"}
...
![Preview Simple schematic](preview-simple-schematic.90d045504e.png)
{border="true"}

The **Preview Full** option gives you a choice of viewing events on the **OUT** side of either the [processing or post-processing Pipeline](pipelines#types-of-pipelines). Selecting this option expands the Preview pane's upper controls to include an **Exit Point** drop-down, where you make this choice.

![Preview Full schematic](preview-full-schematic.222347e854.png)
{border="true"}
...
![Event, Table, and Advanced options (composite screenshot)](preview-INJSON-3.1.3.0b9a4e0cab.png)
{border="true"}
...
At the Preview pane's top, you can select a sample data file and Pipeline, then click Pipeline diagnostics button to access statistics on the selected Pipeline's performance.

![Pipeline diagnostics](pipeline-diagnostics-button.465d8c577f.png)
...
The **Pipeline Profile** tab, available from the **Preview Simple** tab, helps you debug your Pipeline by showing individual Functions' contributions to data volume, events count, and the Pipeline's processing time.

![Pipeline diagnostics > Pipeline Profile tab](pipeline-profile.c54c6049f8.png)
{border="true"}

The **Advanced CPU Profile** tab, available from the **Preview Simple** tab, displays richer details on individual Function stacks.

![Pipeline diagnostics > Adv anced CPU Profile tab](advanced-cpu-profile.06137993a6.png)
{border="true"}
...
![Disabling Pipeline profiling](pipeline-profile-deselect.c7cec41483.png)
{scale="75%" border="true"}
...
The final option on the ⚙️ **Advanced Settings** menu opens a modal where you can preview Cribl Stream's internal logs summarizing how this data sample was processed and captured.
...
![Dropped and added fields in a Pipeline's output](preview-OUT-annotated-3.0-composite.da50d85d65.png)
{border="true"}
...
![Collapse / Expand toggle (composite)](preview-toggle-composite-3.0.0dff3d71ef.png)
