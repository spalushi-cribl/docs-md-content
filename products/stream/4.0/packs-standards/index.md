...
This page outlines the process for Cribl Community members to publish Cribl Stream [Packs](packs) to the [Cribl Packs Dispensary™](https://packs.cribl.io/). It also lists standards that apply to all publicly available Community Packs.
...
Reusability means that the Pack brings value to multiple Cribl Stream users. To make this possible, you should be sure your README provides:
...
- Read the [docs](packs#creating-a-pack) that explain how to create your Pack from within the Cribl Stream UI.
...
For example, consider a device sending data to Cribl Stream, such as a Palo Alto Networks firewall. This device supports multiple datasets, in that Palo Alto Firewall data is really one co-mingled set of data that includes individual datasets like `PAN-Traffic`, `PAN-System`, `PAN-Accept`, and so on.
...
3. Each Pipeline should have an internal **Comment** describing the overall functionality **and benefits** that the Pipeline provides. The procedure for adding a **Comment** is the same as for adding any other @{production} Function. **Comment** is under the **+ Function** > **Standard** drop-down.
