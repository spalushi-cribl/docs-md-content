...
In either case, you'll need outbound internet access to `https://diag‑upload.cribl.io` and a valid support case number. The above URL works only when using the `cribl diag` command or uploading using the Cribl Stream UI. (So connecting directly to it with your web browser will fail.)

> With a [Cribl.Cloud](deploy-cloud) deployment, contact Cribl Support to gather a diag bundle on your behalf.
>
{.box .cloud}

### Using the UI {#ui}

To create a bundle, go to **Settings** > **Global Settings** > **Diagnostics > Diagnostic Bundle** and click **Create Diagnostic Bundle**. 
...
You can create a bundle from individual workers if you have the [Worker UI access](deploy-distributed#worker-access) setting enabled. Go to **Workers >** &lt;worker-name&gt; > **Settings** (top right) > **Diagnostics > Diagnostic Bundle**, and click ** Create Diagnostic Bundle**.
