...
![PACKs badged in Routing table's Pipeline column](packs-in-routes.70141896e9.png)
{border="true"}

The **PACK** badge is also displayed when you click into a resource – shown here on one of the Routes from the above table:

![PACK badge on a Pack connected to a Route](pack-in-1-route.3f7540f94f.png)
{border="true"}
...
![Packs, single-instance navigation](packs-single-instance.ed1374dec3.png)
{border="true"}

#### Distributed/Default Worker Group

In a distributed deployment with the default single Worker Group (Leader mode), select **Manage**, click into the `default` group, and then select **Processing** > **Packs**.

![Packs, Leader mode](packs-leader-mode.a577c518fb.png)
{border="true"}

#### Distributed/Multiple Worker Groups/Fleets

In a [distributed deployment](/stream/deploy-distributed) with multiple Worker Groups/Fleets (Leader mode), Packs are associated with (and installed within) Worker Groups. Navigate to the parent Worker Group, then select **Processing** > **Packs** from that Group's top nav.

![Worker Group > Manage Packs page](packs-worker-group.74a421500b.png)
{border="true"}
...
![Manage Packs page with example Pack](packs-manage-shipped.d51f474d8f.png)
{border="true"}
...
![Pipelines within example Pack](pack-sample-pipelines.a5e44d0e77.png)
{border="true"}
...
![Configuring a Pack](se-pack-config-3.5.934ddd74cb.png)
{border="true"}
...
1. Click **New Pack** at the upper right.
...
![Importing a Pack](pack-import.e3a868a31c.png)
{border="true"}
...
You can install Dispensary Packs directly through Cribl Stream's UI, as outlined in [Add from Packs Dispensary](#score) below.
...
![Pack details page: Composite with **+ Add Pack** button, confirmation banner, Dispensary drawer in background](se-pack-details-confirm-composite.412abc27d0.png)
{border="true"}
...
> To import a Pack from a public URL, Cribl Stream's Leader Node (or single instance) requires Internet access. A [distributed deployment](/stream/deploy-distributed)'s Leader can then deploy the Pack to Workers even if the Workers lack Internet access.
>
{.box .success}
...
> To import a Pack from a public repo, Cribl Stream's Leader Node (or single instance) requires Internet access. A [distributed deployment](/stream/deploy-distributed)'s Leader can then deploy the Pack to Workers even if the Workers lack Internet access.
>
{.box .success}
...
You can install Dispensary Packs directly through Cribl Stream's UI, as outlined in [Import from Git Repos](#git) above. However, if you prefer, you can click through to any Dispensary repo's release page, download the corresponding `.crbl` file, and then [upload the file](#file) into Cribl Stream.
...
![Upgrading an existing Pack](pack-upgrade.994bc84feb.png)
{border="true"}
...
> 
> If you have a Cribl.Cloud account, you can also collaborate there by inviting team members to your Crlbl.Cloud Organization. See [Inviting and Managing Other Users](deploy-cloud#multi-user).
> 
...
1. From the Pack's submenu, select **Settings**. The left **README** tab will gain focus.

![Pack Settings](pack-settings.5f2cf4593d.png)
{border="true"}

2. To populate the Pack's README file, toggle **View** to **Edit**, replace the placeholder markdown content, and **Save**.

![Editing Pack's README](pack-readme-composite.27af6dc9a3.png)
{border="true"}

3. To update other metadata, click the left **Settings** tab.

![Editing Pack's metadata](pack-metadata.091b44c833.png)
{border="true"}
...
![Editing Pack's display (logo) settings](pack-display-settings.739cf941fd.png)
{border="true"}
...
![Exporting a Pack](pack-export.f935090f4e.png)
...
![Exporting multiple packs at once](packs-export-multiple.e6c5993bd7.png)
{border="true"}
