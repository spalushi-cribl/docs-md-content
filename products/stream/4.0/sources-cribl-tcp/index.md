...
As one usage example, assume that you want to send data from one Worker Node deployed on-prem, to another that is deployed in [Cribl.Cloud](/stream/deploy-cloud). You could do the following:
...
- Create a Cribl TCP Source, on the targetWorker Group in Cribl.Cloud.
- For an on-prem Worker Node configure a Filesystem Collector to send data to the Cribl TCP Destination, and from there to the Cribl TCP Source in Cribl.Cloud.
