...
For load balancer configuration examples, see [Configure Third-Party Load Balancers](#lb-configs) below.
...
* For single, high-volume senders (over 300GB/day), use UDP if possible. For both the sender and Cribl Stream, UDP imposes lower overhead than TCP. The stateless nature of UDP allows each log event to be directed to a different worker thread than the last. This ensures maximum utilization of the Worker Nodes. See [Sizing and Scaling](scaling) for more details.
* For lower-volume senders, use TCP if the sender supports it.
* For all other use cases, use UDP.

### Mitigate TCP Pinning

Cribl Stream's TCP Load Balancing feature (available only in Distributed mode) prevents "TCP pinning" by distributing high-volume syslog traffic across all Worker Processes. When enabled, a dedicated load balancer process distributes incoming data via IPC sockets, improving performance. For details, see [TCP Load Balancing](sources-syslog#tcp-lb).
...
](#importing-the-pre-processing-pipeline) to understand the available functions and options and enable or disable them as desired.
...
## Importing the Pre-Processing Pipeline (Optional)

Even before setting up a Cribl Stream Syslog Source, you have the option to install the [Cribl Pack for Syslog Input](https://github.com/criblpacks/cribl-syslog-input) (a.k.a. `cribl‑syslog‑input` Pack), which provides the required pre-processing Pipeline.  

If this is your first time installing from the Cribl Dispensary, see the [full directions with screenshots](packs#import). Otherwise:
...
![Routes page in the Cribl Pack for Syslog Input](syslog-usecase_12.248c5f8c10.png)
...
![The Pipeline's internal documentation](syslog-usecase_06.b7b9684f7c.png)
...
![Stock `SyslogLookup.csv` file, to be filled with customer data](syslog-usecase_05.28ed855f95.png)
...
In the **QuickConnect** UI: Click **New Source**. From the resulting drawer's tiles, select [**Push** >] **Syslog**. Click **Select Existing**, then `in_syslog`.
...
In the [QuickConnect](quickconnect) UI: Click **New Source**. From the resulting drawer's tiles, select [**Push** >] **Syslog**. Click **+ Add New**.
...
![Metadata fields tied to a dedicated-port Syslog source](syslog-usecase_08.5d200b90d9.png)
* **Pre-processing**: From the **Pipeline** drop-down, select `PACK: cribl-syslog-input (Syslog Preprocessing)`.
...
![Pipeline to remove debug events](syslog-usecase_13.96b127f73c.png)
...
![Example of Routes in a Group, each tied to the same sender](syslog-usecase_11.516f45a7da.png)
...
## Configure Third-Party Load Balancers {#lb-configs}

This section provides configuration examples and reference links for setting up third-party load balancers with syslog.
