...
Persistent queuing may be supported for Workers in on-prem deployments and for [hybrid](deploy-cloud#hybrid) Cribl.Cloud Workers, depending on your [Cribl license](https://cribl.io/pricing/). Configuration is considerably simpler with Cribl-managed Workers than with hybrid and on-prem deployments. The following sections describe both configuration scenarios. 

### Configuring PQ for Cribl-Managed Cloud Workers

You configure persistent queueing individually for each Source and Destination that supports it. The following sections describe enabling persistent queueing for Cribl-managed Workers. 
...
When enabled, PQ is initially configured in `Always On` mode, with a maximum queue size of 1 GB per Worker Process. 

#### Destinations

Click a Destination to open its configuration modal, and set **Backpressure behavior** to `Persistent Queueing`. When enabled, PQ is automatically configured with a maximum queue size of 1 GB per Worker Process.
...
You configure persistent queueing individually for each Source and Destination that supports it. To enable persistent queueing for on-prem and [hybrid](deploy-cloud#hybrid) Workers:
1. Click a Source to open its configuration modal. 
2. Select Persistent Queue Settings. 
3. Toggle the **Enable Persistent Queue** slider on.
4. Click a Destination to open its configuration modal, and set **Backpressure behavior** to `Persistent Queueing`.
...
> ##### Source PQ Overactive?{#source-pq-busy}
>
> If you find `Smart` mode causing Source-side PQ to frequently engage, and it's configured with a large **Max file size** (such as, 1 GB), Cribl recommends either of two remedies:
...
>
> For queuing to operate properly, you must provide sufficient disk space. You configure the minimum disk space in **Settings** > **Global Settings** > **General Settings** > **Limits** > **Min Free Disk Space**. If available disk space falls below this threshold, Cribl Stream will stop maintaining persistent queues, and data loss will begin. The default minimum is 5 GB. Be sure to set this on your Worker Nodes (rather than on the Leader Node) when in distributed mode.
>
{.box .warning}
...
These Notifications always appear in Cribl Stream's UI and internal logs, and you can also send them to external systems. For setup details, see [Destination-State Notifications](/stream/notifications#destination).
