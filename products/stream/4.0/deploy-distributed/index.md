...
> 
> With an Enterprise license, you can configure [role-based access control](roles) at the Worker Group level. Non-administrator users will then be able to access Workers only within those Worker Groups on which they're authorized.
>
{.box .warning}
...
2. On the **Groups** tab: For each desired Worker Group, toggle **UI Access** to `On`.
    ...
    ![Worker UI access setting](st-manage-ui-access.833ad63b2f.png)
    {border="true"}
...
![Distributed deployment: menu structure](worker-groups-main-pipeline.a52458a689.png)
{border="true"}
...
![Home page in a single-instance deployment](top-menu-single-instance.2e93bbc9a7.png)
{border="true"}
...
![Commit and Deploy controls per Group/Fleet, at upper right](se-commit-deploy-x2.3b93340071.png)
{border="true"}
...
Click the top nav's **Manage** tab to open the Groups landing page with these upper tabs: **Groups**, **Workers**, and **Mappings**. (With a paid license, you will also see a [Notifications](notifications) tab to their right.)

#### Workers Tab {#workers-tab}

The **Workers** tab provides status information for each Worker Node in the selected Worker Group. 

![Worker Nodes status/controls](workers-status-tab.8b7256289e.png)
{border="true"}
...
##  How Do Workers and Leader Work Together  {#interaction}
...
Worker Groups facilitate authoring and management of configuration settings for a particular set of Workers. To create a new Worker Group, click the top nav's **Manage** tab, then make sure the submenu's **Groups** tab is selected. From the resulting **Groups** tab, click **New Group**.
...
To configure a newly created Group, select the top nav's **Manage** tab, select the **Groups** tab below that, and then click on the Group's name. This displays a submenu from which you can configure everything for this Group as if it were a Cribl Stream single instance – using a similar visual interface for [Routes](routes), [Pipelines](pipelines), [Sources](sources), [Destinations](destinations), and Group-specific **Settings**.
...
![Manage Groups page: Options menu](st-manage-groups-page.232e06f2e1.png)
{border="true"}
...
To create a Mapping Ruleset, click the top nav's **Manage** tab, then click the submenu's **Mappings** tab. Click **New Ruleset**, assign a unique **ID** in the resulting **New Ruleset** modal, and click **Save**.
