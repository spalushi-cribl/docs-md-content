...
> There can be multiple Routes in a Cribl Stream deployment, but each Route can be associated with only **one** processing Pipeline. (Sources and Destinations can also have their own attached pre-/post-processing [Pipelines](pipelines#types-of-pipelines).)
>
{.box .info}
...
![Routes and bytes](routes-2.3.3-wide.250f954e16.png)
{border="true"}
...
Above, note the selectors to toggle between displaying Events versus Bytes, and to display In, Out, or Dropped.
...
![Routes and events (combined menu)](routes-2.3.3-narrow.a4ecabd9cf.png)
...
![Route > Options menu](routes-context-menu.2900acd51d.png)
{border="true"}

Copying a Route displays the confirmation message and the (highlighted) Paste button shown below.

![Paste button for copied Route](routes-copied.c83ec994cd.png)
{border="true"}

Pasting creates an exact duplicate of the Route, with a warning indicator to change its duplicate name.

![Pasted duplicate Route](routes-pasted.e7db5bcfbf.png)
{border="true"}

Comments work like a Pipeline's [Comment Functions](comment-function): You can use them to describe Routes' purposes and/or interactions.

![Comments make the Routing table self-documenting](routes-insert-comment.65708a14e7.png)
{border="true"}
...
If you toggle the **Enable expression** slider to `Yes`, the **Output** field changes to an **Output expression** field. Here, you can enter a JavaScript expression that Cribl Stream will evaluate as the name of the Destination. Sample expression format: `` `myDest_${C.logStreamEnv}` ``. (This evaluation happens at Route construction time, not per event.)

![Output expression enabled](routes-output-expression.d9fdc59d2f.png)
{border="true"}
...
![Non-final Route: add clone fields](routes-clones.15dfc2b222.png)
{border="true"}
...
The `endRoute` bumper appears at the bottom of the Routing table. This is a reminder that, if **no** Route in the table has the `Final` flag enabled, events will continue to Cribl Stream's configured [Default Destination](destinations-default).

![endRoute warning and link](endroute-bumper.de78de1710.png)
{border="true"}
...
![Unreachable Route warning, on hover](unreachable-route-tooltip.e11f6210f0.png)
...
![Unreachable Route warnings, many](routes-unreachable.9a6260bac1.png)
