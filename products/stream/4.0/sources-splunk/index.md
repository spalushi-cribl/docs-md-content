...
**Use Universal Forwarder time zone**: Displayed (and enabled by default) only when **Max S2S version** is set to `v4`. Provides Event Breakers with a `__TZ` field, which derives events' time zone from UF-provided metadata. See [Using the UF Time Zone](#uf-time-zone) below.
...
[tcpout:cribl]
server = in.main-default-<Your-Org-ID>.cribl.cloud:9997
...
> To avoid data loss, Cribl recommends that you use the  `negotiateProtocolLevel = 0` setting shown above. Depending on your environment, enabling `negotiateProtocolLevel` could cause Cribl Stream to not accept data from the forwarder. 
>
{.box .warning}